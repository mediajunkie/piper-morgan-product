...
_INTENTCATEGORY = (
    ...
    "GUIDANCE", "TRUST", "MEMORY", "PORTFOLIO", "PROVENANCE", "UNKNOWN",
)
_TASKTYPE = (
    "ANALYZE_REQUEST", "EXTRACT_REQUIREMENTS", "IDENTIFY_DEPENDENCIES", "CREATE_WORK_ITEM",
    "UPDATE_WORK_ITEM", "NOTIFY_STAKEHOLDERS", "GENERATE_DOCUMENT", "CREATE_SUMMARY",
    "GITHUB_CREATE_ISSUE", "GENERATE_GITHUB_ISSUE_CONTENT", "ANALYZE_GITHUB_ISSUE",
    "ANALYZE_FILE", "SUMMARIZE", "LIST_PROJECTS", "EXTRACT_WORK_ITEM", "JIRA_CREATE_TICKET",
    "SLACK_SEND_MESSAGE", "PROCESS_USER_FEEDBACK", "ANALYZE_DOCUMENT",
    "QUESTION_ANSWER_DOCUMENT", "COMPARE_DOCUMENTS", "SUMMARIZE_DOCUMENT", "SEARCH_DOCUMENTS",
)
...
def upgrade() -> None:
    ...
    if "workflows" not in existing:
        op.create_table(
            ...
            sa.Column("type", postgresql.ENUM(*_WORKFLOWTYPE, name="workflowtype", create_type=False), nullable=True),
            sa.Column("status", postgresql.ENUM(*_WORKFLOWSTATUS, name="workflowstatus", create_type=False), nullable=True),
    ...
    if "intents" not in existing:
        op.create_table(
            "intents",
            sa.Column("id", sa.String(), nullable=False),
            sa.Column("category", postgresql.ENUM(*_INTENTCATEGORY, name="intentcategory", create_type=False), nullable=True),
    ...
    if "tasks" not in existing:
        op.create_table(
            ...
            sa.Column("type", postgresql.ENUM(*_TASKTYPE, name="tasktype", create_type=False), nullable=True),
            sa.Column("status", postgresql.ENUM(*_TASKSTATUS, name="taskstatus", create_type=False), nullable=True),
            sa.Column("input_data", sa.JSON(), nullable=True),
            sa.Column("output_data", sa.JSON(), nullable=True),
            sa.Column("result", sa.JSON(), nullable=True),
            sa.Column("error", sa.Text(), nullable=True),
