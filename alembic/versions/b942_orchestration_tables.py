...
def upgrade() -> None:
    ...
    for enum_name, values in [
        ("workflowtype", ["CREATE_TASK", "ANALYZE_FILE", "GENERATE_REPORT", "EXTRACT_WORK_ITEM", "SUMMARIZE"]),
        ("workflowstatus", ["PENDING", "RUNNING", "COMPLETED", "FAILED", "CANCELLED"]),
        ("intentcategory", ["EXECUTION", "QUERY", "CONVERSATION", "DISCOVERY", "GUIDANCE", "TRUST", "IDENTITY", "MEMORY", "UNKNOWN"]),
        ("taskstatus", ["PENDING", "RUNNING", "COMPLETED", "FAILED", "CANCELLED"]),
    ]:
        exists = conn.execute(text(f"SELECT 1 FROM pg_type WHERE typname = '{enum_name}'")).fetchone()
        if not exists:
