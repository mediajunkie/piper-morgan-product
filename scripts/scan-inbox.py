...
files = [
    f for f in sorted(glob.glob(os.path.join(d, "*.md"))) if os.path.basename(f) != "MANIFEST.md"
]
blank = 0
for i, f in enumerate(files, 1):
    ...
    flag = (
        "⚠ "
        if re.match(r"(URGENT|CORRECTION|RULING|FALSIFIED|WITHDRAWN)", os.path.basename(f))
        else "  "
    )
