...
cycling_now() {
  ...
  while IFS= read -r p; do
    git -C "$REPO" show "origin/main:$p" 2>/dev/null | grep -qE "^(<!--[[:space:]]*)?#{0,4}[[:space:]]*\**[[:space:]]*DAY-CLOSED\**[[:space:]]*[:—-]?[[:space:]]+$today_dash" && return 1
  done <<< "$paths"
  return 0
}
...
# CSV-round-trip corruption detector (added 2026-09-23, CIO, after two same-shape recurrences in
# 24h — root-caused by Docs to csv.writer's QUOTE_MINIMAL silently re-escaping any prose field that
# happens to contain a literal `"`, whenever this TSV is round-tripped through Python's csv module
# for even a single-row edit). A doubled internal quote (`""`) never belongs in this file — it is
# never legitimate content, only ever this specific corruption signature — so a bare grep is exact,
# not a heuristic. Piggybacking on this script rather than a new hook: it already reads this file
# on every fire across all 11 roles, so this is the cheapest path to catching a recurrence the
# moment anyone next runs the belt, rather than waiting for a colleague to notice by chance (as
# happened both prior times).
_csv_corrupt=$(grep -Fc '""' "$REG" 2>/dev/null)
_csv_corrupt=${_csv_corrupt:-0}
if [ "${_csv_corrupt:-0}" -gt 0 ]; then
  echo "⚠️  REGISTRY-CORRUPTION: $_csv_corrupt line(s) carry the CSV-round-trip doubled-quote signature ('\"\"') — data is likely fine for parsing (rows=$_n still counts correctly) but PROSE CONTENT is garbled. See docs/internal/operations/duty-cycle-tick-changelog.log or ask CIO. Do not edit this file via Python's csv module — locate by role-prefix and replace only that line (scripts/trim-registry-history.py is the safe pattern)." >&2
fi
