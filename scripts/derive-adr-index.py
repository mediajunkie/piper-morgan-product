...
def status_bucket(status: str) -> str:
    s = status.lower()
    if "supersed" in s: return "Superseded"
    if "deprecat" in s: return "Deprecated"
    if "dormant" in s: return "Dormant (Proposed, unratified)"
    if "historical" in s: return "Historical"
    if "proposed" in s: return "Proposed"
    if ("accepted" in s or "implemented" in s or "ratified" in s
            or "approved" in s or "complete" in s): return "Accepted"
    if "draft" in s or s.startswith("v0."): return "Proposed"
    return "Other"
...
def generate() -> str:
    ...
    order = ["Accepted", "Proposed", "Dormant (Proposed, unratified)", "Superseded",
             "Deprecated", "Historical", "Other"]
    lines = []
    ...
    total = len(rows)
    missing = [n for n in range(0, max(int(r[0]) for r in rows) + 1)
               if f"{n:03d}" not in {r[0] for r in rows}]
    lines.append(f"**Total ADR files**: {total} · **Numbering gaps (never filed)**: "
...
def main():
    ...
    if "--check" in sys.argv:
        ...
        if current != content:
            print("DERIVED-INDEX DRIFT: committed adr-index.md differs from generated view. "
