...
for f in glob.glob(os.path.expanduser('~/.claude-pm/projects/*/*.jsonl')):
    if os.path.getmtime(f) < WEEK_START.timestamp(): continue
    for line in open(f,encoding='utf-8',errors='replace'):
        ...
        if not t or t < WEEK_START: continue
        rid=d.get('requestId') or d.get('uuid')
        if rid in seen: continue
        seen.add(rid)
        rows.append(dict(
            t=t, model=m.get('model') or '?', cwd=d.get('cwd') or '',
...
def cost(r): return sum(r[k]*W[k] for k in W)

for r in rows: r['seat']=seat(r['cwd']); r['c']=cost(r)
