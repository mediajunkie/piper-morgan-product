...
PULL_TRIGGER_PATTERNS: List[Tuple[re.Pattern, str]] = [
    ...
    (
        re.compile(r"\bwhat (?:do you|did you|have you) figure[d]? out\b", re.IGNORECASE),
        "direct-figured",
    ),
    ...
    (
        re.compile(r"\bwhere did (?:that|this) (?:insight|come from)\b", re.IGNORECASE),
        "source-inquiry",
    ),
    (
        re.compile(r"\btell me about your (?:insights|learnings|observations)\b", re.IGNORECASE),
        "direct-tell",
    ),
    (
        re.compile(r"\bhave you noticed (?:anything|any pattern)s?\b", re.IGNORECASE),
        "direct-noticed-anything",
    ),
]
...
def matched_trigger_labels(user_message: str) -> List[str]:
    ...
    if not user_message:
        return []
    return [label for pattern, label in PULL_TRIGGER_PATTERNS if pattern.search(user_message)]
