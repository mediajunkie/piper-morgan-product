...
async def test_save_route_refuses_409_BEFORE_keychain_write(monkeypatch):
    ...
    assert (
        not stored
    ), "the keychain write must NEVER precede the gate — 'wasn't saved' must be TRUE"
...
async def test_save_route_writes_when_flag_set__positive_control(monkeypatch):
    ...
    assert (
        stored
    ), "flag-set save must reach store_api_key — else the refusal test's belt is vacuous"
