...
def test_radar_entity_is_a_value_object():
    e = RadarEntity(EntityType.CONVERSATION, "auth migration", "active",
                    Provenance.OBSERVED, meta="1h ago", attention=5.0, ref="c1")
...
async def test_feed_filters_non_observed_from_default_view():
    seed = RadarEntity(EntityType.WORK_ITEM, "seed card", "active",
                       Provenance.SEED, attention=99.0)
...
async def test_conversation_source_maps_summary_as_observed():
    ...
    summaries = [{
        "conversation_id": "c1", "title": "auth migration",
        "last_activity": now_iso, "turn_count": 3, "topics": [], "preview": "...",
    }]
...
async def test_conversation_source_dormant_for_old_activity():
    ...
    summaries = [{"conversation_id": "c2", "title": "old", "last_activity": old_iso,
                  "turn_count": 1}]
