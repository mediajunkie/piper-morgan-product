...
async def test_radar_populated_is_attention_first_and_observed():
    ...
    # [:-1]: the populated response ends with the #1635 coming-soon placeholder
    # (example provenance, always last — pinned in test_coming_soon_1635.py).
    assert [e.title for e in view.entities[:-1]] == ["new chat", "old chat"]
    assert all(e.entity_type == "Conversation" for e in view.entities[:-1])
...
async def test_radar_reminder_provider_failure_never_blanks_feed():
    ...
    assert view.state == "populated"
    # [:-1]: the populated response ends with the #1635 coming-soon placeholder
    assert [e.title for e in view.entities[:-1]] == ["chat"]
