...
async def delete_test_user_fully(session, user_id: str) -> None:
    ...
    for stmt in (
        ...
        # slack_link_attempts: no user FK (rate-limit ledger keyed by Slack ids) — blanket-cleared in fresh_database only
        "DELETE FROM slack_link_codes WHERE user_id = CAST(:u AS uuid)",
        "DELETE FROM slack_identities WHERE owner_id = CAST(:u AS uuid)",
