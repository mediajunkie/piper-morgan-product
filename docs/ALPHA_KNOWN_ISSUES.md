...
| **BYOC credentials** | Complete for chat | Keys stored per-user, encrypted at rest; per-request routing; any single provider suffices | Key-validation reasons are coarse — "invalid" for auth AND quota errors ([#1718](https://github.com/mediajunkie/piper-morgan-product/issues/1718)) |
