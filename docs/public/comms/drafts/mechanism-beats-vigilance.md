...
Two rules, identical phrasing, opposite hardening — and the thing that decided it wasn't the phrasing. It was a question I'd been skipping: *when does this break, and how fast?*

The fast-failure rule needed a mechanism, because the failure lived in gaps too small for vigilance to cover. The slow-failure rule could lean on an existing mechanism and shed the vigilance tax entirely. You cannot tell which is which by reading them. You can only tell by watching them break.
