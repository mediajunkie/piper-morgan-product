...
PA sent it as an urgent finding, with the caveats stated plainly: this measured one specific thing, and someone should check what was actually running.

## Everyone checked, and everyone was wrong the same way

Over the next two days, several people did check. My chief of staff agent (Exec), my principal product manager agent (PPM), and my communications agent (Comms) each looked at the gap between the two branches and got numbers in the same alarming range. Each of them reported it as confirmation.
...
My experience-design agent (CXO) picked the thread back up the next morning with one thing already in mind: don't re-run the git comparison that five people had now gotten wrong the same way. Instead, CXO went one layer further than anyone had gone — not the source branches, not even the deployment log, but a direct shell into the actual running container, checking what code was physically present on the machine serving real traffic.
...
The real finding held up exactly as PA had originally raised it: the security gate genuinely wasn't live where users would meet it. That part had been true from the first memo. What had been wrong, for two days, was everyone's sense that the alarming number was itself established fact, because so many people had independently landed on something close to it.

## Why agreement felt like proof

It's a natural mistake, maybe the most natural one there is. If four people check something and get similar answers, the ordinary instinct is to treat that as corroboration — different people, same conclusion, must mean the conclusion is solid. But corroboration requires actually independent methods. Four people running the identical measurement and getting the identical kind of wrong answer is one mistake, made once and repeated by everyone who inherited the same instinct for how to check — not four confirmations.

The fix was one person checking a genuinely different way, closer to the thing that actually mattered, with nothing left to infer — not more people checking the same way, faster.
