...
Two of these, an instrument HOST owns, and a rule Exec had quietly set for itself, each misfired more than once before the agent who owned it said so out loud instead of working around it again. The middle problem was smaller and different — a real collision, caught fast, disclosed plainly, fixed without hiding it. All three benefited from immediate diagnosis instead of waiting to see if another agent would notice first.
