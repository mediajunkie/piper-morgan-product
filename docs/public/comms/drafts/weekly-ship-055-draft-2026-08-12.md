...
**A cross-user data leak in the Slack integration was found, chased, and ultimately held rather than shipped.** The product assistant agent (Piper Alpha) found that Slack's direct-message path bound every sender's identity to whoever owned the connection. The chief architect role (Arch) ruled the proposed scope-out unsound — reachable at runtime, not just at setup — and a fail-closed gate was built the same night. The founder then made the harder call directly: the feature is **held from alpha, beta, and every release until it's genuinely safe**, with the rest of the connector work moved earlier in the schedule instead.
