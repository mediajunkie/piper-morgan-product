...
alt: An inchworm traverses a branching numbered tree while a human and glowing AI-being observe its precise progress through the hierarchy.
caption: "How does that song go?"
---
...
In my session log: "Inchworm Position: 3.1.4.1"

That notation told me exactly where we were. Not vaguely "working on alpha prep." Precisely: the current rung in a nested work hierarchy, with explicit labels for each level: 

(3) ALPHA foundation
(3.1) Initial alpha testing - v0.8.0
(3.1.4) Sprint A9 - final alpha prep
(3.1.4.1) Frontend permission awareness

The inchworm's current position on the branch.
...
Project progress works the same way, especially when it hinges on the attention of one single human being (and there is always some subtrack of every project that does, for what it's worth). You can't be everywhere at once. You're at one specific position, working on one specific thing. The discipline is knowing exactly which thing, and having the whole branch mapped so you know what comes next.

The position notation captures this: where you are in the structure of work, not just what you're doing.

## Why position matters

The exact numbering and order of subbranches changes all the time. It has to. This is not a waterfall tracking system. This is my equivalent of externalized "where were we?" memory in a form that I can manage with my linear attention, by walking the branches. The tree holds the complexity.
...
This creates productive constraint. You can't be at position 3.1.4.1 while still having incomplete work at 3.3.2.6. The position asserts: everything before this is done.
...
Position is precise. "3.1.4.1." Either you're at that position or you're not. The notation forces honesty about where you actually are versus where you wish you were.
...
Position only works if the branch is mapped. You need to know what 3.1.4.1 means — what came before, what comes after, how the segments connect.
...
Each session starts with position check: "Where are we?" Most of the time I'd mention "we're on 3.1.4.1" or whatever. Occasionally I'll reshare the current state of the tree. I used to keep it outside the project in a Notes file, then adopted a Mac app called Bike that is pretty easy to use but pastes plain bullet lists. But that's just tooling. It could be on paper.

This is as much for me as it is for the bots. 
...
The inchworm that knows its position doesn't backtrack. It doesn't wonder what's done — it knows what's done because it knows its position. Slow is smooth and smooth is fast.
...
*Next on Building Piper Morgan: Audit and Talk — the day an IAC talk on ethics-as-architecture and a CIO methodology audit converged.*
