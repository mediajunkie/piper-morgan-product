...
A few weeks ago I added a file called `DECISIONS.md` to one of my projects. It's a lightweight log — date, decision, rationale — meant to keep the morning-brief routine from re-litigating the same questions every Monday. The file is a hundred and fifty lines now and it's saved me at least a dozen of those re-litigation cycles.

I didn't invent it. The pattern came in from a sibling project. *Klatch*, an entity-management system being built in parallel, had already adopted `DECISIONS.md` six weeks earlier; *OpenLaws*, a third project in the same ecosystem, had something very similar. Once the cross-pollination brief surfaced the convergence — *both your siblings have the same file shape solving the same problem* — adopting it for Piper Morgan took five minutes.
...
The SSH-over-port-443 workaround I now use when conference Wi-Fi blocks port 22 came in from *Calliope*, the citation-and-authority engine in the OpenLaws stack, after one of their agents got stuck on the same network constraint. They wrote the workaround into their CLAUDE.md. The cross-pollination brief picked it up. I dropped it into mine.
...
The handoff-memo template I use when a Chat-based agent role migrates to Code came out of a six-section structure my Chief of Staff agent drafted a month ago. By the time HOST and CIO had both used it, two parallel projects had picked up similar templates, with their own modifications: Klatch added a "what I'd tell my successor that I wouldn't tell the PM" section; OpenLaws collapsed the relationship section into the lessons one. None of these are forks. They're cousins.
