...
- **OPEN reconcile (durable:true) — Arch F4 vs PA, 2026-06-08**: Arch reports a `durable:true` cron survived + fired cross-session (Mon 07:03 from a Sun set); PA verified `durable:true` is a **no-op** (session-only message, no `scheduled_tasks.json`). **Likely confound**: Arch's cron firing only proves *durable persistence* if Arch's session was NOT continuously alive across the fire (else it's ordinary session-alive firing, consistent with PA). **Clean test queued** (Arch+PA): (1) does `.claude/scheduled_tasks.json` exist for a durable cron? (2) does a durable cron fire after the session is *fully terminated*? **This gates the watchdog framing** — if durable genuinely works, it's a far cheaper Gap-C floor than the $70/mo Routines watchdog (they'd compose: durable = cheap floor, watchdog = catch). Hold the watchdog escalation until reconciled.
