...
- **Update your cron prompt template** if it embeds the expression. **⚠️ Gotcha (CIO caught on its own session 6/11): the live cron rotation is NOT enough — you MUST write the new schedule into the cron prompt's CONSTANTS too.** The Gap-C self-heal (Step-1: "zero crons → re-arm") re-arms *from the prompt's constants*; if a restart kills your windowed cron and your prompt still says the old hourly shape, self-heal silently recreates the OLD shape → you're back on hourly without noticing (frequent this restart-heavy week). Belt-and-suspenders: add a line to your prompt — *"if self-heal re-arms, use THIS expr, not an older hourly shape."* **Anyone who rotated the live cron only should CronList-check they didn't already revert.**
