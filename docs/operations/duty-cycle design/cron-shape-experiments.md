...
|---|---|---|---|---|
| **Arch** | bursty (burst-then-drained) | **every-3-hours at `:52`** (`52 */3 * * *` — 8 fires/day vs 24 hourly, ~67% fewer fires; matches HOST's shape) | 2026-06-03 07:35 PT (cron launched) | **Hypothesis**: bursty-lane work clears in substantive-burst, so 3hr interval catches signal with far less no-op overhead than hourly. **Watch**: no-op rate, missed-signal incidents (mail sat >3hr that mattered), whether 3hr is too sparse when backlog returns (e.g., active ADR work) → revert toward hourly. Or tune toward 2×/day if drained-state persists. Will memo CIO with findings after ~1 week observation. |
