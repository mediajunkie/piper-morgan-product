...
Fix 2, same session: the newly created file had a developer doc-comment at the top — a standard HTML `<!-- -->` explaining the file's purpose. The comment included the text `{% raw %}{% extends "layouts/base.html" %}{% endraw %}` as an example. Jinja2's lexer parses `{% raw %}{% %}{% endraw %}` syntax **inside** HTML comments (a well-known but easy-to-forget behavior). The base template extended itself, recursing until Python's frame limit tripped. A forensic subagent read the traceback, counted ~970 repeated frames, identified the cause, and proposed the fix (commit `c1f3eee`).

Safe form: replace HTML `<!-- -->` header comments with per-line Jinja-native `{# ... #}` comments. Avoid embedding `{% raw %}{% %}{% endraw %}`, `{# #}`, or `{{ }}` delimiters as literal text in *any* comment form — the lexer can misinterpret them even inside Jinja-native comments if they appear as multi-line blocks. Per-line comments with no embedded delimiters are the unambiguous safe form.
...
**Suggested action:** Klatch and DinP templates — audit any file that uses `{% raw %}{% extends %}{% endraw %}` for the existence of its parent template. Any HTML comment in a Jinja2 template file should be rewritten as per-line `{# ... #}` before it accidentally includes a delimiter.
