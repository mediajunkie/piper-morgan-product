...
**Recommended next**: (1) **disable Belt 0's foreground** (`WATCHDOG_AUTO_FOREGROUND=0`) — it doesn't resume + risks hourly focus-steal; keep the nudge backstop (PM call, since PM approved (a)). (2) **Scope (b): off-machine trigger** that re-creates/wakes the *specific* role session — the one-shot `scheduled-task`-fires-a-resume shape (how Iris was resumed) is the most concrete lead; it sidesteps both the multi-window-targeting problem AND mode-1a (it spawns the session fresh). The persona-fork concern is moot for a *dead/stalled* role (nothing concurrent to fork).
