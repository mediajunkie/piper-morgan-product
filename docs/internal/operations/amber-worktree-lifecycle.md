...
**✅ Closed — the fourth assertion now exists and is mechanized.** This block previously read *"a fourth assertion this spec still needs: verify hooks actually fire"*, and diagnosed the cause as project hooks not firing in Model-A worktrees. **That diagnosis was wrong** (HOST, same day): the real cause was an invalid hook matcher — `Bash(git commit*)` is permission-rule syntax in a field matching tool *names* — which had killed those hooks on every host and account since introduction. Nothing to do with worktrees. Rule 4 now exists, and `amber-agent verify-hooks` mechanizes it at provisioning time. Kept the retraction visible rather than deleting the block, since the wrong diagnosis reached four canonical surfaces before anything tested it.
