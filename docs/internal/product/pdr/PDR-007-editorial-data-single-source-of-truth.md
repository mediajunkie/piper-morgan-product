...
**Status**: ✅ **RATIFIED 2026-08-28 — Arch, Web, and CIO all reviewed with no objection. PDR-007 closes as adopted-without-migration (Option A).** PM asked for this draft 2026-07-29 after raising the underlying question directly.
**✅ CIO boundary-question review COMPLETE 2026-08-28** — read m-36 and m-44 in full rather than pattern-match on the word "measurement." **Ruled: neither.** Not m-44 (that pattern is specifically about a *check's* all-clear being emitted identically whether it measured correctly, wrongly, or not at all — nothing here "measured" anything and reported false). Already covered, in spirit, by **m-36 Class 1** ("state that must be kept current by attention goes stale; the fix is a derived view computed at read time") — the principle generalizes past hand-maintained trackers to any "stored field asserting an external fact whose truth can drift independent of the state's own update," which is exactly `draftPath`'s shape. **Not filing a new entry or amending m-36 today** — PDR-007's own zero-drift result means the current design already works in practice for this case; CIO is watching for a second, non-editorial instance before treating the generalization as worth naming formally.
**✅ Measurement window CLOSED 2026-08-27 (Docs, running the shipped instruments rather than let the window expire unmeasured) — all three pre-registered criteria hold.** `scripts/measure-editorial-drift.py`: Class 2 stale `draftPath` = **0** (criterion: 0, met) · Class 3 field-level disagreements = **17** (criterion: ≤17/no-growth over the 07-29 baseline, met exactly, zero growth). `scripts/validate-editorial-calendar.py`: Class 1 column-shift = **0** hard failures (exit 0; 15 advisory warnings, none Class-1 — a pre-existing caption/cartoon-mismatch class with its own documented tiebreaker, and legacy `theme='shipping news'` vocabulary drift, both explicitly non-blocking by the validator's own design). **Per the pre-registered rule, Option A is sufficient — PDR-007 closes as adopted-without-migration**, pending CIO's still-open boundary-question review (§Implications) to formally ratify. Not declaring this ratified unilaterally — that's CIO's call to complete, not mine to skip — but the numerical outcome the window was designed to produce is now in hand, not stale.
...
**✅ CIO's ruling (2026-08-28)**: neither m-44 nor a new entry — **already covered, in spirit, by m-36 Class 1**, generalized past hand-maintained trackers to any stored field asserting an external fact that can drift independent of its own update. Not amending m-36 today given PDR-007's own zero-drift result; watching for a second, non-editorial instance first. See the Status banner above for the full text.
