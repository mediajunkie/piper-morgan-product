...
- ⚠️ **UNDER ACTIVE QUESTION — do not read as settled, but materially better than this line once said.** ~~A BYOC ChatGPT user can add `mcp.pipermorgan.ai` as a remote MCP and individual skills and get equivalent core capabilities~~
  > **CORRECTED 2026-09-15 (PA) — the "untested mechanism" below was tested the same day this line was written, and never made it back here.** #1462's body carries the actual result (PA, N=6/cell, re-scored, 2026-08-02): prose framing survives 17% of the time on GPT; a structured field triples that to 50% (the figure this line stopped at); but **a consequential refusal emitted as a failure-shaped payload** (ordinary tool-result content that reads as a failure, `{"error": "REFUSED", ...}`) reaches **6/6 — 100%, matching Claude.** The sharper finding: **the effective variable is framing, not transport channel** — the GPT arm never used a protocol-level error at all, so this needs no MCP error semantics and no host cooperation to ship. **Known limit, not yet closed**: these probes exercise the provider APIs directly, not the shipping Claude/ChatGPT products against a deployed `mcp.pipermorgan.ai` — a deployed-host retest remains a real gate (CXO's own caveat) before calling this settled.
  > **So**: honest-decline is no longer an open capability gap with an untested fix — it's a tested, portable mitigation pending one verification pass once the server exists. PPM's wording-proposal-pending-a-mechanism framing is superseded by this; worth PPM/PM revisiting the criterion's wording now that the "untested mechanism" premise no longer holds.
  > *(setup)*
