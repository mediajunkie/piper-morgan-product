...
1. 🔴 **The MCP caller-identity boundary must be FAIL-CLOSED.** This is the one real architectural risk in the mechanism set. **Every MCP call must resolve to an `owner_id` before touching server state — no identity, no read, never defaulting to a system or anonymous owner.** The reason it is load-bearing: **all existing ADR-079 owner-scoping enforcement sits DOWNSTREAM of this mapping.** If a tool handler can reach a repository with a caller-chosen identity, the derived `check_unscoped_reads` lint cannot save us — the read *looks* correctly owner-scoped while the **owner** is the forged thing. A multi-tenant server is a strictly harder problem than the anonymous-caller one #1351 was scoped to.
   - ✅ **PREMISE VERIFIED AT THE PRIMARY SOURCE 2026-08-05 (PA raised the risk; Arch fetched the reference).** PA flagged that the plugin reference documents `mcpServers` **only** as local `command`-based servers, which would break this PDR's whole distribution path — a plugin connecting to a **hosted** `mcp.pipermorgan.ai`. **The capability exists.** `code.claude.com/docs/en/plugins-reference` documents four transports, in the placeholder-substitution table rather than in the `mcpServers` examples (both of which *are* command-based, so PA's reading of that section was correct):
     ...
     ⚠️ **Anyone re-checking this must use `code.claude.com/docs/en/plugins-reference`** — the older `docs.claude.com/en/docs/claude-code/…` path **301-redirects**, and a fetch of the old host is a plausible reason to come away with a different picture.
     ⭐ **And `headersHelper` is directly load-bearing for condition 1**, not merely permissive: it is the documented mechanism by which a plugin supplies **dynamic** auth headers per request — i.e. the transport by which an OAuth token or API key reaches the hosted endpoint and becomes the `owner_id` the fail-closed boundary resolves. **The identity boundary has a supported carrier; it does not need to be invented.**
...
⚠️ **Ratified ≠ shippable** — [#1458](https://github.com/mediajunkie/piper-morgan-product/issues/1458) (cross-caller state isolation) remains open, unstarted. The recomposition rubric gate ([#1463](https://github.com/mediajunkie/piper-morgan-product/issues/1463)) closed 2026-09-01 with the branch delivered (`byoc-recomposition-rubric-v0.1.md`, now **v0.6** as of 2026-09-06 — both vendors tested across multiple rounds, not the single vendor pair this line previously described) — ⚠️ **but its T axis still scores `PENDING-PROBE` and cannot issue a pass** (n=1 per cell per vendor throughout, by the instrument's own design; CXO's governing hypothesis for the axis was revised three times, v0.2→v0.3→v0.4→v0.5, before PA's member-not-metadata mechanism produced a clean pass in both vendors, first try, on the one shape tested). The instrument can inform design decisions; it cannot yet close a Layer-B gate on honesty-under-recomposition — CXO's own ruling (2026-09-06) was to stop the probe series and hand the mechanism to Lead as-is, with a named re-trigger (a second class-B case where the mechanism fails), not to keep chasing a statistical pass. *(Re-verified 2026-09-12, PA — the prior "v0.4, one vendor pair" text had gone stale by two full rounds; caught while assessing hosted-alpha readiness against PM's direct ask, per the same "verify a document's own claims against current state" discipline this section's own provenance note below required once already.)*
...
- ⏳ **PROVISIONALLY CONFIRMED, pending one deployed-host retest — wording revised 2026-09-15 (PPM), superseding the "under active question" framing below.** A BYOC ChatGPT user can add `mcp.pipermorgan.ai` as a remote MCP and individual skills and get **honest-equivalent** core capabilities: when Piper must decline, the decline survives ChatGPT's recomposition as reliably as it does on Claude. *"Equivalent core capabilities"* (the original wording) implied parity nobody had tested; *"honest-equivalent"* names the specific property that's actually confirmed — the refusal, not the full capability surface — and is the property this PDR's differentiation argument actually depends on (§"Testable NOW" above).
  > **Why the wording moves now rather than waiting for the retest**: the mechanism (a failure-shaped payload, not a protocol-level MCP error) is confirmed at 6/6 against provider APIs directly (PA, N=6/cell, 2026-08-02, result recovered into this doc 2026-09-15 after not propagating the first time) — my prior hold was on that mechanism existing at all, and it does. What's still owed is a **verification pass, not new design work**: the same probe re-run against a live `mcp.pipermorgan.ai`, which can't happen before the server exists. Holding the wording any longer than that would be optimizing for a settled-looking sentence over an accurate one — the exact mistake this week's epic-2 thread cost real hours to correct elsewhere.
  > **What "provisionally confirmed" licenses and doesn't**: licenses citing this as the differentiation argument's honesty leg in planning and Comms framing now. Does NOT license marking this criterion fully met, or removing it from Phase-0-gate tracking, until the deployed-host retest actually runs.
  > *(setup)*
...
| — | 2026-09-12 | PA | Corrected stale #1463 rubric-status line (was citing v0.4/one-vendor-pair; actual state is v0.6, both vendors, PA's member-not-metadata mechanism) while assessing hosted-alpha readiness per PM's direct ask. Note: this changelog table itself is incomplete relative to the document's actual revision history (many undated inline corrections exist above, e.g. 09-02's T-axis-caveat fix) — not backfilled here, flagged for whoever next does a full PDR-006 pass. |
| — | 2026-09-15 | PA | Corrected the ChatGPT honest-decline OQ3 note — the "untested mechanism" it named (failure-shaped payload framing) was actually tested the same day (#1462's body carries the result: 6/6, 100%, matching Claude) but the result never made it back to this doc. Found while building the hosted-alpha readiness checklist. |
| — | 2026-09-15 | PPM | Revised the ChatGPT-equivalence criterion's wording, per PA's finding above superseding the premise the held wording-proposal was waiting on. Moved from "under active question" to "provisionally confirmed, pending one deployed-host retest" and narrowed "equivalent core capabilities" to "honest-equivalent" — naming the specific tested property (decline-survives-recomposition) rather than implying a fuller parity nobody has tested. Does not close Phase-0-gate tracking; the deployed-host retest is still owed. |
