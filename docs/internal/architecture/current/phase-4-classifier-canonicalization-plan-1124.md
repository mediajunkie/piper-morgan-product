...
**Status**: PLANNING ✅ COMPLETE · **Q1+Q2 RATIFIED by Arch 2026-06-07** (`memo-arch-to-lead-...-phase4-plan-ratified-q1q2-2026-06-07.md`) → **BUILD UNBLOCKED**. Phase -1 ✅, decisions ✅, audit-cascade ✅ verified, shim spec ✅, Phase-0 build-prep ✅. Gated phase — build proceeds behind the canonical-retest gate (needs a live `/api/v1/intent` + LLM run, same auth-limit as #1155 UAT). Build order: (1) shim `verb_sourcetype_to_legacy_action()` + tests [solo-safe]; (2) prompt big-bang behind canonical-retest [needs live gate]; (3) migrate 6 consumers off legacy aliases, one commit each; (4) retire shim → enables Phase 4.x enforce-floor.
