...
- **OQ-1 (build)**: extend `PersonalityProfileRepository` / a profile table, or a dedicated personalization store? (Lead's call at build.)
  - **→ RESOLVED at Component B build (2026-07-06, Lead)**: a new dedicated table (`personalization_contexts`). `PersonalityProfileModel` holds HOW Piper talks (warmth/confidence-style/action-orientation/technical-depth — tone traits); this is WHAT Piper knows about the user (system-prompt content) — different concerns, and the existing table's `user_id` column is the deprecated string pattern (not ADR-071 D2's canonical `owner_id` FK), so extending it would also require migrating that column. Cleaner to start the new store on the canonical pattern directly.
- **OQ-2 (classification)**: is `default_labels` per-user (category 1) or install-wide (category 3)? Decide at build from actual usage.
  - **→ RESOLVED at Component B build (2026-07-06, Lead)**: category 1 (per-user — different testers may reasonably want different default issue labels), but it's connector-config, not system-prompt personalization — belongs with the *existing* `ConnectorConfigService`/`connector_configs` (same home as `default_repository`), not a new personalization-content store. Flagged as a separate, smaller follow-up; not pulled into Component B's build.
- **OQ-3 (trust — for CXO/HOST)**: the neutral-default personality/context a non-PM principal gets on a shared instance — what is it, and is the "you're seeing the default" surfaced transparently or silent?
  ...
  - **→ CXO UX direction RESOLVED 2026-07-06 (PASS); OQ-3 now FULLY RESOLVED, v0.2 ACCEPTED cut.** CXO's Component-B-scoped UX: (1) **surface = first-response injection** — appended *after* Piper's first substantive answer to a no-record principal, never before (capability first, metadata second: "answer the question, prove useful, then invite personalization" — the colleague register), one-time (never reappears post-personalization / post-seen, whichever Component B tracks cleaner), never per-response; (2) **register = capability-affirming casual parenthetical** — one sentence, "fully useful as-is" (explicitly NOT a degraded/error state), actionable (→ Settings → Profile), non-catastrophizing (no warning/alert styling); example copy *"(Running with a default configuration for now — I'm fully useful as-is, but once you add your context in Settings → Profile, I'll be tuned to your role and priorities.)"* (Lead adjusts to Piper's established voice at build); (3) **the seeded neutral-default persona = a professional product-management assistant** (role="PM assistant", style="direct/concise/colleague-register", domain="product development + team coordination") — genuinely useful out-of-box, NOT PM's portfolio/priorities/repos, NOT a generic chatbot, NOT blank. HOST ratified (all four HOST conditions map onto CXO's answer). **OQ-1 (store shape) + OQ-2 (`default_labels` classification) remain build-time (Lead's call at Component B) — they do NOT gate ACCEPTED.**
