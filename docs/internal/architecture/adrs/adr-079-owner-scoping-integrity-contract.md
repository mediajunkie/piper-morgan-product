...
**D4a — the rationale names WHICH KIND of global, and contingently-global entries self-expire (HOST trust-lens, 2026-07-19).** The CLEARED set holds two kinds, which must be distinguished because they decay differently:
- **Constitutively global** — no per-user version could exist in any architecture (e.g. the Slack socket-mode connection is a singleton by protocol). Stable rationale.
- **Contingently global** — a platform default *today* that acquires a per-user version under a future model (e.g. LLM provider keys: platform-default now, per-user in BYOC/M4). An allowlist entry correct under the current model becomes **silently wrong** in the future one — the exact "exemption that decays" this contract exists to prevent, one horizon out.
So a contingently-global rationale carries a **self-expiring review trigger**: `"Platform-default credential; per-user override exists in BYOC — review at M4 landing."` One clause, no separate tracking issue, and the lint's `# global-ok:` grep surfaces every such entry when M4 lands. HOST's D5 endorsement was clean; this is the D4 sharpening folded.
