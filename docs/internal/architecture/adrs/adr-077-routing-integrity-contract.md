...
> a canonical is *reachable* iff it is handled via **rail ∪ category ∪ pre_classifier ∪ floor-internal-dispatch ∪ action-mapper (LLM→ACTION_MAPPING→elif) ∪ intentional-floor-allowlist**.

(The **action-mapper surface** was added 2026-07-17 — derived from `ActionMapper.ACTION_MAPPING.values()`, never hand-listed, so it can't drift. It is the honest recognition that LLM→mapper→elif is a real fifth dispatch surface, documented by census D. Caveat: mapper-*only* reachability is *reachable* but **mode-4-fragile** (an LLM paraphrase the mapper has no alias for still misses) — the D4 lint attests *reachability*, not *robustness*; fragility is the D5 behavioral corpus's concern, and the deterministic-preference (migrate live/critical paths to pre_classifier/rail) still holds.)
...
**Scoped-gap note (2026-07-15, #1411) — RETIRED 2026-07-17.** *The gap it described (elif-only `mapped_action` handlers invisible to the lint) is now closed by two composing guards: (1) the **forward-guard** (`TestForwardGuardExecutionCohort`) asserts every `mapped_action`-dispatched token is a **registry canonical** — membership; and (2) the D4 predicate now includes the **action-mapper surface** (above) — reachability. So the whole `mapped_action` cohort (create_issue/update_issue via #1411/#1412, the todo cohort migrated 2026-07-17) is registry-registered + reachability-checked by construction; a new elif-only handler fails the forward-guard at birth. The lint can now be trusted to catch this class.*
