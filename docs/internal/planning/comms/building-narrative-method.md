...
- **Footer teases the very next scheduled calendar item, regardless of category (building/insight) — but never a Weekly Ship.** Skip past any Ship in between and tease the next narrative or insight. (`memory/feedback_footer_teases_next_post_on_calendar_any_category.md`.) ⚠️ **Correction 2026-08-18 (PM)**: caught on "The Architect's Own Trap" teasing Weekly Ship #056, the item immediately after it on the calendar — PM: "we don't do that." Ships already carry their own backward-looking list of the week's other posts (§2 table, "Ship variant"); they aren't a forward tease target from a narrative/insight footer. The underlying memory pin needs the same correction — it currently says "any category" without the Ship exception.
