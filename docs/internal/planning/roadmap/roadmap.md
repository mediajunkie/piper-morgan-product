# Piper Morgan Roadmap v18.8

**Date**: July 16, 2026 (v18.7 fold — narrative body below is current only through this date; see v18.8 changelog immediately below for a stale-flag correction, not a full re-fold. v18.7 fold: #1394 architecture COMPLETE + Production 1.0 connector gate defined + Finish-the-Unfinished sprint ratified — reconstructed by PPM after a ~22hr session gap during which PM made these calls in-conversation with Lead; body narrative otherwise per v18.6 July 12, 2026)
...
**v18.8 changelog** (August 24, 2026, PPM): **Stale-flag correction only — NOT a full fold.** This document's narrative body (sprint triage, milestone dates, changelog detail) reflects state as of July 16, 2026 (v18.7) and had gone over five weeks without a header update despite real project movement — flagged by Docs (#1644, first flagged 2026-08-17, repeated 2026-08-24: `mailboxes/ppm/inbox/2026-08-24-docs-briefing-current-state-still-stale-second-flag.md`). This entry corrects the header date honestly; it does not attempt the full v19 historical fold that's actually owed (tracked as the open half of #1644). Notable since v18.7 not yet reflected in the body below: PM moved the beta target back a month on 2026-08-08 without setting a new fixed date (MVP milestone remains the gate — see `beta-blockers.md` and #1386, whose criteria 1/4/5/6 are still open as of this writing); first-contact criterion ratified 2026-08-15 (`docs/internal/product/first-contact-criterion-merged-2026-08-10.md`); surfaces taxonomy ratified v1.0 2026-08-21 (`docs/internal/design/surfaces-taxonomy-2026-08-16.md`); FTUX experience model landed 2026-08-21 (`docs/internal/design/ftux-experience-model-2026-08-21.md`). For current sprint/gate counts, use `scripts/sprint-truth.py` directly and `docs/briefing/BRIEFING-CURRENT-STATE.md` (refreshed same-day, 2026-08-24) over this document's dated narrative sections.

**v18.9 changelog** (August 30, 2026, PPM): **New canonical doc, not a fold of this one.** The
audience/milestone gate model (v18.4's "MVP = beta gate," v18.7(kk)'s Production connector-gate
precedent, and 2026-08-30's ESSENCE v1.0 ruling adding a PUBLIC-BETA GATE keyed to MCP-path
completion) is now written up as one citable document: **[release-model.md](../release-model.md)**
— per Arch's ask on ESSENCE v1.0 ratification, so this question stops needing re-derivation from
scattered changelog entries. This document's own gate-model prose (§ narrative body, still current
only through July 16) is superseded by that doc going forward for anything touching audience
stages; this roadmap remains canonical for scope/sprint narrative. #1688 moved MVP → Production
(front-loaded, Sprint `PUB - Public Beta`) same day, per the same ruling — detail in
`release-model.md` §4, not duplicated here.
...
All 16 open M4-sprint issues reviewed Jul 4-5: 15 moved to Production milestone as enhancements/infrastructure/scoping work not required to gate beta (#302, #558, #712, #713, #954, #955, #956, #1062, #1166, #1174, #1217, #1242, #1244, #1245, #1326), and #1190 (destructive-mutation confirmation gate) also confirmed Production once the GitHub-write investigation showed it's a narrow UX-polish item, unrelated to credential routing. **#1032** (push-insight trust-gating) is already CLOSED, separate from this sweep. **#1216** (honest-provenance data model, `source`/`is_seed` on `InsightDB`) — ✅ **CLOSED COMPLETED 2026-07-07, milestone MVP** *(corrected by PPM 2026-08-06)*. ⚠️ **This line claimed for a month that it "remains OPEN" and was a "Beta Blocker candidate pending PM's call."** It was neither by the time anyone read it: landing in MVP means it was treated as beta-gating, so **the call I was waiting on had already been made by action rather than by answer** — and I kept carrying it as PM-gated. Its failure shape (confidently claiming seed data is real) does parallel #1331, and that reading stands. It was correctly NOT part of the M4 sprint triage (no matching Sprint-field tag).
