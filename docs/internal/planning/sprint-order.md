...
| #542 | SEC: Implement actual token revocation on disconnect | Real trust property -- a disconnected tester's token must actually stop working; flagged on PPM's earliest hard-gate list, resolved into Beta Blockers Jul 5 |
| #1305 | SEC-ENCRYPT: encrypt PII-bearing JSON/JSONB structured columns (deferred from #358-B) | Sibling scope of #358 (encryption-at-rest) -- travels with the parent principle per PM |
| #1306 | SEC-ENCRYPT: encrypt uploaded file content at rest (storage-layer; deferred from #358-B) | Sibling scope of #358 -- same reasoning as #1305 |
