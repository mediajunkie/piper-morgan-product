...
**Confirming data point — gbrain (June 2026)**: a CIO survey + deep-dive of Garry Tan's `gbrain` harness (`src/core/cycle/`) found its nightly "dream cycle" is **Type-1-family**: extract/grade/synthesize/enrich consolidation plus `drift.ts`/`anomaly.ts` (contradiction- and drift-detection over *stored* material). That is *"what's wrong with what I have"* (assimilation/coherence), **not** *"what could go wrong that the data didn't contain"* (Type-2 threat-rehearsal). So gbrain is one more system with sophisticated Type-1 and **no Type-2** — a fresh (and independently-built, production) confirmation of the novelty claim, on top of Anthropic Dreams + the Janus 20+-system survey. Its drift/anomaly machinery is plausibly the *substrate* a Type-2 layer would extend, but it is not itself Type 2.
