...
**The 2-vs-3 distinction matters.** A response that scores Context 2 is not failing — it's performing at the floor we'd expect of any competent LLM. Context 3 is the bar Piper has to clear *as Piper*. When the canonical retest shows responses clustering at C=2, that's the signal that context assembly is not flowing into generation, even when the response sounds knowledgeable. ⚠️ **NARROWED 2026-08-31 on measured evidence — read this diagnostic on the `required` bucket ONLY.** The corpus tagging pass (Lead, n=61) found **10 of 61 queries (16.4%) are `not_applicable`** — context is irrelevant to a correct answer, so they **cap at C=2 by construction, no matter how good the product is.** Consequence, arithmetic not speculation: **a perfect system scores 2.84 pooled, not 3.00** — a 5.5% structural drag — and ~16% of any observed C=2 mass is instrument, not product. **The diagnostic remains valid and load-bearing for the 49 `required` queries (80.3%), which is where context assembly is actually measurable.** See `context-requirement-tag-spec.md` §6: report per bucket, never pooled.
...
- **v2.3.5 (2026-08-31)** — CXO. C-axis diagnostic NARROWED on measured evidence: the "clustering at C=2 means context assembly is not flowing" reading now applies to the **required bucket only**, since 10 of 61 corpus queries are not_applicable and cap at C=2 by construction (a perfect system pools to 2.84, not 3.00). Scoring criteria unchanged; this corrects an interpretive claim, not a measurement.
