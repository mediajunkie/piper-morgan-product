...
**Current Position**: 4.4.3 — **M2 Sprint Active** (M2a-e DONE; M2f Groups A+B+C+E ALL SHIPPED May 9–12; **M2g-A+B SHIPPED May 14**; M2g-C+ active)
**Version**: v0.8.6 (pyproject.toml source of truth)
**Last Updated**: May 15, 2026
**Current Focus**: **M2g sub-epic in progress — A+B complete.** M2g-A (owner reviews) shipped May 14: **#1000** services/auth/ + **#999** services/mcp/consumer/ both closed; #1087 SEC-JWT-SECRET-PROD-GUARD filed as priority:high spinoff from #999. M2g-B (arch cleanup) shipped May 14: **#1019** adaptive_boundaries Path C remove (−543 LOC); **#1010** KG-refactor + legacy boundary_enforcer cleanup (−46 LOC, #1089 KG-PRIVACY-FILTER filed as designed-feature replacement); **#1021** UserHistoryService Layer 3 DB backend — Architect's 4-6 day estimate compressed to one session via Q1=γ ConversationDB extension discovery (Pattern-067 audit found schema 80% already present). Migration `a1021userhist` (4 columns + 3 indexes); DBUserHistoryRepository (4 ABC methods); heuristic topic-extraction + preview hooks at turn-save / archive; UserHistoryService.get_history_summary; 4-route user-history API surface (`/api/v1/users/me/history` + search/detail/privacy); context_assembler latent bug resolved → ADR-054 Layer 3 + PDR-002 adaptive greetings now producing real signal. 32 new tests (14 unit helper + 7 summary + 11 integration against real Postgres). #1088 GITHUB-ADAPTER-DEMO-FALLBACK + #1090 UI-1.0-PLAN also filed. **M2g-C+ open**: #1017 (priority:critical, post-generation content filter), #1087 (priority:high JWT), #1088, #1020, #1016, #1015, #1089, #1011. **M2f sub-epic remains complete.** **Canonical retest Run 7** Routing 93.4% / Quality 68.9% PASS / 24.6% MARGINAL / 4.9% FAIL. **PreCompact hook shipped May 8**. **ADR-061 v1.0 RATIFIED** May 4. **#1018 audit_transparency durability Phase 2 SHIPPED May 2**. **Phase F flag-flip MERGED Apr 30**.
**Sprint Structure**: M2a (baseline, DONE) → M2b (testing + cleanup, DONE) → M2c (floor consciousness + voice, DONE) → M2d (MUX Lifecycle, DONE) → M2e (Integrations, DONE) → M2f-A (security canonical, DONE May 9) → M2f-B (persistence via deletion, DONE May 9) → M2f-C (FastAPI upgrade + token refresh, DONE May 11) → M2f-E (post-floor-coverage cohort #983-#986, DONE May 12) → M2g-A (owner reviews, DONE May 14) → M2g-B (arch cleanup #1010+#1019+#1021, DONE May 14) → **M2g-C+ (ARCH-DESIGN cohort + sec follow-ups) ← ACTIVE** → M2-discovered (testing infra incl. #1070 multi-turn harness)
