...
**Current Position**: 4.4.3 — **M2 Sprint Active** (M2a-e DONE; **M2f Groups A+B+C+E ALL SHIPPED May 9–12**; M2g cued up for next push)
**Version**: v0.8.6 (pyproject.toml source of truth)
**Last Updated**: May 13, 2026
**Current Focus**: **M2f sub-epic complete.** Group A (HIBP semantics #932 + API key validation re-enable #933) shipped May 9. Group B (UserService + BudgetManager dead-code deletion, #935 + #936) shipped May 9 via −1813 LOC. **Group C shipped May 11**: **#921** FastAPI/Starlette/httpx upgrade landed via directional-evidence merge after thorough verification (#1074 closed with no regression on ASGITransport blast radius `test_conversation_lifecycle.py` 0 failures); **#857** token refresh mechanism shipped Option A (full implementation — `JWTService.refresh_access_token` + `POST /api/v1/auth/refresh` + ApiWrapper.fetch 401-intercept silent refresh + 5 new tests). **Group E cohort SHIPPED May 12** (4 ships in one day): **#984** CONTEXT-CACHE Redis TTL (cache helper + 7 cached source-level surfaces + eager invalidation on todos + trust mutations, 31 tests); **#983** CONTEXT-BLOCKED (`status: blocked` GitHub issues in floor context, 7 tests); **#985** CONTEXT-SPRINT (active milestones from GitHub, 7 tests); **#986** CONTEXT-ACTIVITY (recent issues+PRs from GitHub, 8 tests). 53 new tests across M2f-E. **3 stragglers also shipped May 12**: **#1068** pre-classifier milestone routing (early STATUS check before GITHUB_QUERY dispatch resolves #1039 vs #898 conflict), **#1069** attention_query empty-state source-transparent wording, **#1078** Set-Cookie preserve via HTTPExceptionWithCookieClear (closes the #283 friendly-error handler's Set-Cookie drop discovered during #857). Floor now has cached fail-graceful access to: trust profile (eager-invalidated), todos+reminders (eager-invalidated), calendar, projects, user_context, blocked items, active milestones, and recent activity. **Canonical retest Run 7** post-3-bug-fix landed at **Routing 93.4% / Quality 68.9% PASS / 24.6% MARGINAL / 4.9% FAIL** (combined non-FAIL 93.4%); exceeds Apr 12 baseline 65.6%. **PreCompact hook shipped May 8**. **ADR-061 v1.0 RATIFIED** May 4. **#1018 audit_transparency durability Phase 2 SHIPPED May 2**. **Phase F flag-flip MERGED Apr 30**.
**Sprint Structure**: M2a (baseline, DONE) → M2b (testing + cleanup, DONE) → M2c (floor consciousness + voice, DONE) → M2d (MUX Lifecycle, DONE) → M2e (Integrations, DONE) → M2f-A (security canonical, DONE May 9) → M2f-B (persistence via deletion, DONE May 9) → M2f-C (FastAPI upgrade + token refresh, DONE May 11) → M2f-E (post-floor-coverage cohort #983-#986, DONE May 12) → **M2g (memory governance + arch cleanup) ← NEXT** → M2-discovered (testing infra incl. #1070 multi-turn harness)
