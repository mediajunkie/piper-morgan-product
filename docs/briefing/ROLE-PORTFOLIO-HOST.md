...
| **The audit nobody owns** | Open MVP issues checked against PM's verbatim beta conditions | **RESOLVED 2026-08-15 — PM ruled after this exact question repeated across two workstream reviews.** Lead owns it as part of the sprint's final gate, plus an independent subagent cross-check of Lead's results (`decisions.log:1690`, verified directly, not accepted from the relay memo alone). Neither check is built yet — the ruling assigns ownership, doesn't complete the work. **Renamed from "the audit nobody owns" to "the audit Lead owns"** — the title itself was the tracked fact | Watch for the first actual audit run and the subagent cross-check landing — this priority row retires once both exist, not once ownership does |
