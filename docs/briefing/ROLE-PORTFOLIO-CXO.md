...
| **Honesty of user-facing claims** | The interface must not manufacture a belief | 🔴 **CORRECTED 08-06: "#1482 shipped" was a LAYER ERROR — I meant merged to `main`.** ⚠️ **object corrected same day**: the risk-bearing object is the **deployed artifact** (Fly v29, 2026-08-02, `main@b619794af`), not the `production` branch I first measured. Re-derived against the artifact — `ce31b09d6` **not an ancestor**, and **THREE false permanence claims are live** (`home.html` ×1, `insights.html` ×2). *The count was identical to my branch reading, which was luck: nothing touched those strings in that window.* **Two of the six mapped surfaces (`insight_controls.html`, `insight_card.html`) are ABSENT from the artifact entirely.** Cure is the **v30 deploy** (Lead, word-batch item 1) — no code change needed; the honest replacement returns zero occurrences there. The credential-delete **inversion is also intact** on production — still no claim on the one deletion that IS irreversible. Latent `dialog.js` defaults still proposed. | Zero false permanence claims **in the DEPLOYED ARTIFACT** — ⚠️ **currently 3, i.e. UNMET.** Two prior wordings were wrong about the object: "any reachable surface" checked against `main` (reachable by nobody), then `origin/production` (a branch, not what runs). |
