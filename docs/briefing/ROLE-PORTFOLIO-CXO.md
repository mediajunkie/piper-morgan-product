...
| **Honesty of user-facing claims** | The interface must not manufacture a belief | 🔴 **CORRECTED 08-06: "#1482 shipped" was a LAYER ERROR — I meant merged to `main`.** `origin/production` is at 2026-07-26 and does not contain it (`ce31b09d6` not an ancestor). **THREE false permanence claims are live on production right now** (`home.html` ×1, `insights.html` ×2); the honest replacement returns zero occurrences there. The credential-delete **inversion is also intact** on production — still no claim on the one deletion that IS irreversible. Latent `dialog.js` defaults still proposed. | Zero false permanence claims **on `origin/production`** — ⚠️ **currently 3, i.e. UNMET.** The old wording said "any reachable surface" and I checked it against `main`, which is not reachable by any user. |
