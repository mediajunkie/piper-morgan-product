...
| **Recurring-instrument self-firing** | PM's 8/07 ask (CIO+HOST): make lapsed instruments fire by themselves | **2 of 3 DONE, 1 correctly blocked, not built-around.** Role Health fixed 8/07 (boundary bug in HOST's suppression check). **Skill-candidates review: workflow shipped 8/14** (`skill-candidates-review-check.yml`) — monthly/1st-Tuesday, delegated to a subagent and independently verified (day-guard traced by hand across all 12 months of 2026, the corrected suppression-check pattern cross-checked against its own origin fix). **Agent 360: deliberately NOT built** — it has no ratified cadence (last fielding 6/13, no next-due date anywhere), so a workflow would require guessing a schedule nobody decided. Routed to HOST 8/14 to ratify a cadence first; the build itself is then ~30 min, template proven twice. | Role Health + skill-candidates verified by **step-level conclusions, not the green tick** (done). Agent 360 waits on HOST's cadence ruling. |
