...
| **Recurring-instrument self-firing** ⭐ NEW | PM's 8/07 ask (CIO+HOST): make lapsed instruments fire by themselves | **SCOPED, evidence in.** Denominator = 3 lapsed instruments: **Role Health has a working workflow** (fires weekly, months of green runs) — its gap was one line in HOST's procedure, now fixed, **plus the CI boundary bug I fixed 8/07**. **Agent 360 and Skill-candidates have NO workflow at all.** So the answer is **copy the now-correct role-health pattern**, not build a framework. | the 360 + skill-review get workflows; each verified by **step-level conclusions, not the green tick** |
