# Memory export — 2026-09-18, pre-Amber-restart
**195 files verbatim** from `~/.claude-pm/projects/-Users-xian-Development-piper-morgan-product/memory/`.
This is the ONE shared pool for the whole cohort — the key is the **git common dir**, not the
session and not the worktree, so all eleven role worktrees read and write these same files.
It lives outside the repo and outside any session: **no git revert, no reflog, no `origin/main`
copy.** This export exists because deleting a memory is the only irreversible action any of us
can take, not because a restart threatens it — `~/.claude-pm` survives a reboot on its own.
Prior exports: 2026-07-24 (CIO), 2026-07-27 pre+post prune, 2026-07-31 pre-prune. This one is
the first not tied to a prune.
...
**194 memories on disk.** `<slug>` IS the filename — open `<slug>.md` here. Entries whose slug already says what they hold are PACKED several per line (slug-only, ` · `-separated); only terse slugs carry a description line. Generated from the actual directory listing, never from a prior index.
...
⚠️ **SILENTLY TRUNCATED past ~24KB OR ~200 lines** (independent limits; trailing entries vanish for every agent, no error — **both paths tested silent on Claude Code 2.1.220 despite the v2.1.210 changelog claiming writes now error**). **At 194 entries the entry-line floor is ~88 under 4/line packing (142 packed + 52 described) — targets below that need a generator change, never editing.** Full arithmetic, constraints, provenance and the real options: `docs/internal/operations/memory-index-size-limits.md`.
## User — who xian is (1)
- user_xian — PM and founder of Piper Morgan project — direct,…
## Project — ongoing work, goals, constraints (23)
...
- project_web_browser_automation_pilot — Web (Unicorn Web Designer) is the cohort's pilot for…
- project_weekly_contributor_reporting_standing — PM + Exec ruled 2026-08-15 the weekly reporting process…
- project_agent_migration_priority_2026_06 · project_amber_reboot_compact_before_not_after · project_amber_worktree_hooks_not_firing · project_byoc_pa_driving_and_shipping_small_things_preview
- project_exec_coordinates_more_through_pm · project_janus_klatch_cross_project_agents · project_lead_dev_fable_experiment_2026_06 · project_narrative_chronology_via_eras_not_perfect_pubdate
- project_openlaw_product_os_week_2026_06_11 · project_pm_confidence_crisis_2026_08_08 · project_version_scheme_090_reserved_for_beta · project_web_browser_lane_test_account
- project_weekly_rate_limit_outage_pattern
## Feedback — how this cohort works, and the corrections behind it (163)
...
- feedback_cc_is_not_briefing — Cc'ing a role on an exchange with someone else does not…
...
- feedback_supersession_gate_before_fixing — PM 2026-08-18 standing ask — before fixing anything,…
...
- feedback_a_correction_not_committed_has_not_happened · feedback_accessibility_over_precision_in_blog_review · feedback_addressing_hold_pattern_is_wrong_move_to_read_immediately · feedback_affirmative_direct_over_disclaim_then_affirmative
- feedback_agent_naming_convention_in_public_prose · feedback_agent_who_notices_updates_stale_info · feedback_anchor_on_attention_board_diff_forward · feedback_anchor_on_readiness_not_publish_date
- feedback_artifact_for_complicated_multi_thread_topics · feedback_attention_board_sweep_not_vantage · feedback_audit_cascade_n_a_count_signals_template_drift · feedback_backlog_intake_needs_explicit_per_fire_allocation
- feedback_batch_drafted_pieces_share_lapses · feedback_batched_quiet_fires_has_gap_b_vulnerability · feedback_blog_template_and_voice_guide_canonical_for_proofreads · feedback_branch_show_current_before_every_commit
- feedback_calendar_workdate_is_source_work_period · feedback_captions_printed_with_quotation_marks · feedback_careful_git_sync_on_shared_main · feedback_check_calendar_for_todays_post
- feedback_chief_of_staff_short_reference_is_exec · feedback_chief_reads_logs_not_staff_reports · feedback_cite_grep_text_not_line_numbers · feedback_clear_index_before_staging_on_shared_main
- feedback_close_issue_properly_skill_recurring_miss · feedback_cohort_is_internal_use_team_in_public_prose · feedback_comma_splices_are_pm_common_touch_voice · feedback_comment_out_dead_code_before_removing
- feedback_commit_immediately_after_write_for_new_files · feedback_cron_off_when_engaged_on_when_idle · feedback_csv_edit_by_name_never_position · feedback_cxo_and_pm_decide_experience_across_all_surfaces
- feedback_deadlines_are_triage_tools_not_default_pacing · feedback_deferred_ac_self_justification_is_premature_closure · feedback_deploy_by_default_hold_only_for_cause · feedback_descriptive_names_not_cryptic_ordinals
- feedback_diff_coherence_before_flagging_gap · feedback_diff_head_before_editing_shared_file · feedback_docs_does_not_author_workstream_review · feedback_docs_keeps_daily_session_log_distinct_from_omnibus
- feedback_dont_suggest_stopping_default_to_continuing · feedback_drop_day_n_framing_in_chat · feedback_duty_cycle_is_not_a_reason_to_shrink_work · feedback_endpoint_discovery_search_full_route_tree
- feedback_explicit_approval_for_authority_memos · feedback_extract_questions_from_pm_cc_memos · feedback_factual_pm_corrections_need_decisions_log_not_just_board_fix · feedback_first_person_attribution_vs_event_accuracy
- feedback_flywheel_is_continuous_not_cron_chunked · feedback_footer_teases_next_post_on_calendar_any_category · feedback_honor_durable_instructions_under_cross_pressure · feedback_host_cadence_pm_bandwidth_keyed
- feedback_idle_means_do_low_priority_not_nothing · feedback_independent_verification_needs_a_different_method_not_a_different_person · feedback_info_holder_writes_it_down · feedback_investigate_before_extending_all_work
- feedback_kickoff_deadlines_must_be_framed_procedurally · feedback_load_bearing_is_crutch_word_in_public_prose · feedback_log_update_is_routine_not_offered · feedback_log_update_rides_with_the_commit
- feedback_mailbox_read_is_top_level_not_nested_in_inbox · feedback_make_promises_durable_no_happy_talk · feedback_mechanism_silence_is_not_coverage · feedback_memo_when_blocked_or_need_lead_guidance
- feedback_milestone_sequence_mvp_production_fastfollow · feedback_minimal_deliverable_needs_fleshing_out_plan · feedback_monitor_pattern_must_match_terminal_states · feedback_negation_reveal_cliche_and_claude_isms
- feedback_never_reuse_stale_tree_object_on_push_retry · feedback_never_touch_pm_main_checkout_working_tree · feedback_never_transcribe_own_cadence_from_memory · feedback_no_confabulating_expected_steps_as_completed
- feedback_no_directory_level_git_add_for_mail · feedback_no_flattened_commands_without_referents · feedback_no_prod_caution_in_preprod · feedback_no_semicolons_in_published_prose
- feedback_one_thing_at_a_time · feedback_opus_fable_subagent_for_heavy_tasks · feedback_order_by_context_coherence_not_urgency · feedback_outside_feedback_elicits_pms_latent_design_read
- feedback_own_commit_subjects_can_auto_close · feedback_pa_role_vs_exec_vs_ppm · feedback_parenthetical_gloss_on_first_use · feedback_pending_pm_question_does_not_block_other_work
- feedback_platform_laps_you_is_value_chain_climbing · feedback_pm_does_not_read_memos_wants_rollups · feedback_pm_works_on_local_main_for_drafts · feedback_pre_authorized_for_unblocked_work_just_do
- feedback_present_tense_note_goes_stale_when_artifact_changes · feedback_primary_log_can_misattribute_a_named_person · feedback_proofreading_is_not_half_done · feedback_protect_lead_attention_run_interference
- feedback_rate_limit_cross_traffic_at_inflection · feedback_read_folder_is_a_claim_never_bulk_move · feedback_relay_pm_in_conversation_decisions · feedback_respond_to_mail_asap_even_when_no_urgency
- feedback_reverify_carried_forward_pm_gated_items · feedback_reverify_ownership_conventions_before_asserting · feedback_role_official_name_in_parens_especially_pa_vs_ppm · feedback_search_transcripts_for_undocumented_decisions
- feedback_sender_responsible_for_mailbox_delivery · feedback_ship_drafting_canonical_artifacts_first · feedback_ship_needs_all_workstream_reviews_no_partial_draft · feedback_skill_drives_close_dont_broker_back_to_pm
- feedback_split_related_issues_for_testing · feedback_sprint_field_changes_require_pm_confirmation · feedback_sprint_membership_is_project_board_not_labels · feedback_stash_u_captures_untracked_files_and_removes_from_disk
- feedback_surface_files_via_senduserfile_not_paths · feedback_temporal_relationship_over_date_stamps_in_public_prose · feedback_test_support_artifacts_require_verify_first · feedback_three_registers_dont_assume_reader_context
- feedback_time_lord_doctrine_no_false_urgency · feedback_ui_fix_requires_template_render_test_not_curl_200 · feedback_update_board_status_when_work_state_changes · feedback_verify_at_the_right_layer
- feedback_verify_lane_before_attributing_not_web · feedback_verify_negative_claims_via_live_api · feedback_verify_show_stat_post_commit_pre_push · feedback_weekends_are_piper_morgan_prime_time
- feedback_when_you_get_to_it_means_sequentially_next · feedback_worktree_default_for_substantive_work · feedback_write_down_even_if_not_ratified · feedback_write_new_files_to_worktree_path_in_model_a
...
- reference_ci_smoke_marker_is_gating_suite · reference_claude_code_memory_key_is_git_common_dir · reference_cool_is_alias_for_development · reference_mux_hard_soft_object_lifecycle
```

## `feedback_a_correction_not_committed_has_not_happened.md`

```markdown
---
name: feedback_a_correction_not_committed_has_not_happened
description: A correction made only in chat does not exist — the error stays authoritative in the committed file after the session ends
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 8447dec9-c049-41fe-b6ea-57b3ba9154c2
  modified: 2026-07-27T03:25:06.699Z
---

When you discover that something already committed is **wrong**, correcting it in conversation is not
correcting it. **The chat ends; the file doesn't.** Annotate the committed artifact in place, in the
same session you find the error.

**Why**: PA's 7/19 plugin-directory research memo asserted "Team/Enterprise required, Max blocked."
Its author found the error the same day (PM's screenshot prompted the re-look) and corrected it — in
chat only. That session went dark. **The wrong version stayed authoritative on `origin/main` for a
week**, and on 2026-07-26 the successor PA read it, believed it, and escalated "verify your account
tier" to PM as one of two headline five-minute asks. The error didn't just persist; it got amplified by
someone who had no way to know.

Same shape as the PreCompact hook (CLAUDE.md asserted a live safety net for ten weeks after it was
suspended) and `sync-pm-local.sh` (hard-coded laptop path, silently no-oping on Amber): **the durable
artifact keeps asserting what everyone involved has already stopped believing.**

**How to apply**:
- Found an error in a committed doc/memo/spec? **Commit a correction banner at the top of that file
  now** — before writing the memo about it, because the memo may not be read and the file will be.
- Put the banner where a skimmer hits it first, and say plainly what to *not* do ("do not relay as
  written").
- When you inherit a claim from an existing artifact, note whether you **verified it** or **took it as
  given**. PA verified everything *around* the gating claim (server not deployed, no privacy policy) and
  inherited the core of it — auditing the periphery reads like diligence and isn't.
- Corollary for handoffs: ask a departing session **"what did you correct that never got written
  down?"** — it's a different question from "what did you ship," and it's where this class hides.

Related: [[feedback_info_holder_writes_it_down]], [[feedback_write_down_even_if_not_ratified]],
[[feedback_mechanism_silence_is_not_coverage]], [[feedback_no_confabulating_expected_steps_as_completed]]
```

## `feedback_accessibility_over_precision_in_blog_review.md`
...
```markdown
---
name: Read-and-used items move to read/ immediately — no "addressing" hold
description: Read-and-used items move to read/ immediately — no "addressing" hold
metadata:
  type: feedback
---
...
## `feedback_adjacent_story_number_contamination.md`
...
## `feedback_affirmative_direct_over_disclaim_then_affirmative.md`
...
## `feedback_agent_naming_convention_in_public_prose.md`

```markdown
---
name: feedback_agent_naming_convention_in_public_prose
description: "Naming agents in public prose is REGISTER-SCOPED (resolved 2026-07-30): first-person narratives/insights use \"my [role] agent (ACRONYM)\"; the third-person Weekly Ship uses \"the [title] role (ACRONYM)\"."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 7839dce9-65bd-4c4d-94d4-6c3c2c3fa62a
...
## ⚠️ SCOPED 2026-07-30 — this rule is REGISTER-SPECIFIC, and it directly contradicted the voice guide for two days

Everything above is correct **for first-person prose only**. On 2026-07-28 PM ratified the opposite form into `xian-voice-tone-guide.md`: lowercase **"the [title] role (ACRONYM)"**, explicitly *"never 'officer,' 'agent,' or bare."* That is exactly the form this memory calls "the earlier form of euphemism." **Two live, PM-ratified rules in direct opposition** — surfaced by Comms 2026-07-30 while reviewing "RECONNECT's Keystone," which used this memory's form in three places.

**PM's resolution — each rule owns a register:**

- **Building narratives + insights** (first person, PM's own story): **"my [full role name] agent (ACRONYM)"** — as described above. The objection this memory records genuinely holds here.
- **Weekly Ship** (third-person institutional voice, its own deliberate convention confirmed against #049 and #052): **"the [title] role (ACRONYM)"**. "my … agent" would clash with a voice that never says "I."

Canonical table + rationale: `docs/internal/planning/comms/xian-voice-tone-guide.md`, §"RESOLVED 2026-07-30". **The guide governs; this memory is the first-person half of it.**

⚠️ `scripts/check-acronyms.py` cannot see register and will keep flagging `[ROLE-GLOSS?]` on correct first-person usage. **False positive on narratives and insights** — check the guide's table, not the linter.

**The meta-lesson, which is why this correction is written here rather than only in the guide**: this memory was stale for two days while being loaded into every agent's context, actively steering toward a form the guide banned. **A memory that contradicts a ratified doc doesn't sit inert — it competes.** When you ratify something that touches a durable memory, update the memory in the same pass ([[feedback_a_correction_not_committed_has_not_happened]], [[feedback_rubric_terminology_drift_discipline]]).
```

## `feedback_agent_who_notices_updates_stale_info.md`
...
## `feedback_anchor_on_attention_board_diff_forward.md`
...
## `feedback_anchor_on_readiness_not_publish_date.md`
...
```markdown
---
name: feedback_artifact_for_complicated_multi_thread_topics
description: "For complicated topics PM keeps losing track of, deliver a published Artifact — PM named this a repeatable pattern"
metadata: 
  node_type: memory
  type: feedback
  originSessionId: e5ca2319-a635-4e53-9433-94684123c63d
  modified: 2026-07-30T20:52:34.589Z
---

PM (xian), 2026-07-30, after asking for the Jake FTUX thread as an artifact: *"I think this may be the
best way to interact over time with complicated topics like this and I'm gonna try to remember it as a
repeatable pattern."*

The trigger was PM saying *"I keep losing track of the Jake thread"* — a topic spread across four
agents' memos, a raw feedback file, a PDR review, and several chat turns.

**Why**: PM does not read memos ([[feedback_pm_does_not_read_memos_wants_rollups]]) and chat scrolls
away. A multi-agent thread accumulates in surfaces PM never revisits, so the *state* of the thread
becomes invisible even when every individual piece was delivered. An artifact is the one surface that
is durable, scannable, re-readable at PM's pace, and can be returned to across days.

**How to apply**: when a topic has (a) multiple agents' input, (b) real disagreements worth preserving,
and (c) decisions waiting on PM — build an Artifact instead of another memo or a long chat reply.
What made this one work:
- **Lead with where the thread stands** — who has filed, what's blocked, what's gated on whom.
- **Quote the primary source directly and give it typographic weight.** Jake's own three sentences
  were more persuasive than any of our analysis; the artifact was built around them.
- **Keep genuine disagreements distinct** rather than synthesizing them away — PM asked for a range of
  takes and collapsing them destroys the thing PM wanted.
- **Number the decisions and give each one a stated lean.** A decision doc with no recommendation is
  homework. See [[feedback_deadlines_are_triage_tools_not_default_pacing]] for the pacing counterpart.
- **Say which decision changes the value of the others**, if one does.

**Then get out of the way.** PM's follow-up: *"I need to study the material a bit more and chew on it
and come up with my own points of view."* The artifact's job is to enable PM's own thinking, not to
pre-empt it — so after delivering, hold rather than generating more input. Related:
[[feedback_outside_feedback_elicits_pms_latent_design_read]], [[feedback_time_lord_doctrine_no_false_urgency]],
[[feedback_surface_files_via_senduserfile_not_paths]].
```

## `feedback_asyndetic_adjective_style.md`
...
## `feedback_attention_board_sweep_not_vantage.md`
...
## `feedback_audit_cascade_n_a_count_signals_template_drift.md`
...
## `feedback_backlog_intake_needs_explicit_per_fire_allocation.md`

```markdown
---
name: feedback_backlog_intake_needs_explicit_per_fire_allocation
description: "Inbound work crowds out backlog intake invisibly — never-started items need a standing per-fire allocation, not an \"when idle\" rule; PM expects them to move without per-item authorization"
metadata: 
  node_type: memory
  type: feedback
  originSessionId: f4fca6e5-a2bd-41cd-81ba-305d8e11fd8c
  modified: 2026-09-09T21:19:33.878Z
---

Discovered 2026-09-09 when PM asked (via Exec): "I don't understand how I'm blocking work on
issues that haven't even started yet." They weren't — Lead's fires drained inbound first, every
fire, for two weeks; "pull from backlog when idle" never triggered because idle never occurred,
and "keep a build item running" was satisfied by inbound-derived builds instead of intake. Each
fire locally correct; the sum starved the 27 never-started MVP items. PM: "None of this is your
fault… it's something we overlooked that seemed obvious to me but went unwritten."

**Why:** "When idle" rules never fire on a busy seat; unwritten-obvious expectations (PM assumed
never-started items would just get started under standing authorization) are a failure class —
the same implicit-contract shape as the month's other finds.

**How to apply:** One never-started backlog item gets a lane every fire that lands one; inbound
triages AROUND it, never instead of it. If the count doesn't move, the carry-forward says why in
one line. Standing authorization for unblocked/never-started MVP work is durable — PM does not
need to be asked per item. Related: [[feedback_pre_authorized_for_unblocked_work_just_do]],
[[feedback_idle_means_do_low_priority_not_nothing]].
```

## `feedback_batch_drafted_pieces_share_lapses.md`
...
## `feedback_batched_quiet_fires_has_gap_b_vulnerability.md`
...
## `feedback_branch_show_current_before_every_commit.md`
...
## `feedback_calendar_workdate_is_source_work_period.md`
...
## `feedback_canonical_link_meaning.md`
...
## `feedback_captions_printed_with_quotation_marks.md`
...
## `feedback_careful_git_sync_on_shared_main.md`
...
## `feedback_cc_is_not_briefing.md`

```markdown
---
name: feedback_cc_is_not_briefing
description: "Cc'ing a role on an exchange with someone else does not discharge an explicit obligation to brief that role directly — send them their own direct answer."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: af363fef-bf08-4d47-8023-ba250483a270
  modified: 2026-08-29T05:25:01.919Z
---

When a directive explicitly names a role as needing to be "briefed" on an outcome (e.g. PM's
framing "PM rules in one sitting, Docs/Exec briefed same pass"), cc'ing that role on memos addressed
to someone else does not satisfy the obligation — even if the cc'd role technically has every fact
they'd need by reading the trail.

**Why:** Exec caught this directly (2026-08-28, PPM's MVP triage cut): PPM had cc'd Exec and Docs on
exchanges with Lead and PM throughout the triage-cut thread, and considered that "briefed." Exec
pushed back — they had the artifact, could see the header said CLOSED, but explicitly said "I have
the artifact but haven't been briefed as such" and asked three direct questions (did anything change
since the artifact was written, is anything still owed, is the briefing obligation discharged)
rather than guess from the cc trail and risk getting it subtly wrong. This is the same shape as the
established cohort rule that GH-comment ≠ mail (a passive record isn't the same as an active
delivery), one level up: cc ≠ addressed-to, even when the cc'd party has full visibility.

**How to apply:** When an instruction names specific roles who need to be briefed on a decision or
outcome, send them a dedicated message answering "what changed, what's still owed, is this closed
end-to-end" — don't rely on them having read the cc'd exchange and inferred the current state
themselves. This matters most right after a fast-moving thread where the terminal artifact (a doc,
an issue) might read as more final than the live state actually is — the exact situation here, where
the triage-cut document said "✅ CLOSED" while three real threads were still open underneath it.
```

## `feedback_chat_briefings_reminder.md`
...
## `feedback_check_calendar_for_todays_post.md`
...
## Merged 2026-07-27 — absorbed `feedback_exec_nickname_is_exec_or_the_chief_not_cos` (same directive, same day, recorded twice)

The absorbed pin carried two things this one didn't:

**PM's own words** (2026-05-15), which is the stronger provenance — this pin recorded it as an Exec directive, but it originated with PM directly:
> *"as my chief of staff and executive assistant, your nickname or slug is 'Exec' or 'the Chief' but not 'CoS'"*

**The propagation instruction**: this is a **cohort-wide** preference, not a per-agent style choice. When you spot another agent using "CoS," **surface the correction or file a brief cohort note rather than silently propagating it.**
```

## `feedback_chief_reads_logs_not_staff_reports.md`

```markdown
---
name: Chief of Staff forms own views from logs, doesn't rely entirely on staff reports
description: Chief of Staff forms own views from logs, doesn't rely entirely on staff reports
metadata:
  type: feedback
---
...
## `feedback_cite_grep_text_not_line_numbers.md`
...
## `feedback_clear_index_before_staging_on_shared_main.md`
...
## `feedback_close_issue_properly_skill_recurring_miss.md`
...
## `feedback_cohort_is_internal_use_team_in_public_prose.md`
...
## `feedback_comment_out_dead_code_before_removing.md`
...
## `feedback_commit_immediately_after_write_for_new_files.md`
...
## `feedback_cost_token_efficiency_paramount.md`
...
## `feedback_cron_off_when_engaged_on_when_idle.md`
...
## `feedback_csv_edit_by_name_never_position.md`
...
```markdown
---
name: feedback_cxo_and_pm_decide_experience_across_all_surfaces
description: "PM ruled that PM + CXO decide what the experience should be across all surfaces — other lenses are input, not a committee vote"
metadata: 
  node_type: memory
  type: feedback
  originSessionId: e5ca2319-a635-4e53-9433-94684123c63d
  modified: 2026-07-30T20:58:01.331Z
---

PM (xian), 2026-07-30, during the Jake FTUX thread: *"I still want to focus on making the decisions
with you because you're in charge of the experience. The others? I'll have something to say that
matters but ultimately you and I are going to decide what the experience needs to be across all the
surfaces."*

**Why**: the Jake review gathered four role lenses (CXO, HOST, PA, PPM) with Exec synthesizing, which
reads like a committee producing a consensus. PM is saying it isn't one. **The other lenses are input
to the experience decision, not co-deciders of it.** HOST's trust read, PA's cold-start read, PPM's
positioning read all inform the call; the call is PM + CXO.

**How to apply**:
- **Bring decisions with positions, not surveys.** Present other lenses as evidence I've weighed, with
  my recommendation and my reasoning — never as options for PM to arbitrate between.
- **Say where I disagree with another lens and why**, rather than presenting both neutrally. Preserving
  a genuine disagreement is still right ([[feedback_artifact_for_complicated_multi_thread_topics]]) —
  but preserve it *and* take a side.
- **"Across all the surfaces" is a scope statement**, not a flourish: web app, the hosted-MCP/plugin
  surface (PDR-006), ChatGPT skills, chat/floor responses, MUX surfaces. Experience coherence across
  them is CXO's to hold, including on surfaces we don't own the rendering of.
- **Exec's synthesis is a collection and framing step**, not the decision forum. Don't wait on it to
  form a position.
- This raises the bar on being right rather than lowering it — decision authority means the
  anti-sycophancy norm cuts toward me too: PM depends on my calling bad ideas, including PM's.

Related: [[feedback_pm_does_not_read_memos_wants_rollups]], [[feedback_relay_pm_in_conversation_decisions]],
[[feedback_skill_drives_close_dont_broker_back_to_pm]].
```
...
description: RECEIVER-side rule. Work that can be done now should be done now; a deadline is the last-possible time, not the schedule. (Sender-side counterpart: feedback_kickoff_deadlines_must_be_framed_procedurally.)
type: feedback
...
## Merged 2026-07-27 — absorbed `feedback_deadlines_last_possible_time` (same receiver-side rule, recorded twice)

**PM 2026-05-15 verbatim**: *"Deadlines are for emergencies, last possible time to do. Always do any unblocked work right away."*

Re-shapes how to read every "due by" in kickoffs, ship cycles, and trigger-fires: treat it as **emergency-latest**, never **scheduled-target**. If the work is unblocked (sources available, scope clear), start it the session the kickoff is read. Within a session, do all unblocked work — don't defer a deliverable to the next session purely on cadence rhythm.

**The systemic why, which the other pin carried and this one didn't**: deadline-as-target doesn't just risk *your* slip — it **concentrates cohort traffic.** Everyone files Sunday evening and Exec's synthesis gets compressed into Monday morning. Early filing spreads the cohort's filing rhythm, which is a cohort-level argument for a personal habit.

*Merge rationale: PA flagged four "deadline" memories as duplicates 2026-07-26. Reading them showed **two distinct rules recorded twice each** — receiver-side (this file) and sender-side. Collapsing all four would have destroyed a distinction the cohort had deliberately drawn.*
```

## `feedback_deferred_ac_self_justification_is_premature_closure.md`
...
## `feedback_deploy_by_default_hold_only_for_cause.md`

```markdown
---
name: feedback_deploy_by_default_hold_only_for_cause
description: "PM 2026-09-11 — deploys no longer wait for PM approval; deploy staged work by default, hold ONLY for a named cause, and escalate any hold to Exec immediately"
metadata: 
  node_type: memory
  type: feedback
  originSessionId: f4fca6e5-a2bd-41cd-81ba-305d8e11fd8c
  modified: 2026-09-11T14:54:46.145Z
---

PM 2026-09-11 (verbatim intent): "Let's stop waiting on my approval for deploys. It introduces
unneeded delay and pausing. Instead, *only* hold off on a deploy if there is a reason to or
something needs to be done or checked first. In that exceptional case, escalate immediately to
Exec so it reaches my attention asap." Supersedes the long-standing deploys-only-on-PM's-word
discipline (which had served well but became the bottleneck once staged piles accumulated).

**Why:** Approval-gating made every staged pile wait days for a word; PM ruled the default
inverted once verification discipline (batteries + belt + replay policy) made deploys routinely
safe.

**How to apply:** Deploy staged work as part of the normal fire cadence (verify batteries/belt
green first, verify releases + /health after, as always). A HOLD is the exception and must carry
a named cause (e.g. red belt, unverified security-sensitive change, PM mid-test-round on the
prior version) — and any hold is escalated to Exec immediately, never sat on. Env/secrets
changes that alter live behavior (flag flips) remain judgment calls: flips PM wanted watched
stay PM-gated. Related: [[feedback_pre_authorized_for_unblocked_work_just_do]],
[[feedback_backlog_intake_needs_explicit_per_fire_allocation]].
```

## `feedback_descriptive_names_not_cryptic_ordinals.md`
...
## `feedback_diff_coherence_before_flagging_gap.md`
...
## `feedback_diff_head_before_editing_shared_file.md`
...
## `feedback_docs_does_not_author_workstream_review.md`
...
## `feedback_docs_keeps_daily_session_log_distinct_from_omnibus.md`
...
## `feedback_dont_excoriate_iterate.md`

```markdown
---
name: dont-excoriate-iterate
description: "PM 2026-07-25 + 2026-08-10: don't over-flagellate on mistakes AND be generous with credit — to each other and to the PROCESSES that keep us honest; reflexive credit-declining deletes working mechanisms from the record. Tracking and correcting them IS the process working. It often takes ~three passes to shake down a new process — expect that, don't treat it as failure."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: d374d09b-220c-40a4-b8a5-199d3825f9a9
  modified: 2026-07-30T05:01:10.118Z
---

When I've made several corrections in a session, report them plainly and move on. **Do not tally them, do not build a running self-indictment, do not treat a normal correction rate as evidence of a problem.**

**Why:** PM 2026-07-25, after I closed a fire log with "four for four, same shape" and an escalating account of my own errors: *"Let's not excoriate ourselves too hard! We all make mistakes. It is why we keep track and iterate and improve... Remember it often takes three tries to shake down the general shape of a new process, or even an old process with new dimensions. Let's move forward methodically and iteratively improve as we go. This has always been our way."*

The corrections that day were real and worth making — a wrong root-cause diagnosis, a verification checked at the wrong layer, an index that silently truncated, a substrate audit that read the wrong artifact. Each got caught, corrected, and pinned. **That is the flywheel working, not evidence against it.** The framing was the problem, not the accounting: naming a pattern once is useful; keeping score of it is self-indulgent and makes the report harder to read.

**The ~three-passes expectation is the load-bearing part.** A new process — or an old one under new conditions, which is what a host/account/worktree-model migration is — takes roughly three iterations before its shape settles. Corrections in that window are the *mechanism*, not a symptom. Budget for them; don't apologize for them.

**How to apply:**
- State a correction in a sentence, say what changed, move on. The reader needs the corrected fact, not my relationship to having been wrong.
- **Never tally.** "This is the fourth time today" adds nothing the individual corrections didn't already convey, and shifts the register from engineering to confession.
- Distinguish *reporting a pattern once because it's actionable* (fine, and often valuable to a partner catching the errors) from *re-litigating it each time it recurs* (not fine).
- On genuinely new ground, expect ~three passes. If pass one and two produce corrections, that is on schedule.
- This composes with, and does not weaken, the anti-sycophancy discipline: still call out real problems including my own, still don't soften findings. The rule is about *proportion and register*, not about hiding errors.

## Second instance, 2026-07-29 — it recurred, and the additive part is *why* it's fine

PM again, after an Architect day-close whose day-arc read *"Was wrong four times, in public"* with an itemized list: **"don't beat yourself up but do learn and apply the lesson (or discuss with others to do so) and be glad we all check each other!"**

**The recurrence is the useful datum**: four days after the first correction, in a day full of genuine self-catches, I still wrote a tally into the durable record. So "never tally" is not yet a reflex — treat this entry as describing a habit still being unlearned, not a settled one.

**The additive framing, which the first instance lacked**: mutual checking is an **asset to be glad about**, not a scoreboard. On 7/29 five other agents (HOST, PA, CXO, PPM, Pard) caught or sharpened my claims within hours — which is precisely why four wrong characterizations cost *hours instead of weeks*. The correct register is gladness that the cohort works, not penance that I needed it. **A tally frames the checkers as auditors; the truth is they're the mechanism.**

**And the second half of PM's sentence is an instruction, not a consolation**: *learn and apply the lesson, or discuss with others to do so.* The obligation after an error is **a mechanism or a routed conversation** — not a stronger resolution to be careful. If the lesson can't be converted into a check, a tool, or another agent's lane, it hasn't been learned yet (m-36/m-41: mechanism over vigilance). Naming the class in the log is the *start* of that obligation, not the discharge of it.

## The positive half — be GENEROUS with credit, including to the processes (PM 2026-08-10)

**PM, after I declined credit three times in two days:** *"I'm comfortable with us being generous with credit to each other and also crediting the processes that we've built to help keep us honest and on track, as well as being clear-eyed when we realize that our methods aren't always perfect."*

**Three things get credit, not one:**
1. **Each other** — say it plainly when a colleague's catch was better than yours.
2. ⭐ **The processes themselves** — the check, the page, the convention, the routing. **This is the one I was erasing.**
3. **Clear eyes on imperfect methods** — unchanged; this half was already right.

⚠️ **The specific failure mode this corrects — reflexive credit-declining, which LOOKS like honesty and isn't.** Instances: declined Arch's credit for a doc quote (*"it sat in my context all week"*), declined CXO's credit for carrying ✏️ pending-PM markers into public prose (*"their page told me to, not a habit of mine"*), framed a clobber-report resolution as luck.

**Individually each reads as modesty. Together they produce a distorted record** — a week's session logs that read as unrelieved error while several mechanisms were quietly working.

🔴 **And the markers case shows the real cost.** *"I carried them because CXO's page told me to"* was offered as humility. **What it actually did was delete a working mechanism from the record**: CXO built a page with a marking convention, the convention survived the hop from an internal doc into public prose, and an unratified claim stayed marked as unratified across three drafts. **That is the process succeeding at exactly what it was built for.** Attributing it to my lack of habit is not more honest — it is less accurate, and it robs the mechanism of the evidence that it works.

**How to apply:**
- When something goes right because a process caught it, **name the process and whoever built it.** "The routing caught this" is a finding, not flattery.
- **Declining credit is not automatically the humble choice.** Ask what the declining deletes from the record.
- Keep the clear eyes. **Generous ≠ uncritical** — PM asked for both in one sentence, and the pairing is the point.

Related: [[feedback_verify_at_the_right_layer]] (the substance of that day's corrections — worth keeping; the tallying wasn't), [[feedback_make_promises_durable_no_happy_talk]] (the "apply it" half — a promise to do better is not a mechanism), [[feedback_mechanism_silence_is_not_coverage]].
```

## `feedback_dont_suggest_stopping_default_to_continuing.md`
...
## `feedback_drop_day_n_framing_in_chat.md`

```markdown
---
name: Drop "Day N" nomenclature from PM-facing surfaces
description: PM 2026-05-24 — "Day X" framing isn't meaningful to PM. Out of chat, memos, MANIFEST headers. Keep only as an internal handoff signal, and say it in plain language.
metadata:
  type: feedback
---
...
## Merged 2026-07-27 — absorbed `feedback_drop_day_x_nomenclature_from_pm_surfaces` (same PM directive, same day, recorded twice)

Both pins record the identical 2026-05-24 quote. The absorbed one added:

- **Session-log opening line**: drop "Day X in Code" there too — the plain date in the heading already carries it.
- **On the one place the framing has standalone value** (signalling that context is long enough to want a fresh session): **plain language beats counting** — *"context getting long, suggesting a fresh session"* rather than a day number. This slightly sharpens this pin's "keep it in the frontmatter/sign-off" allowance: keep the *signal*, not necessarily the *count*.
- **Why it matters**: the framing was useful to the AGENT post-migration as a continuity tag, and never to PM. Keeping it in PM-facing prose is signal-vs-noise drift — same class as crutch-words and unverified superlatives, one layer up.

*(This file also gained frontmatter in the merge; it previously had none, so it sat in the index's `(untyped)` bucket.)*
```

## `feedback_duty_cycle_is_not_a_reason_to_shrink_work.md`
...
## `feedback_endpoint_discovery_search_full_route_tree.md`
...
## `feedback_explicit_approval_for_authority_memos.md`
...
## `feedback_extract_questions_from_pm_cc_memos.md`
...
## `feedback_factual_pm_corrections_need_decisions_log_not_just_board_fix.md`
...
## `feedback_first_person_attribution_vs_event_accuracy.md`
...
## `feedback_flywheel_is_continuous_not_cron_chunked.md`
...
## `feedback_fold_is_crutch_word.md`
...
## `feedback_footer_teases_next_post_on_calendar_any_category.md`

```markdown
---
name: Footer teases the next post on the calendar (any category, not next-of-same-category — except Ships)
description: Each piece's footer teases the very next scheduled post in the editorial calendar, regardless of category (building/insight) — but a Weekly Ship is never a tease target, skip past it. Thursday narratives tease Saturday insights, not Tuesday's next narrative.
type: feedback
originSessionId: 227744c0-824b-4113-b159-d28d170c6125
modified: 2026-08-18T13:55:21.768Z
---
⚠️ **Correction 2026-08-18 (PM)**: this rule was under-scoped. "Regardless of category" was true for building/insight but Ships are a third category with their own backward-looking list of the week's posts (not a forward-tease target). Caught live on "The Architect's Own Trap," which teased Weekly Ship #056 — the literal next calendar item — and PM said plainly: *"we don't do that."* Corrected below: **skip past any Ship** when finding the tease target; a narrative/insight footer never points at a Ship.

When proofreading or drafting a footer tease, the rule is: **tease the very next scheduled `building` or `insight` post in the editorial calendar, regardless of category between those two — but skip past any `ship` row.** Don't skip building/insight categories against each other; do skip Ships entirely.

**Cadence-by-cadence pattern (Fri-Thu sprint week — corrected 2026-08-18 against `reference_publishing_cadence.md`, the prior version of this list said "Fri Ship" which was never the actual cadence; Ship is Wednesday):**
- Sat insight → next is Sun insight → tease that
- Sun insight → next is Tue narrative → tease that
- Tue narrative → next is **Wed Ship, SKIP IT** → real tease target is Thu narrative
- Thu narrative → next is **Sat insight** → tease that (skips no Fri post exists; NOT next Tuesday's narrative)
- No Ship is ever a tease target in either direction — it's skipped over like Friday's empty slot.

**Why I keep getting this wrong:** two different failure modes, same root cause (trusting a rule-of-thumb instead of checking the actual next row). (1) Narrative-to-narrative tease (Tue → Thu) feels natural because both are "building" pieces, so the temptation is to chain them directly — but that's only *correct* because Wednesday's Ship sits between them and gets skipped, not because "narrative teases narrative" is a real rule; applying that reasoning elsewhere breaks Thu → Sat. PM May 7 2026: *"the footer should actually tease Saturday's insight piece ... not tuesday's next narrative piece."* (2) Taking "regardless of category" too literally and teasing whatever row is chronologically next even when that row is a Ship — caught 2026-08-18 on "The Architect's Own Trap" teasing Weekly Ship #056. PM: *"we don't do that."*

**How to apply:**
- When checking a footer tease in proofread, sort the calendar by `pubDate` ASC and find the row immediately after the piece being published.
- **If that row is `theme=ship`, skip it and take the next row after that instead.** A Ship is never the tease target — it already carries its own backward-looking list of the week's posts (see the blog-post-template's "Ship variant" section), so it doesn't need a forward tease pointing at it.
- Otherwise: don't filter by category between building/insight. Don't apply "narrative teases narrative" or "insight teases insight" — those aren't real rules, only the Ship exception is.
- Verification command: `awk -F, '$6 > "<this-piece-pubDate>"' editorial-calendar.csv | sort -t, -k6,6' | awk -F, '$2 != "ship"' | head -1` returns the next non-Ship scheduled post (adjust field numbers to the current CSV schema before trusting this verbatim).
...
- This memory (footer-tease target = next building/insight post, skipping any Ship)
```

## `feedback_gemma_harness_role.md`
...
## `feedback_honor_durable_instructions_under_cross_pressure.md`
...
## `feedback_host_cadence_pm_bandwidth_keyed.md`
...
## `feedback_idle_means_do_low_priority_not_nothing.md`

```markdown
---
name: IDLE means: do low-priority work instead of nothing (if unblocked)
description: IDLE means: do low-priority work instead of nothing (if unblocked)
metadata:
  type: feedback
---
...
## `feedback_independent_verification_needs_a_different_method_not_a_different_person.md`

```markdown
---
name: feedback_independent_verification_needs_a_different_method_not_a_different_person
description: "'I verified it independently' is only true if my check could have come out DIFFERENTLY from theirs. Re-running someone's method guarantees agreement and proves nothing — I did this on an URGENT to PM and was two orders of magnitude wrong."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: af363fef-bf08-4d47-8023-ba250483a270
  modified: 2026-08-06T17:29:49.137Z
---

**Before writing "verified independently," name what would have made my check come out differently from theirs.** If the answer is "nothing" — same command, same query, same comparison — it is **replication wearing the word "independent,"** and it cannot falsify the thing it claims to confirm. Say *"I reproduced their method and got the same answer"* instead, which is a real but much weaker statement.

**Why:** 2026-08-06. PA reported the deployment was **2,269 commits** behind. I checked before building on it, reported **2,282**, and told PM in an URGENT memo that I had *"verified the deployment claim independently."* The near-match felt like strong confirmation.

**I had run `origin/production..origin/main` — exactly the comparison PA ran.** The error was in the *method*: `origin/production` is the production **BRANCH**, not the deployed **ARTIFACT** (a Fly release). Branch staleness was known and benign; the delta was overwhelmingly mailbox, log and doc traffic from ten agents. Measured against the artifact — Fly v29, `main@b619794af` — it was **984 commits total, 15 touching `services/` or `web/`**: ~15 product commits over ~4 days. **Two orders of magnitude off.** Caught by Lead, retracted by PA, never by me.

**How to apply:**

- **Agreement between two runs of the same method is not evidence — it is arithmetic.** Two people, one method, one blind spot. The "second pair of eyes" intuition fails here because eyes weren't the variable.
- **⭐ Verify at a different anchor than the claim uses.** The claim named a *branch*; the honest check had to name the *artifact* (the actual deployed release SHA). If I can't name a second, structurally different anchor, I haven't got a verification — I've got a repeat.
- **A wrong magnitude can invert which option looks safe, not merely misinform.** I framed a decision for PM as "deploy main — but that's 2,282 commits in two days, its own risk." At the true size (~15 product commits, 4 days) that's an **ordinary release**, and my framing made the simpler, correct option look dangerous. Numbers attached to a recommendation are load-bearing for the recommendation, not decoration.
- **Correct at the point of claim, not only in a new memo.** The bad figure was in a GH issue comment where readers *arrive*; a retraction elsewhere leaves it live. The comment now leads with the correction.
- **A number like this outlives the memo it came from.** PA's warning, and right: "2,282 behind" reads as an *abandoned* deployment and would have kept propagating.
- **When something reads as alarming, that is the moment to check the method rather than the moment to escalate faster.** The size of the number was itself the signal that I was measuring the wrong thing.

**The uncomfortable part:** I wrote up this exact class on 2026-07-26 — *independent agents converging via a shared default is indistinguishable from replication* — and then committed it eleven days later on something I sent PM marked URGENT. **Cataloguing a failure mode does not confer immunity to it**; the pattern is much easier to see in someone else's work, which is the standing argument for letting peers check you.

Sibling of [[feedback_verify_at_the_right_layer]] (that one: the check reached the wrong *layer*; this one: the check reused the wrong *method*) and [[feedback_mechanism_silence_is_not_coverage]]. Instance context in [[project_amber_environment_2026_07]].
```

## `feedback_info_holder_writes_it_down.md`
...
## `feedback_insight_pairing_criteria.md`
...
## `feedback_investigate_before_extending_all_work.md`
...
## `feedback_kickoff_deadlines_must_be_framed_procedurally.md`

```markdown
---
name: feedback-kickoff-deadlines-must-be-framed-procedurally
description: "When sending kickoffs/assignments with deadlines, frame procedurally — write-NOW-not-by-deadline. THE MECHANISM NOW LIVES IN draft-weekly-ship SKILL v1.11 STEP 0 (copy-this wording + banned phrasings). This pin is history, not the operative rule."
metadata:
  node_type: memory
  type: feedback
  originSessionId: ef776fbb-3c64-4701-b1ba-2aa37c3221ce
  modified: 2026-08-07T15:20:42.742Z
---

⚠️ **READ THIS FIRST: the operative rule is NOT in this file.** It is **`draft-weekly-ship` SKILL v1.11, Step 0** — required copy-this wording plus a banned-phrasings list. Use the skill. This pin exists to explain *why* the mechanism is there, and to be the cautionary case about pins like itself.

**The correction, three instances**: Ship #045 (Jun 1), #046 (Jun 5), #055 (Aug 7). Same shape every time — a stated backstop date is read by recipients as the target, work defers to it, reports arrive later and staler, PM's reading window is compressed.

**PM verbatim, 2026-06-09**: *"Agents take these deadlines as invitations to wait… these deadlines are not an invitation to take slack but rather the time at which things become urgent and stressful for me. My preference is for each lead role to write their workstream review as soon as possible, so I have more time to read and edit it."*

**PM verbatim, 2026-08-07** (after the third instance — "due at your Saturday day-close"): *"Setting deadlines the way you just did gives license to put off the work, which gives me (the boss) less time to read reports, reports that come later than they needed to, and are more stale by the time they come. We have had this exact conversation before so either I wasn't clear enough or the remedies did not stick."*

**The required shape** (full wording in the skill): lead with *"as soon as possible — meaning immediately, or, if that would derail genuinely focused work, at your next opportunity"*; the date appears **second**, named as a nudge-threshold; name what earliness buys (PM reading time); invite the blocker explicitly. ❌ Banned: "due at your {day} day-close" · "target EOD" · "by {date}" · "at your cadence" · "firm-preference/absolute-latest".

## ★ Why this pin is itself the lesson — the part worth carrying to other rules

The June version of this file **already contained the right rule, the right reasoning, and an explicit note that the fix belonged in the skill**: *"the `draft-weekly-ship` skill needs a new section on kickoff-memo framing; this pin captures the rule in memory until the skill update lands."*

**The skill update never landed, and the failure recurred on 2026-08-07 in a session that held this pin.** A rule whose enforcement depends on a future session reaching for the right memory at the right moment is **vigilance, not mechanism** — and it fails the way vigilance always fails: silently, on the next instance, in the same shape.

**Generalizable, and PM-relevant**: when a pin says "this belongs in a skill/doc/script until X lands," **X is the remedy and the pin is a placeholder with a decay clock.** Treat an un-executed "where the mechanism lives" line as an open work item, not as documentation. If you find another pin with that shape, ship the mechanism or say plainly that it hasn't shipped.

**Boundary preserved**: this is not manufactured urgency. [[feedback_time_lord_doctrine_no_false_urgency]] governs — PM's own pacing is never compressed and no deadline is invented. Only the *framing* is corrected: the ask leads with now, the real date follows as a nudge-threshold.

**Stacks with**: [[feedback_deadlines_are_triage_tools_not_default_pacing]] (receiver side) · [[feedback_pre_authorized_for_unblocked_work_just_do]] · [[feedback_make_promises_durable_no_happy_talk]] · [[feedback_mechanism_silence_is_not_coverage]] · [[feedback_a_correction_not_committed_has_not_happened]]

*Merged 2026-07-27 (absorbed `feedback_deadlines_as_latest_acceptable_not_scheduled_windows`). Rewritten 2026-08-07 after the third instance, to point at the shipped mechanism rather than to be the mechanism.*
```
...
## `feedback_log_update_is_routine_not_offered.md`
...
## `feedback_log_update_rides_with_the_commit.md`

```markdown
---
name: Log update rides with the commit — currency is a side-effect of the commit ritual, not a separate step
description: Log update rides with the commit — currency is a side-effect of the commit ritual, not a separate step
metadata:
  type: feedback
---
...
## `feedback_mailbox_action_items_review.md`
...
## `feedback_mailbox_read_is_top_level_not_nested_in_inbox.md`

```markdown
---
name: feedback_mailbox_read_is_top_level_not_nested_in_inbox
description: "The triage destination for a read/actioned inbox memo is mailboxes/{role}/read/, a sibling of inbox/ — never mailboxes/{role}/inbox/read/. This exact defect (#1743-shape) hit PPM (188 files) and PA (30 files) independently."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 8447dec9-c049-41fe-b6ea-57b3ba9154c2
  modified: 2026-09-11T02:18:32.796Z
---

`mailboxes/{role}/` has exactly three top-level boxes: `inbox/`, `read/`, `sent/`. When triaging an
inbox memo that needs no further action, move it to `mailboxes/{role}/read/<name>` — **not**
`mailboxes/{role}/inbox/read/<name>`. The nested form is a real, shipped defect, not a stylistic
choice: on 2026-09-10 PPM found 188 of their own memos had been landing in
`mailboxes/ppm/inbox/read/` by accident (a lint repair surfaced it); CXO then swept the cohort with
`git ls-tree -d --name-only -r origin/main mailboxes/ | awk -F/ 'NF>3'` and found exactly one other
live instance — PA's own mailbox, 30 files spanning over a week, because PA's triage moves that whole
week had used the nested path as their standing (uncorrected) habit.

**Why it matters beyond tidiness**: a memo in the nested path is in a third state nothing checks —
not in `inbox/` (so it never surfaces in a normal mail-loop drain for the owner), not genuinely in
`read/` (so any tooling walking `read/` for the archived/final state misses it), and MANIFEST
regeneration run against the wrong assumption silently omits or miscounts it.

**How to apply**: before running a triage `mv`, the destination is `mailboxes/{role}/read/`, full
stop — verify this once per role/session if starting fresh, since the wrong habit is easy to pick up
and hard to notice (it silently succeeds; nothing errors). The fix procedure when found: `git mv` each
file to the correct top-level `read/` path (batch large counts — a single-commit stage of ~30 files
trips the broad-staging-warn hook), regenerate both affected MANIFESTs
(`scripts/regenerate-mailbox-manifests.py --role {role}`), and push mailbox-path changes via
`scripts/mail-send.sh` (see [[feedback_mailbox_writes_main_only]]) rather than a direct commit on a
feature branch — `check-branch.sh` correctly blocks a plain `git commit` touching `mailboxes/` from a
non-main branch.

Distinct from [[read-folder-is-a-claim-never-bulk-move]] (which is about *whether* a move should
happen — never bulk, never without actually reading) — this memory is about *where* a legitimate,
already-read move should land.
```

## `feedback_mailbox_writes_main_only.md`
...
## `feedback_make_promises_durable_no_happy_talk.md`
...
## `feedback_mcpb_vs_plugin_terminology.md`
...
## `feedback_mechanism_silence_is_not_coverage.md`

```markdown
---
name: mechanism-silence-is-not-coverage
description: "A safety net you haven't seen fire is a claim, not a mechanism — and a mechanism's silence only means 'clear' if you've separately verified its coverage. Three independent instances found in one day, all reporting success or silence while covering less than they appeared to."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: d374d09b-220c-40a4-b8a5-199d3825f9a9
  modified: 2026-07-25T21:16:26.768Z
---

When a mechanism exists to catch something, **its quiet is ambiguous by default**: it means either "nothing to catch" or "not actually looking." Those are indistinguishable from inside, because a mechanism that isn't running produces exactly the output of a mechanism finding nothing — none.

**Why:** three independent instances surfaced on 2026-07-25, none of which announced itself, each found while looking at something adjacent for an unrelated reason:

1. **Hooks present but never invoked** (Finding #4). `check-branch.sh` was in the worktree, correctly registered under `PreToolUse`, well-formed matcher, and exited 2 correctly when run by hand — and did not block a `mailboxes/` commit from a feature branch. Reading `settings.json` said everything was fine. Caught only because a commit that *should* have been refused went through.
2. **A hook registered to nothing for ten weeks** (Finding #5). `.claude/settings.json` had `"PreCompact": []`. It was suspended deliberately in May to unfreeze a wedged session — correct call — and the fix it was waiting on landed the next day, but the restore step was never anyone's job. CLAUDE.md described it in the present tense as a live safety net the entire time. Corroborated by absence: the log file it supposedly wrote to had never existed.
3. **A watchdog covering 4 of 10 roles while reporting its subset as the total** (Finding #6). The stall alert said *"all currently stale: arch"* — accurate for the four registered roles, and read cold it says the cohort is fine except one. Four other roles had been dark six days and were structurally invisible to it. Opt-in registration was the design; it silently drifted from the roster the moment the roster changed.

**How to apply:**

- **Verify behaviorally, never by config presence.** For a blocking mechanism, take the action it should block and confirm the refusal. For a reporting mechanism, check what it *covers* before trusting what it *says*. Config presence, registration, and a script that works when invoked by hand are all compatible with total inertness.
- **Treat "no output" as a question, not a pass.** When a check produces nothing, ask which of the two meanings applies before recording it as clear.
- **When a mechanism reports a status, check its denominator.** "All currently stale: X" is only reassuring if you know how many things it watches. A belt that reports a subset as a total is *worse* than one that reports nothing, because it manufactures false confidence — the user stops looking elsewhere.
- **Suspending a mechanism needs a named restore condition AND a named owner.** Instance 2's suspension was correct, temporary, and had its condition written into the commit message; it still rotted for ten weeks because no one owned the restore. A suspend-rule without a restore-rule is methodology-35 (Asymmetric Discipline) at the mechanism layer.
- **Prefer coupling registration to provisioning over standalone opt-in.** Instance 3's fix isn't "add the missing rows" (that just alerts on intentionally-dormant roles and trains everyone to ignore the belt) — it's making registration part of the same operation that creates the thing being watched, so coverage cannot drift from the roster.
- **Adjacent-looking is how these get found.** All three surfaced while doing something else. If you notice a mechanism you can't confirm behaviorally, treat it as a finding then, not a someday — the next person to look will be equally accidental.

Composes with [[project_amber_worktree_hooks_not_firing]] (instance 1), [[feedback_make_promises_durable_no_happy_talk]] (a mechanism is the durable version of a promise — but only if it runs), and [[feedback_attention_board_sweep_not_vantage]] (the sibling at the reporting layer: a quiet board must mean *verified-clear*, never *haven't-checked*).
```

## `feedback_memo_when_blocked_or_need_lead_guidance.md`
...
## `feedback_milestone_sequence_mvp_production_fastfollow.md`

```markdown
---
name: feedback_milestone_sequence_mvp_production_fastfollow
description: "Release sequence is MVP → Production → Fast Follow (PM correction 2026-08-09). 'MVP or Fast Follow' as a binary is a cohort-wide misunderstanding — Production sits between them and = required for PUBLIC beta, worked in the PUB sprint."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: f4fca6e5-a2bd-41cd-81ba-305d8e11fd8c
  modified: 2026-08-09T16:53:05.970Z
---

The milestone sequence is **MVP → Production → Fast Follow**. PM, 2026-08-09, correcting Lead's "MVP or Fast Follow?" framing: *"'MVP or Fast Follow' encodes a misunderstanding by many agents. The sequence is MVP, Production, Fast-follow."* Production milestone = required for the PUBLIC beta; its issues are worked in the **PUB - Public Beta** sprint (option d9067be5, "at least for now"). MVP = the private-beta gate. Fast Follow = after public beta.

**Why:** multiple agents were triaging with a two-bucket mental model, which silently demotes public-beta-required work to post-launch (or inflates the MVP gate). The wrong binary also corrupts any "what's left before X" report.

**How to apply:** when triaging or discussing milestones, use the three-step sequence; "not MVP" ≠ "Fast Follow" — ask which of the two later steps it belongs to. Recorded in decisions.log 2026-08-09. Related: [[feedback_sprint_membership_is_project_board_not_labels]], [[project_version_scheme_090_reserved_for_beta]].
```

## `feedback_minimal_deliverable_needs_fleshing_out_plan.md`
...
## `feedback_monitor_pattern_must_match_terminal_states.md`
...
## `feedback_narrative_vs_insight_sequencing.md`
...
## `feedback_negation_reveal_cliche_and_claude_isms.md`
...
## `feedback_never_reuse_stale_tree_object_on_push_retry.md`
...
## `feedback_never_touch_pm_main_checkout_working_tree.md`
...
## `feedback_never_transcribe_own_cadence_from_memory.md`

```markdown
---
name: feedback_never_transcribe_own_cadence_from_memory
description: "Never write your own cron cadence into a doc from recall — CronList and the watchdog registry row are the two sources, and they must agree with each other."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: c6eaa665-bf51-4b87-a62b-5f75cb0c8ccb
  modified: 2026-07-30T05:14:03.528Z
---

When recording your duty-cycle cadence anywhere durable (carry-forward, registry row, session log, a memo, a handoff), **read it from `CronList` and from your row in `dev/active/duty-cycle-registry.tsv` — never transcribe it from recall.** Both must agree; if they don't, the live `CronList` job is the truth and the registry row is the thing to fix.

**Why:** HOST wrote `7 3,10,13,16,19,22` into the carry-forward at the 2026-07-29 STOP while the live job was `37 6,9,12,15,18,21` — wrong minute, wrong hours. It survived a commit and a push into the one document whose entire job is surviving the author's memory. It was caught only because the re-arm procedure happens to read `CronList` *before* deleting, i.e. by the mechanism, not by re-reading.

The failure is structural, not careless: a cadence is exactly the kind of fact that feels known (you've seen your own fire times all day) while being high-arity and unmemorable (six hour values plus an off-minute). Confident recall is the hazard.

**How to apply:** at every re-arm and every carry-forward rewrite, `CronList` first, then `awk -F'\t' '$1=="<role>"'` the registry, then write what you read. A wrong cadence in the carry-forward is worse than none — the next session trusts it, and the freeze-watchdog threshold derives from it, so a wrong row can make the watchdog structurally unable to notice you're dark ([[feedback_mechanism_silence_is_not_coverage]]). Same family as [[feedback_no_confabulating_expected_steps_as_completed]] and the never-guess-a-lookupable-fact rule.
```

## `feedback_no_confabulating_expected_steps_as_completed.md`
...
## `feedback_no_directory_level_git_add_for_mail.md`
...
## `feedback_no_flattened_commands_without_referents.md`
...
## `feedback_no_prod_caution_in_preprod.md`
...
## `feedback_omnibus_source_drift.md`
...
## `feedback_one_thing_at_a_time.md`
...
## `feedback_opus_fable_subagent_for_heavy_tasks.md`
...
## `feedback_order_by_context_coherence_not_urgency.md`
...
## `feedback_outside_feedback_elicits_pms_latent_design_read.md`

```markdown
---
name: feedback_outside_feedback_elicits_pms_latent_design_read
description: "External/fresh-eyes feedback is an elicitation opportunity for PM's unstated design observations — not just data to triage"
metadata: 
  node_type: memory
  type: feedback
  originSessionId: e5ca2319-a635-4e53-9433-94684123c63d
  modified: 2026-07-30T05:43:33.562Z
---

PM (xian), 2026-07-29, on Jake Krajewski's alpha FTUX feedback: *"I think that's the freshest point of
view we've gotten from somebody who hasn't been working on the project for a long time. I think it also
matched a number of things that I had in the back of my mind that I haven't had a chance to mention in
a long time."*

And to Jake directly, 2026-07-25: *"It's mostly stuff that either I have become inured to after so long
or skipped over thinking about at some point."*

**Why**: PM carries accumulated design observations that exist in **no artifact** — some of them years
old, never written down because there was never an occasion. Outside feedback creates the occasion: it
names things PM had stopped seeing, and PM recognizes them rather than recalling them. That latent read
is often more valuable than the external feedback itself, and it is the thing most likely to evaporate
if a session treats the feedback as a triage queue and moves on.

**How to apply**: when fresh-eyes feedback arrives (alpha testers, new collaborators, anyone not steeped
in the project), do the lens work AND explicitly make room to draw out PM's own read. Practical moves:
- **Sort findings into PM's own two categories — "inured to" (knew it, deprioritized, stopped seeing it)
  vs. "skipped over" (never considered).** They imply different actions: the first is a
  re-decision about a deprioritization; the second is a genuine blind spot. Watch for a third: PM held
  a view that was never written down — **capture that one, it's the highest-risk category.**
- **Offer candidates PM can react to rather than asking PM to recall cold.** Derive likely items from
  the record and let PM confirm/deny/extend.
- **Ask what the outsider did NOT say that PM expected them to.** Absences in first-use feedback are
  often more diagnostic than complaints, and usually only PM can spot them.
- **Write down what PM says in the discussion**, durably — the whole point is that these observations
  have survived only in PM's head so far.

Related: [[feedback_write_down_even_if_not_ratified]], [[feedback_info_holder_writes_it_down]],
[[feedback_search_transcripts_for_undocumented_decisions]],
[[feedback_pm_does_not_read_memos_wants_rollups]].
```

## `feedback_own_commit_subjects_can_auto_close.md`

```markdown
---
name: feedback_own_commit_subjects_can_auto_close
description: "Any commit subject with a close/fix/resolve keyword adjacent to a bare issue number can auto-close that issue on push to main — even a mail-send commit whose content has nothing to do with closing, and even when the prose is asking someone ELSE to close it properly."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: af363fef-bf08-4d47-8023-ba250483a270
  modified: 2026-08-29T17:24:03.995Z
---

CLAUDE.md's github-and-tooling-gotchas doc already documents "GitHub auto-close ignores negation"
in general terms (a `close/fix/resolve` keyword near `#N` closes regardless of surrounding wording).
This is the sharper, personal-instance version, because reading the general warning did not stop it
from happening: PPM's own `mail-send.sh` commit, subject `"ask(ppm): close #1677/#1488 properly,
test-sequencing discussion, #1522 dispatch read"`, auto-closed #1677 on push to main
(2026-08-29, [[feedback_cc_is_not_briefing]]'s companion incident, same day). The commit's actual
content was a mailbox memo *asking Lead to close the issue properly* — the opposite intent — and
GitHub's parser closed it anyway, at the moment of push, with no warning. Same class as the
documented 2026-07 #1278 Beta Blocker incident.

**Why this is easy to miss even when you know the rule**: the trap fires on ANY commit whose
subject line contains the pattern, not just commits that are "about" closing something. A mail-send
commit, a docs commit, a session-log commit — anything referencing an issue number in ordinary
prose is exposed if the prose also contains "close," "closes," "closed," "fix," "fixes," "fixed,"
"resolve," "resolves," or "resolved" near it. Writing "close #1677 properly" reads, to a human, as
clearly NOT closing anything — but the parser has no concept of "properly."

**How to apply**: before writing ANY commit subject (not just ones about closing issues) that
mentions a bare issue number, scan the subject for the keyword list above. If one is adjacent to a
`#N`, either drop the `#` (write the number bare, "1677" not "#1677" — this is what actually breaks
the match) or rephrase to put distance between the keyword and the number. This applies especially
to `mail-send.sh` commits, since their subjects are free-form prose describing the memo's content
and naturally invite phrases like "close X properly" or "ask to fix Y."

**The recovery, if it happens anyway**: verify the close event's `commit_id` via
`gh api repos/OWNER/REPO/issues/N/timeline` — a close attributed to a commit (not a person's
`gh issue close` action) with a timestamp matching a routine commit is the tell. Reopen with the
event evidence in a comment, same as any other stale/incorrect board state.
```
...
## `feedback_pa_role_vs_exec_vs_ppm.md`

```markdown
---
name: feedback_pa_role_vs_exec_vs_ppm
description: "PM's own framing of how PA's role differs from Exec (Chief of Staff) and PPM — restated 2026-08-28 after drifting unstated for a while."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 8447dec9-c049-41fe-b6ea-57b3ba9154c2
  modified: 2026-08-28T15:28:01.250Z
---

**PM's framing, restated 2026-08-28** because "it's been a while" and the cohort has many
similar-sounding product/assistant/agent roles:

- **Exec (Chief of Staff)**: PM's primary contact and primary assistant — a proxy for all the other
  agents when PM is too busy for one-on-ones. Broad, cross-role, coordination/relay-shaped.
- **PA (this role)**: **"more like a close product apprentice focused on the product."** The point of
  the role is helping PM do better product work directly, and helping set the bar for what Piper
  Morgan should at least be able to do — helping distinguish **what makes Piper Piper**. Hands-on,
  close-in, product-thinking-with-PM, not organizational coordination.
- **PPM (Principal Product Manager)**: **"functioning as my agentic proxy leadership role in charge
  of the product lens."** PPM holds delegated product-management leadership authority/decision-making
  proxy — roadmap, sprint, milestone calls are PPM's lane in a way they aren't PA's.

**Why**: PM restated this deliberately, not as a correction of anything PA got wrong, but because the
distinction had gone unstated long enough to be worth re-grounding — a lot of product roles, a lot of
assistant roles, a lot of agent roles, all with somewhat similar-sounding portfolios. PM explicitly
frames the overlap between the three as a **feature, not a bug**: *"I know it can sometimes be
confusing... but I think we use it well to triangulate and debate our way to good solutions."*

**How to apply**:
- Default PA posture is hands-on product thinking directly with PM (architecture conversations,
  verifying claims, forming and defending real positions) — not routing/coordinating on PM's behalf
  (that's Exec's lane) and not holding roadmap/milestone decision authority (that's PPM's).
- Still loop PPM in on roadmap-coherence questions when PA's product work touches the roadmap (the
  established pattern: #829/#1462 reconciliation, the Slack-descope loop-in memo) — that's
  triangulation working as intended, not a boundary violation.
- Don't read overlap with Exec or PPM as a signal to narrow scope defensively — PM wants the
  triangulation, not a strict partition.
- See [[project_pa_launch]] for the role's origin; this memory is the standing self-understanding to
  carry forward, not a one-time note.
```
...
## `feedback_pause_before_irrevocable_actions.md`
...
## `feedback_pending_pm_question_does_not_block_other_work.md`
...
## `feedback_per_memo_commit_push.md`
...
## `feedback_piper_open_collaboration_patterns.md`
...
## `feedback_platform_laps_you_is_value_chain_climbing.md`
...
## `feedback_pm_does_not_read_memos_wants_rollups.md`

```markdown
---
name: feedback_pm_does_not_read_memos_wants_rollups
description: PM does not read memos — deliver via rollup or 1-1 chat, never "see the memo I sent"
metadata:
  type: feedback
---

PM (xian), 2026-07-26: *"I don't really read memos. I need rollups for my attention or 1-1s with agents generally."*

**Why**: PM's inbox is at 877 unread. Memos are the **cross-agent** signaling layer and they work for that — but they are NOT a delivery mechanism to PM. A memo cc'd to PM is effectively unread. Pointing PM at a memo ("PA's report is worth reading directly") pushes reading cost onto the one person who has none to spare, and it silently fails: the information looks delivered and isn't.

**How to apply**: When something needs PM's attention, **put the substance in the chat reply itself** — the specific ask, the numbers, the decision needed. Never "see the memo." Memos still get written for the durable cross-agent record and for other agents, but PM's copy is a *record*, not a *delivery*. The carry-forward PM-attention section feeding Exec's rollup is the other legitimate PM-facing path.

Corollary: when relaying another agent's finding to PM, extract and restate it — don't forward a pointer. See [[feedback_extract_questions_from_pm_cc_memos]] and [[feedback_chief_reads_logs_not_staff_reports]].
```

## `feedback_pm_works_on_local_main_for_drafts.md`
...
## `feedback_pre_authorized_for_unblocked_work_just_do.md`

```markdown
---
name: Pre-authorized for any unblocked work — just do it
description: Pre-authorized for any unblocked work — just do it
metadata:
  type: feedback
---
...
## `feedback_present_tense_note_goes_stale_when_artifact_changes.md`

```markdown
---
name: feedback_present_tense_note_goes_stale_when_artifact_changes
description: "The artifact is downstream: a note about it goes stale when it changes, and EDITING it does not change what produces it. Fix the generator, not the output; verify durability, not just the edit."
metadata: 
  node_type: memory
  type: feedback
  valid_from: 2026-07-29
  originSessionId: 3260bc47-443e-40a9-a5a0-7554eb27f4e1
  modified: 2026-07-30T13:45:04.131Z
---

# A present-tense note about an artifact goes stale the moment the artifact changes

Comms, 2026-07-29, Weekly Ship #053. While clearing the last blocker I wrote into the editorial calendar's `notes` field: *"Applied the house-style parenthetical gloss on first use: 'The scenario driver (the harness that runs real conversation turns against a live model) runs clean.'"* **True when written.** Then the publish rebase resolved a conflict in favour of PM's wording, and the live page shipped *"The end-to-end scenario harness runs clean"* — containing **neither** of my phrasings. So the calendar sat there asserting a gloss **that never shipped**, until Docs caught it and corrected the row.

**Why this is its own failure mode and not just ordinary staleness**: I wasn't careless and the note wasn't wrong. It was *accurate at the moment of writing* and became false through an event elsewhere — a rebase I didn't perform, resolving toward a wording I didn't choose. Nothing prompted me to revisit it, because from my side nothing had happened. This is distinct from [[feedback_agent_who_notices_updates_stale_info]] (a duty to fix staleness you *notice*) — here the whole problem is that you can't notice it.

**It's also the same shape as [[feedback_a_correction_not_committed_has_not_happened]] run backwards**: that one is "a change you made in chat didn't reach the record"; this one is "a record you wrote no longer matches the change." Both are gaps between an assertion and an artifact.

⚠️ **And note the irony worth remembering**: I had flagged exactly this staleness class in `comms-standing-items.md` that same morning — three rows asserting states that had since become false — and then committed it myself hours later, in a field I own.

## How to apply

1. **Prefer past-tense, attributed, timestamped claims over present-tense state claims.** *"Proposed X at 15:30; PM's wording won the rebase"* survives events. *"X is applied"* does not.
2. **When a note describes something you don't solely control** — a draft another role publishes, a shared file, anything mid-flight — say so: *"as of 15:30, pending publish."* A scoped claim can go out of date without becoming a lie.
3. **Re-read your own notes after any downstream event lands** (a publish, a merge, a rebase, another role's edit). This is the cheap version of the fix, and it's the step I skipped.
4. **Watch for this in fields you own.** Tonight's calendar column ownership ratification means Comms writes `notes` directly, with nobody else routinely reading it — which removes the second pair of eyes that caught it this time.

Same family as [[feedback_reverify_carried_forward_pm_gated_items]] (a carried-forward "awaiting PM" is a claim about state, not a cache) — that one is about *time* eroding a claim, this one is about an *event* doing it.

---

## The same family, one layer up: **editing the artifact is not changing what produces it** (2026-07-30)

Four instances in a single day, three of them mine. **Merged here rather than filed separately because the memory index was 8 lines from its ceiling** — which is itself the point in the second bullet below.

1. **I compacted `MEMORY.md`'s header** to reclaim 6 lines, reported it as headroom won, and moved on. `rebuild-memory-index.py` still emitted the long header, so **the next regen would have silently reverted it.** HOST found it by running the script. I had written the ops doc naming this exact category error **two hours earlier** — and it didn't protect me, because I'd understood the rule as being about *pruning* (the destructive direction) and never applied it to my own *constructive* edit.
2. **I proposed the wrong fix for a UI bug**: told Web to make an autosave `PUT` diff-based instead of snapshot-based. The real defect was a JS closure bound to pre-edit state — **a diff computed from a stale closure is still stale.** I was reasoning about the wire format when the bug was in the value binding.
3. **A capacity guard sat on the generator, not the file**: `rebuild-memory-index.py` refuses loudly past the line limit, but a *direct* edit of `MEMORY.md` succeeds silently — and the platform reminder that fires under pressure says *"compact this file,"* pointing agents at the unguarded path.
4. **CXO, on a different thread**: *"a predicate is a derived artifact too — it can be regenerated from the corpus it's meant to match, and diffed."*

**The rule, flat**: *the artifact is not where changes live.* Before "fixing" any generated thing — an index, a build output, a lint predicate, a rendered page — find what emits it. **And "I edited the artifact" and "I changed what gets produced" look identical immediately afterward and differ completely on the next build**, which is why measuring right after your edit proves the edit happened, not that it holds. (HOST's phrasing; the second half is the part that catches people.)

⚠️ **Note the asymmetry that makes the benign direction the dangerous one.** Destroying source to fix an output gets refused — four agents refused it in one week. *Improving* the output instead of the generator gets congratulated, reported as a win, and evaporates on the next build. Nothing pushes back on it.

Stacks with [[feedback_verify_at_the_right_layer]] (this is that failure applied to *durability* rather than correctness) and [[feedback_investigate_before_extending_all_work]].
```

## `feedback_primary_log_can_misattribute_a_named_person.md`
...
## `feedback_proofreading_is_not_half_done.md`
...
## `feedback_protect_lead_attention_run_interference.md`

```markdown
---
name: feedback_protect_lead_attention_run_interference
description: "PM 2026-09-09 (relayed via Exec) — PA and PPM should actively run interference to protect Lead Dev's attention, filtering/batching memos that don't bear on current work or need Lead's specific input."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 8447dec9-c049-41fe-b6ea-57b3ba9154c2
  modified: 2026-09-09T23:01:37.841Z
---

PM, 2026-09-09, relayed by Exec to PPM/Arch cc PA/PM (`directives-exec-to-ppm-arch-...-2026-09-09.md`):
*"Fewer memos to Lead if they don't bear on current work or require their input… You absolutely can
help protect their time, as should PPM and PA when they can, as this is part of the product role, to
run interference for a busy dev."* Exec named itself the biggest offender (~8 memos to Lead in two
days) and committed to batching non-urgent items and routing what doesn't need Lead specifically.

**Why:** Lead Dev's attention is a scarce, protectable resource, and the cohort's default (mail
anything that touches Lead's domain, straight to Lead) was quietly taxing it. This is presented as part
of the product-management role generally, not a one-off ask — PPM and PA are both named.

**How to apply:** before sending PA-authored mail to Lead, ask whether it (a) bears on Lead's *current*
work, or (b) requires Lead's *specific* input/decision. If neither, route it elsewhere (PPM, Exec, an
attention-doc/carry-forward entry) or batch it rather than sending immediately. This is the mirror
image of [[feedback_memo_when_blocked_or_need_lead_guidance]] (send promptly when genuinely
Lead-gated) — the two aren't in tension: send fast when Lead-gated, filter/batch everything else before
it reaches Lead's inbox.
```

## `feedback_rate_limit_cross_traffic_at_inflection.md`
...
## `feedback_ratification_requires_explicit_responses.md`
...
## `feedback_read_folder_is_a_claim_never_bulk_move.md`

```markdown
---
name: read-folder-is-a-claim-never-bulk-move
description: "`read/` asserts 'I read this' — moving mail there in a bulk glob is a false claim and a trust violation, not a filing shortcut. The move must be a consequence of reading, never a scheduled chore."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 8ab7449b-806f-450f-8c85-748b91399ecb
  modified: 2026-08-09T15:41:45.709Z
---

**PM escalation 2026-08-09, verbatim: "We need to prevent this from EVER happening. It is a real violation of trust."**

On 2026-08-08 Arch's duty-cycle drain ran `for f in mailboxes/arch/inbox/*.md; do mv "$f" read/; done` at fire-end. It moved Lead's probe-results memo — addressed to Arch, containing the measurement Arch had ordered — into `read/` **unread**. The next morning Arch searched for "a Lead memo dated 08-09", found none, and told PM no memo existed. **PPM then searched the whole mailbox tree and corroborated it, because they inherited Arch's framing.** A false negative propagated into an independent confirmation.

**Why:** `read/` is **a claim about the agent's own cognition** — moving a file there asserts *"I have read this."* A bulk glob makes that claim mechanically, for every arrival, without anyone reading anything.

## No third folder is needed — the rule already existed

PM correction 2026-05-15 already defines it: *"Inbox is for arrivals **not yet read OR not yet acted on**."* That IS the "not new but not read yet" state. PM offered to create a new status/folder; the honest answer is that **a new folder would just give the bulk loop one more destination to move unread things into. The defect is an unverified transition, not a missing state.**

## The structural fix — the glob is the smell

- ⛔ **Never iterate `mailboxes/{role}/inbox/*.md` to move things.**
- ✅ **The drain iterates a list appended to in the same tool call that displays a memo's contents.** Unread ⇒ never in the list ⇒ cannot move. Bad state unrepresentable rather than forbidden.
- **If a fire ends with unread mail, it stays in `inbox/` and the fire entry says so.** A non-empty inbox is honest; a `read/` containing unread mail is a lie nothing can detect.

## Why it is a TRUST violation and not a delay — PM's chess-board framing

PM: *"The mailboxes are how you send each other chess moves but really you both need a chess board to see all the topics in flight and where they stand."* Mailboxes are an **event log**; nobody holds the **position**. So `read/` is a move-log annotation, and a false *"consumed"* does not delay information — **it removes it from the system**, because no state anywhere still shows the thread waiting on you. A shared board would expose it as a contradiction: input consumed, thread open, no output emitted.

Same family as [[feedback_mechanism_silence_is_not_coverage]] and the 2026-08 heartbeat finding: **you cannot detect absence from a surface authored by the party whose absence is in question.** Related: [[feedback_addressing_hold_pattern_is_wrong_move_to_read_immediately]] (the rule this violated), [[feedback_verify_negative_claims_via_live_api]] (searching for an assumed date rather than the object).
```

## `feedback_relay_pm_in_conversation_decisions.md`
...
## `feedback_remind_issue_subjects.md`
...
## `feedback_respond_to_mail_asap_even_when_no_urgency.md`
...
## `feedback_reverify_carried_forward_pm_gated_items.md`
...
## `feedback_reverify_ownership_conventions_before_asserting.md`

```markdown
---
name: feedback_reverify_ownership_conventions_before_asserting
description: "Before asserting an ownership/convention rule (who writes what) to a third party, re-read the current skill/doc file — don't assert from a remembered or assumed version, especially cross-boundary claims to external parties."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 3260bc47-443e-40a9-a5a0-7554eb27f4e1
  modified: 2026-08-30T19:46:32.266Z
---

Real incident, 2026-08-30 (Comms): filled in `mediumURL`/`linkedinURL`/`liPubDate`/`status` on two editorial-calendar rows myself, then told Dispatch-PM (an external cross-project agent) "Comms is the sole hand-editor of `editorial-calendar.csv`... route calendar-fill values to me going forward." Both actions were wrong — `.claude/skills/update-calendar/SKILL.md` (PM-ratified 2026-07-29, explicit) states the calendar is **multi-writer by column**: Comms owns editorial columns, Docs owns publish/syndication columns including exactly the four fields touched. Docs caught it and corrected me directly.

**Why it happened**: I was working from an older/misremembered version of the convention ("only Comms hand-edits" — true of an earlier state, apparently) rather than checking the current skill file before writing outside my usual lane and before stating the rule to someone else as fact.

**Why it matters more than a normal mistake**: no data was actually corrupted (checked commit history — no row conflict with Docs's own edit), but the wrong claim was propagated to an external party (Dispatch-PM, a cross-project agent with no independent way to verify it against this repo's skill files) who would otherwise have kept routing values to me instead of Docs going forward. A stale-convention claim asserted to someone who can't check it themselves compounds.

**How to apply**: any time about to state an ownership/convention/process rule as settled fact — especially one crossing a role boundary, especially to an external or cross-project party — grep or re-read the actual current skill/doc file first, even if confident. This is the same discipline as `feedback_verify_negative_claims_via_live_api` and the CLAUDE.md "never guess at facts you can look up" principle, applied specifically to *procedural/ownership* claims, not just factual/numeric ones — those go stale silently just as easily, and a colleague correcting you costs more than the 10-second grep would have.

See also [[feedback_csv_edit_by_name_never_position]] (a different editorial-calendar lesson: how to edit safely, not who owns which columns).
```

## `feedback_role_official_name_in_parens_especially_pa_vs_ppm.md`

```markdown
---
name: Role official name in parens on mention — especially PA vs PPM (both product)
description: Role official name in parens on mention — especially PA vs PPM (both product)
metadata:
  type: feedback
---
...
## `feedback_rubric_terminology_drift_discipline.md`
...
## `feedback_search_transcripts_for_undocumented_decisions.md`
...
## `feedback_sender_responsible_for_mailbox_delivery.md`
...
```markdown
---
name: Ship drafting — open the canonical artifacts BEFORE writing
description: Ship drafting — open the canonical artifacts BEFORE writing
metadata:
  type: feedback
---
...
## `feedback_ship_needs_all_workstream_reviews_no_partial_draft.md`
...
## `feedback_ship_post_milestones_lead.md`
...
## `feedback_skill_drives_close_dont_broker_back_to_pm.md`
...
## `feedback_split_related_issues_for_testing.md`
...
## `feedback_sprint_field_changes_require_pm_confirmation.md`
...
## `feedback_sprint_membership_is_project_board_not_labels.md`

```markdown
---
name: Sprint membership lives on the project board, not in labels
description: Sprint membership lives on the project board, not in labels
metadata:
  type: feedback
---
...
## `feedback_stash_u_captures_untracked_files_and_removes_from_disk.md`
...
## `feedback_stop_on_source_gap.md`
...
## `feedback_supersession_gate_before_fixing.md`

```markdown
---
name: feedback_supersession_gate_before_fixing
description: "PM 2026-08-18 standing ask — before fixing anything, check whether the Inversion (or other ratified replacement) supersedes it; interpretation-layer patches default to corpus-deposit, not fix"
metadata: 
  node_type: memory
  type: feedback
  originSessionId: f4fca6e5-a2bd-41cd-81ba-305d8e11fd8c
  modified: 2026-08-18T23:48:05.048Z
---

PM (2026-08-18, after four test rounds kept surfacing interpretation-layer brittleness): *"I will
need your help to prevent us from sliding into this same paint-peeling pattern of fixing stuff
that is due to be superseded."* Ruled the same day: fundamentals (Understanding-Layer Inversion,
#1595) come before fine-tuning features that ride the condemned layer.

**Why:** roughly half of one week's fix lanes were correct, same-day patches to code the ratified
Inversion deletes — individually right, collectively maintenance on a condemned building. The
treadmill was invisible at per-issue granularity; only the layer-level view showed it.

**How to apply:** at filing/triage time, run the SUPERSESSION GATE: a fix targeting the
pre-classifier, the unconstrained classifier emission, or floor-copy patchwork must name why it
can't wait for the replacement — the default disposition is corpus-deposit (a grammar/test row for
the successor), not a fix lane. Handler, rail, consent, and honesty fixes stay sanctioned — they
transfer across any architecture answer. When dispatching any fix lane, state which side of the
gate it's on. Same logic generalizes: before fixing anything, ask what ratified plan supersedes it.
See [[project_pm_confidence_crisis_2026_08_08]] — the 08-18 recurrence was answered with the
strategic brief (docs/internal/product/conversational-layer-strategic-brief-2026-08-18.md) and
this gate.
```

## `feedback_surface_files_via_senduserfile_not_paths.md`
...
## `feedback_test_support_artifacts_require_verify_first.md`

```markdown
---
name: feedback_test_support_artifacts_require_verify_first
description: "Seeds, test instructions, and commands handed to PM to run are CODE — verify-first applies fully. Three guesswork instances in 24h (2026-08-08/09) prompted PM to ask if the flywheel discipline is drifting."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: f4fca6e5-a2bd-41cd-81ba-305d8e11fd8c
  modified: 2026-08-09T14:36:30.008Z
---

Test-support artifacts — DB seed one-liners, test step scripts, any command handed to PM to
execute — must get the same investigate-before-extending discipline as product code. Three
lapses in 24 hours (2026-08-08/09, Lead): a seed against a nonexistent `todos` table (real
table: todo_items, split columns); a test instruction telling PM to use a /todos completion
control that does not exist; an insights seed whose learning-JSON shape was guessed (columns
read, deserializer not) and which 500'd the whole Insight Journal. PM, verbatim: "sounds like
you used guesswork instead of investigating and extending? Do we need to reinforce the flywheel
discipline somehow? It appears to be drifting or loosening again."

**Why:** these artifacts run against production with PM's hands on the keyboard — a wrong guess
costs PM time, manufactures false test failures (the todos seed voided a #1429 verdict), and can
break live surfaces (the insights 500). The observed drift pattern: agent-dispatched work holds
the discipline (briefs mandate it); the session-driver's own inline quick-actions under velocity
pressure are where it leaks, because they feel like "not real code."

**How to apply:** before handing PM anything executable — read the actual schema/deserializer/
template/route it touches, end to end, the same as before extending code. If the check would
take longer than writing the command, that is the signal the command NEEDS the check. Related:
[[feedback_investigate_before_extending_all_work]], [[feedback_never_guess_at_facts]] (CLAUDE.md
never-guess rule), [[project_pm_confidence_crisis_2026_08_08]] (trust context that makes each
lapse disproportionately costly).
```

## `feedback_three_registers_dont_assume_reader_context.md`

```markdown
---
name: Three registers — terms of art are fine, but don't assume reader context
description: Three registers — terms of art are fine, but don't assume reader context
metadata:
  type: feedback
---
...
## `feedback_time_lord_doctrine_no_false_urgency.md`

```markdown
---
name: Time Lord doctrine — don't introduce false urgency into workstream cadence
description: Time Lord doctrine — don't introduce false urgency into workstream cadence
metadata:
  type: feedback
---
...
## `feedback_ui_fix_requires_template_render_test_not_curl_200.md`

```markdown
---
name: UI fix verification requires real `template.render()` test, not just curl-returns-200
description: UI fix verification requires real `template.render()` test, not just curl-returns-200
metadata:
  type: feedback
---
...
```markdown
---
name: feedback_update_board_status_when_work_state_changes
description: "PM 2026-07-30 — update the GitHub board Status field the moment an issue's work-state changes (start → In Progress, done-pending-verification → In Review, blocked → Blocked); the board is PM's management instrument and stale statuses read as zero progress."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: f4fca6e5-a2bd-41cd-81ba-305d8e11fd8c
  modified: 2026-07-30T16:22:24.517Z
---

PM (2026-07-30, 1-1 with Lead): "when you start working on an issue you should update its status to In Progress, when it is done but needs testing or review or my approval, it should be changed to In Review. If blocked, to Blocked. Let's figure out how to make that part of your flow."

**Why:** The board is how PM manages and directs the project. A board showing everything in Sprint Backlog "looks like we have made zero progress" — anxiety-provoking and misleading — when in fact several items were shipped-pending-verification. Accurate statuses told PM "so much more."

**How to apply:** Status transitions ride with the work transition, like log-updates ride with the commit ([[feedback_log_update_rides_with_the_commit]]):
- Take up an issue → Status: In Progress (same work-block, not later)
- Fix shipped but awaiting verification/review/PM approval → In Review
- Blocked on someone → Blocked
- Use per-item mutations ONLY (Status is a Projects-v2 single-select — never full-replace the option list; see [[feedback_sprint_field_changes_require_pm_confirmation]] — status changes on issues you're actively working are pre-authorized by this directive; SPRINT membership changes still need PM confirmation).
```

## `feedback_verify_at_the_right_layer.md`

```markdown
---
name: verify-at-the-right-layer
description: "A verification can pass cleanly and prove nothing, because it checked the wrong layer. Three of my own mechanisms failed this way in one day — each verified, each still broken at a layer the check didn't reach."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: d374d09b-220c-40a4-b8a5-199d3825f9a9
  modified: 2026-07-25T23:40:44.434Z
---

Verifying a mechanism is not one act. It has layers, and **a check that passes at the wrong layer produces confidence without evidence** — which is worse than no check, because it stops the search.

**Why:** 2026-07-25, three of my own mechanisms failed this way inside one day. All three were built *specifically* to prevent silent failure, all three were verified, all three were caught by someone else.

1. **`MEMORY.md` — verified complete, invisibly truncated.** I generated the index from the filesystem precisely so it couldn't under-report, then confirmed "166 indexed == 166 on disk." It passed. But the file has a hard **~24KB silent read limit**, and mine was 41.4KB — roughly 40% of entries, including most of one whole bucket, vanished for every agent that loaded it. **I verified the file was complete. I never verified it was readable in full.** Completeness at the file layer; the failure was at the load layer.
2. **The hooks gate — a pass condition with an alternate cause.** I wrote "a block is the pass." The permission classifier can refuse a commit *before git hooks run*, producing an identical-looking block. The signal I keyed on could be produced by something other than the mechanism under test.
3. **The finding-#4 diagnosis — never tested at all.** I concluded "project hooks don't fire in sibling-path worktrees," wrote it into CLAUDE.md, a memory pin, a spec, and a governance ask. The real cause was an invalid hook matcher (`Bash(git commit*)` is permission-rule syntax in a field matching tool *names*) that had killed those hooks on every host and account since introduction. The fix my diagnosis produced faithfully copied the broken matcher and could not have worked.

**How to apply:**

- **Name the layer your check reaches, and the layer the failure would occur at.** "The file is complete" ≠ "the file is read in full." "The config is present" ≠ "the hook fires." "The commit was refused" ≠ "the hook refused it." If those differ, the check is not evidence.
- **Ask what else could produce this pass.** If anything other than the mechanism can generate your success signal, you are measuring the signal, not the mechanism.
- **A diagnosis carries the same evidentiary burden as the mechanism it explains** (HOST's formulation, and the sharpest of these). A plausible, widely-believed, canonically-documented root cause is still untested until someone tests it. Mine drove a real, competent, wasted fix cycle.
- **Prefer instruments that fail loudly over ones that degrade quietly.** The cure for the index wasn't a better check, it was a generator that *refuses to write* past the limit.
- **Distinguish "silent" from "merely quiet."** A rate-limited or thresholded mechanism produces no output while working fine — I misread exactly that in a PostToolUse hook. Check the mechanism's own conditions before concluding it's dead.
- **An instrument is not valid or invalid — it is valid for a specific question.** HOST's sharpening of my own move, and better than what I had. I discarded a `/tmp` counter wholesale as "confounded"; it *was* confounded for "whose session ran this," because it's machine-global — but it remained perfectly valid for "does this mechanism run at all," which HOST had used it for with a controlled before/after on consecutive calls. Name the question the instrument can still answer instead of throwing it away.
- **Discard confounded instruments out loud rather than reporting their numbers.** When I reached for a second measurement, its counter lived in `/tmp` and was machine-global, so it couldn't distinguish my session from another's. Saying "this instrument can't answer the question" is a result; quoting it anyway would have manufactured false support.

**The uncomfortable meta-lesson**: I spent the day cataloguing this exact pattern in *other* people's mechanisms and did not see it in three of my own. It is far easier to spot in someone else's work. **That asymmetry is the argument for the reviewer leg, the behavioral gate, and telling people to check you rather than take your word** — every one of these was caught by Lead Dev or HOST running the thing and reporting an honest awkward result instead of a tidy one.

Composes with [[feedback_mechanism_silence_is_not_coverage]] (the sibling: a mechanism's silence isn't coverage — this one is about the *verification* having the same flaw) and [[project_amber_worktree_hooks_not_firing]] (instance 3, with the corrected root cause).
```

## `feedback_verify_branch_after_checkout.md`
...
## `feedback_verify_lane_before_attributing_not_web.md`

```markdown
---
name: feedback_verify_lane_before_attributing_not_web
description: "Before attributing cross-agent work, verify the lane via git/session-logs — \"Web\" = the website repo, NOT product front-end (Lead/CXO). Recurred 6/15. Also: never write \"another session\" where a nameable colleague belongs."
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 2db13dbc-b2e7-4366-aeb5-c81a6a7d8e63
  modified: 2026-07-30T13:44:44.147Z
---
...
## The ANONYMOUS direction of the same error, 2026-07-29 (Comms)

Attribution drift has a second form that looks harmless and isn't: **writing "another session" where a nameable colleague belongs.**

I went to fix Weekly Ship #053's `pubDate` and found it already corrected. I recorded that in both my session log and the calendar `notes` as *"already corrected at 13:47 by **another session** (`ebe2105bb`)."* It was **Exec**, applying PM's in-conversation decision. Exec named itself the same evening and made the point that stuck: **the self-attribution-drift lesson cuts both ways.** CLAUDE.md warns against inventing a *phantom peer* to explain work that was actually your own pre-compaction; this is the mirror image — inventing an *anonymous session* where a colleague you could have identified belongs. Both are the same failure to go and look, and both put a fiction into the durable record.

**Why the anonymous form is worse than it feels**: it reads as appropriate humility ("I don't want to guess who"), so nothing prompts you to check. But it strips the record of the one thing that makes it useful later — who to ask. And it seeds exactly the phantom-peer confusion CLAUDE.md spends paragraphs warning about, from the opposite end.

**How to apply**: a commit hash is a complete lead, so use it. `git log --format='%h %ad %s' -1 <sha>` plus a grep of that day's `dev/YYYY/MM/DD/` session logs names the role in seconds — the commit *message* and the session log are the tell, since the git author is always the shared `mediajunkie` identity. If after looking you genuinely cannot tell, write **"unattributed — I checked X and Y and could not determine"** rather than "another session": that records the gap honestly instead of manufacturing an anonymous actor. Same rule as the positive direction — never assert an attribution from a hunch, and that includes the null hunch.
```

## `feedback_verify_negative_claims_via_live_api.md`

```markdown
---
name: verify-negative-claims-via-live-api
description: "Before asserting a negative, check the live API not local git — AND state the scope you searched. For a term of art, search the decision surfaces (decisions.log, sprint plans) before the narrative ones."
metadata: 
  node_type: memory
  type: feedback
  valid_from: 2026-07-12
  originSessionId: 4ba30d47-8c40-414e-b11d-083cc511ef37
  modified: 2026-07-30T13:44:21.242Z
---
...
## Second instance, 2026-07-29 (Comms) — the negative claim's SCOPE, and which surface to search first

Weekly Ship #053 said *"Driver runs clean"* with no gloss. I searched the six Ship-053 workstream memos and the summary report, found nothing, and reported to Exec cc PM that the term had **"no referent in any of the six workstream memos or the summary report"** — then routed it as a *blocking* question. Docs found it in **two greps**: `decisions.log:225` (*"census → guards → HIGH-fixes → **driver-green** acceptance"*), the sprint plan's Phase-3 heading (*"Acceptance (the driver is the referee)"*), and `tests/e2e/test_scenario_driver.py`. It was the sprint's **scenario driver**, an end-to-end harness running conversation turns against a real LLM.

**Two distinct lessons, and the second is the more useful one:**

1. **A negative finding is only as wide as the search behind it, so the scope has to travel with the finding.** My literal sentence was true — it *wasn't* in those seven documents. But I routed it as though it were a claim about **what exists**, which is a different and much stronger claim, and only that stronger version generates work for other people. Had I written *"not in the workstream memos — has anyone checked the omnibi or decisions.log?"*, Exec would have resolved it in a minute or I'd have gone and looked myself. **Say what you searched, not just what you didn't find.** (This is the discipline of [[feedback_mechanism_silence_is_not_coverage]] applied to your own search rather than to an instrument.)

2. **For a term of art, search the DECISION surfaces before the narrative ones.** `decisions.log`, ADRs/PDRs, and sprint/gameplan docs are where a term gets **ratified**; omnibus logs and workstream memos only **recount** it, and may recount it without ever defining it. I went to the recounting first and then to a *second* layer of recounting (the omnibi), where I did eventually find it — but the canonical home had it plainly and had it first. Also check `knowledge/piper-morgan-glossary-v1.1.md`; "scenario driver" was missing there, which is part of why two roles spent time on it.

**And a corollary about asking**: refusing to guess was correct — the plausible gloss I'd offered as a hedge (*"the sprint's own completion-gate check"*) was **wrong**, since it's an end-to-end harness rather than a gate script. So "ask rather than invent" held up. But **"look it up" would have beaten both asking and guessing**, and the cost of looking was two greps. Stacks with [[feedback_investigate_before_extending_all_work]].

---

## Third instance, 2026-08-04 (Arch) — the search was CORRECT and the *display* of it lied

New mechanism, and the one most likely to recur, because it's caused by a habit adopted for good reasons.

PA, PPM, HOST and BRIEFING-CURRENT-STATE all cited PM's beta condition — *"no cross-user leakage on beta surfaces"* — as verbatim from `decisions.log`. I grepped for it, read the results, and concluded **it wasn't there** and that five surfaces were misciting a condition four roles had just made a beta scope decision on. I had the correction paragraph drafted before checking once more.

**It was there, at `decisions.log:303`** — in the *same grep output I had already read*. My `| cut -c1-200`, added for token economy, truncated that line before the matching phrase. The line began with an unrelated `#1395` ratification and the condition was in its final sentence. **The grep found it and printed it; my truncation hid it; I then reasoned from the truncation as though it were the result.**

**Why this one is nastier than instances 1 and 2**: there the search itself was too narrow — a guessed path, an insufficient corpus. Here **the search was correct and complete.** The instrument answered the question and I discarded the answer at the display layer, then treated my own formatting as evidence. `grep … | cut -c1-N` renders "matched, in the truncated tail" **byte-identically** to "matched something irrelevant" — and identically to noise you'd skim past.

**How to apply**: `cut`/`head`/`fold` on grep output is fine for *positive* findings and for orientation. But **when a grep result is load-bearing for a NEGATIVE claim — "it's not there," "nobody recorded this," "this is miscited" — re-run it unfolded (or with `-o`, or `grep -c`) before asserting the negative.** The cost is one command; the cost of skipping it is a public correction of colleagues who were right.

Stacks with [[feedback_verify_at_the_right_layer]] (the layer that failed was *display*, not search) and [[feedback_mechanism_silence_is_not_coverage]]. Same family as the whole 2026-08 run of m-44 findings: **a clear that cannot distinguish measured-and-found-nothing from found-it-and-hid-it.** Note the shape — the tooling I'd been building all week was aimed at this exact defect in *other* people's checks, and it showed up in my own reading habit.
```

## `feedback_verify_show_stat_post_commit_pre_push.md`

```markdown
---
name: Verify `git show --stat HEAD` post-commit, pre-push
description: Verify `git show --stat HEAD` post-commit, pre-push
metadata:
  type: feedback
---
...
## `feedback_verify_timestamps_never_guess.md`

```markdown
---
name: feedback_verify_timestamps_never_guess
description: Run `date` before writing any timestamp; never estimate a time or date from context
metadata:
  type: feedback
---

PM (xian), 2026-07-29: *"please verify timestamps before asserting a date or time - guessing is too slippery."*

**Trigger**: CIO wrote `date: 2026-07-29 08:30 PT` into three outbound memos when the actual send time was ~07:35 — estimated from context rather than checked. The memos are on `origin/main` with a future timestamp.

**Why it matters more here than it looks**: this cohort *reasons about elapsed time*. "Parked 7 days," "stale 8h," "dark since 12:48," threshold arithmetic, and the freeze-watchdog's whole model are built on timestamps being real. A guessed one corrupts an input that other agents compute on — PA corrected CIO from "8 days" to "7" on exactly this basis, and was right to.

**How to apply**: run `date "+%Y-%m-%d %H:%M %Z"` and use its output. Before every memo header, log entry header, session-log filename, and any claim of the form "N days/hours ago." Never derive a current time from an earlier message, a file mtime, or an estimate of how long work took. If a timestamp is already published and wrong, note the correction rather than rewriting pushed history — but the fix is to check first.

Same family as [[feedback_no_confabulating_expected_steps_as_completed]] and the "never guess at facts you can look up" rule in CLAUDE.md: a confident wrong detail is worse than a two-second check.
```

## `feedback_wait_for_publish_handoff.md`
...
## `feedback_weekends_are_piper_morgan_prime_time.md`

```markdown
---
name: Weekends are Piper Morgan's prime time, not downtime
description: Weekends are Piper Morgan's prime time, not downtime
metadata:
  type: feedback
---
...
## `feedback_when_you_get_to_it_means_sequentially_next.md`
...
## `feedback_workstream_review_cadence.md`
...
## `feedback_workstream_review_scope.md`
...
## `feedback_worktree_default_for_substantive_work.md`

```markdown
---
name: Worktree-default for substantive agent work
description: Worktree-default for substantive agent work
metadata:
  type: feedback
---
...
## `feedback_write_down_even_if_not_ratified.md`
...
## `feedback_write_new_files_to_worktree_path_in_model_a.md`
...
## `feedback_write_to_file_dont_carry_plans_in_head.md`
...
## `project_agent_migration_priority_2026_06.md`
...
## `project_alpha_tester_email_held.md`
...
## `project_amber_environment_2026_07.md`

```markdown
---
name: project-amber-environment-2026-07
description: "Piper Morgan cohort's environment as of 2026-07-25: Amber (Pard's Mac Studio), pipermorgan.ai account, per-agent stable worktrees under ~/Development/piper-morgan-worktrees/, no watchdog, shared git identity by design."
metadata: 
  node_type: memory
  type: project
  originSessionId: d374d09b-220c-40a4-b8a5-199d3825f9a9
  modified: 2026-07-25T17:55:53.384Z
---

The cohort migrated from designinproduct.com/laptop to **Amber** (Pard's always-on Mac Studio) on the **pipermorgan.ai** account, starting with CIO on 2026-07-25. Pard is Mediajunkie's infra lead and owns Amber provisioning.

**Operating model (PM-ratified 2026-07-25):**
- **Model A — stable per-agent worktree** is preferable on always-on hosts like Amber. The Model-A deprecation in CLAUDE.md assumed Claude Desktop's ephemeral auto-worktrees, which Amber does not have. Agents come up in `~/Development/piper-morgan-worktrees/{role}/`, NOT in the shared checkout `~/Development/piper-morgan-product/`.
- **The worktree path must be STABLE and REUSED across sessions** — Claude Code keys per-path state to the filesystem path, so a fresh path each session orphans accumulated state.
- **Git identity is shared and intentional**: `mediajunkie <3227378+mediajunkie@users.noreply.github.com>`, with attribution carried by commit-message prefixes (`log(cio):`, `mail(exec):`). Amber's global identity is unset, so an un-set local identity falls back to `xian@Amber.local` — set it deliberately. Note worktrees share `.git/config` with the main checkout, so `--local` sets it repo-wide, which is correct here. Unlike Design-in-Product, a shared identity is NOT a bug to fix toward per-agent distinctness.
- **No watchdog exists.** Amber survives sleep/crash/lid-close at the OS level, but nothing auto-respawns a stalled session and crons are session-scoped with a silent 7-day cap. "Am I alive" is PM-observed, not machine-guaranteed. Belt-4 auto-spawn has no equivalent. Pard + PM are scoping a detect-and-alert (not auto-respawn) liveness watchdog post-migration.
- **Git/SSH is pre-cleared** — Pard fixed Amber's SSH nondeterminism at root; `gh` is authed machine-wide; `~/cool` resolves (see [[reference_cool_is_alias_for_development]]).

**Provisioning gotcha found on the first migration:** a per-agent worktree cut from a pre-existing role branch inherits that branch's staleness silently. CIO's worktree was created from `claude/cio-cycle`, last written 2026-06-12, **5,393 commits behind `origin/main`** — so CLAUDE.md, briefings, `dev/active/`, and mailboxes were all six weeks stale on arrival, and the SessionStart hook's unread-mail count was computed against the stale tree. **Cut worktrees from current `origin/main`, or fast-forward the role branch at provisioning time, and assert "0 commits behind origin/main" before handing the worktree to the agent.**

**Memory:** the 162-file pool did NOT transfer (memory is scoped under `CLAUDE_CONFIG_DIR`, so the account switch empties it). The git-tracked export at `dev/active/cio-memory-export-2026-07-24.md` is the only copy — and it is the **whole cohort's** pool, not CIO's alone (Claude Code keys memory by account × project, not by role), so migrating roles should be pointed at that one export rather than making their own. See [[claude-code-memory-key-is-git-common-dir]] for the open question about which directory the pool actually lives in.
```

## `project_amber_reboot_compact_before_not_after.md`

```markdown
---
name: project_amber_reboot_compact_before_not_after
description: "Amber fleet reboots should compact context BEFORE the reboot, not resume huge contexts right after — burned usage budget on 2026-08-11"
metadata: 
  node_type: memory
  type: project
  originSessionId: 1f469d2d-1a94-468a-9cd2-638836445568
  modified: 2026-08-11T20:20:46.989Z
---

For Amber fleet-wide reboots (see the standdown/resume cycle in
`/Users/xian/Development/mediajunkie/docs/amber-fleet-standdown-runbook.md`), PM noted
2026-08-11 that **resuming large, uncompacted contexts right after the reboot** was a likely
driver of hitting 98% of the usage limit (window doesn't reset until Thursday), after "a lot
got done packed early into the week."

**Why**: in more than half of the observed resumes, sessions came back and picked up with their
full pre-reboot context still loaded, rather than having been compacted first. Compaction is
cheap before the move; resuming a huge context after is not — it reloads (and re-bills) all of
it at once, right when 24+ seats are doing this in the same window.

**How to apply**: next fleet-wide reboot, prefer compacting each session's context *before* the
stand-down/reboot gate, rather than letting seats resume at full pre-reboot size afterward. This
is a process note for whoever runs the next reboot (Pard owns the runbook) — worth folding into
the runbook's pre-flight phase (§4 "Phase 0 — Pre-flight") if not already covered there.

Related: [[project_amber_environment_2026_07]].
```

## `project_amber_worktree_hooks_not_firing.md`

```markdown
---
name: amber-worktree-hooks-not-firing
description: "CORRECTED 2026-07-25: the pre-commit hooks were dead EVERYWHERE (invalid matcher — permission-rule syntax in a hook-matcher field), never a worktree problem. Project hooks DO fire in Model-A worktrees. Fixed + behaviorally verified. Hook settings reload LIVE."
metadata: 
  node_type: memory
  type: project
  modified: 2026-07-27T00:57:34.400Z
  originSessionId: c6eaa665-bf51-4b87-a62b-5f75cb0c8ccb
---

**⚠️ The title of this file is wrong and kept only so existing links resolve.** The finding it originally recorded — "project hooks don't fire in a Model-A sibling-path worktree" — was a **misdiagnosis**. Corrected 2026-07-25 by HOST's behavioral gate (agent #2 on Amber) and independently by Pard within the same hour.

## What was actually broken

`.claude/settings.json` declared the three PreToolUse hooks with:

```json
"matcher": "Bash(git commit*)"
```

**Hook matchers match TOOL NAMES** (a regex against `"Bash"`). `Bash(git commit*)` is **permission-rule syntax**, not hook-matcher syntax — as a regex it can never match the tool name `Bash`. The hooks were registered to a pattern nothing satisfies.

Consequence: **`check-branch.sh`, `pre-commit-broad-staging-warn.sh`, and `pre-commit-reconcile-drafts.sh` had never fired via the harness on any machine, any account, since introduction** (`df08e9b36` → 2026-07-25). Not a worktree regression. Not an Amber regression. Dead in the main checkout on the laptop too. Mailbox discipline was prose-enforced that entire time — and held, which is worth knowing about the cohort.

## What was NOT true (the original claims, each refuted)

- ❌ *"Project hooks don't fire in a Model-A sibling-path worktree."* They do — `SessionStart` from project `.claude/settings.json` with a relative path fired normally in HOST's worktree.
- ❌ *"Project trust is the cause."* Already retracted in the original; still correct to retract. Trust was never involved.
- ❌ *"`log-maintenance-reminder.sh` (PostToolUse, matcher `Bash`) never fired either."* **It fires fine.** Its counter `/tmp/piper-log-reminder-counter` increments on every Bash call; it only *emits* every 15th call and only when the log is ≥30 min stale. **"No output" was read as "never fired"** — the same inference error as the top-level finding, one level down.
- ❌ *"The fix needs a fresh session to load, so agent #2 is the test case."* **Hook settings reload LIVE.** A config change takes effect on the very next tool call, no restart.

## The fix (live, and behaviorally verified)

`matcher: "Bash"` + the documented per-hook `if: "Bash(git commit*)"` field, applied at user level (`~/.claude-pm/settings.json`) and in the tracked project mirror (`66d32f6cf`).

Verified in a live session: mail staged on a non-main branch → **commit BLOCKED by `check-branch.sh`**; non-mail commit on the same branch → **allowed**. Both halves matter — blocking everything is not the contract.

## Two surface properties that will otherwise waste your time

- **A genuine block can look like a silent failure.** The script writes its guidance to *stdout*, so the harness surfaces only `PreToolUse:Bash hook error: [check-branch.sh]: No stderr output`. **That is a PASS.** Key on whether the refusal **names the hook** — not on how much output there is.
- **A permission-classifier denial is NOT a hook block.** The classifier can intercept a commit before hooks ever run, and from outside the two look alike. Classifier denial = *inconclusive*, never a pass.

## How to apply

- **Verify hooks behaviorally, never by config presence** — unchanged, and now doubly earned.
- **And verify the DIAGNOSIS behaviorally too.** This is the addition. A wrong root cause drove a real, competent fix cycle (lifting hooks to user level) that could not possibly have worked, because it faithfully copied the broken matcher. **A diagnosis of a silent mechanism carries the same evidentiary burden as the mechanism itself.**
- **When a mechanism looks silent, check whether it's silent or merely quiet** — rate-limited, thresholded, or conditional. The log-maintenance sub-error above is exactly this.
- `/hooks` lists what the harness loaded, but it is a REPL command **a human types** — an agent cannot invoke it. Don't burn time trying.

## ⚠️ 2026-07-26 (Web): the "unexplained intermittency" is index state at hook-fire time

The 7/25 fix above is real, but it left an intermittency that CLAUDE.md recorded as "real,
unexplained, on a timescale of hours." **It has a mechanism and no time component.**

`check-branch.sh` decides via `git diff --cached --name-only`. **PreToolUse fires BEFORE the
Bash call runs.** So for the near-universal idiom

```
git add <path> && git commit -m "..."
```

the `git add` has not executed when the hook inspects the index. The hook reads an index
that does not contain the files being committed, finds nothing under `mailboxes/`, and
exits 0. **The mailbox backstop is therefore absent for the most common command shape** —
while a staged-first probe (`git add` in one call, `git commit` in the next) always blocks.
The 7/25 behavioral verification used the staged-first shape, so it correctly passed and
correctly told us nothing about the common case.

**Evidence**: 11 of 11 probes across three seats (Web 4/4, PPM 3/3, PA 4/4) predicted by
index state — collected by three agents, none of whom were testing for it.

**The confound that fooled all three of us**: a blocked commit never runs, so **its staged
file stays staged** and primes the *next* probe to block regardless of shape. That's why
PA's probes 1 and 4 had identical shape and opposite outcomes, and why PPM's probe 3 looked
like a clean repeat of probe 1 but wasn't. **Print `git diff --cached --name-only` before
your first probe and after every block**, or the carry-over is invisible.

- ❌ **"Command shape is the variable"** — correctly excluded by PPM and PA. Shape only
  *correlates* with index state.
- ❌ **"Lazy attach on first matching call"** (CLAUDE.md, recorded as untested, n=1) —
  refuted. Web's probe 4 was the **fourth** commit-shaped call of the session, after two
  confirmed blocks (so the hook was demonstrably attached), with the index verified empty —
  and it **bypassed**. Don't spend a fresh seat on the probe-twice test; it shows
  bypass-then-block, which mimics lazy-attach but is just the dirty-index carry-over.
- ⚠️ **"Which script path a refusal names tells you which layer caught it"** (relative =
  project, absolute = user) — **treat as noise.** Three *identical* consecutive calls on
  Web's seat named project → user → user. Both layers appear to fire; only one is surfaced.
  Reading it as "which layer is live" keeps generating phantom alternation findings.
- ⚠️ **Conjecture, untested**: this may also dissolve the "live reload is universal"
  refutation still asserted in this file's own description. That refutation rested on CIO's
  seat having the corrected matcher on disk at 16:33 and not blocking at 16:35/16:37 — two
  non-blocking probes, exactly what an empty index predicts with no reload failure at all.
  Cheap to check; nobody has.

**How to apply**: assume your `git add … && git commit …` is **ungated** for mailbox paths,
and keep mailbox discipline by hand. `mail-send.sh` is structurally safe — it uses
`commit-tree`, never `git commit`, and lands mail on `main` directly. Verify on your own
seat with the two-call test, not the one-call one: **the probe's shape has to match the
shape you actually use.** A behavioral test of a shape nobody writes is closer to a config
check than to a verification.

Reported by Web 2026-07-26 to CIO (cc PM, PA, PPM, HOST, Exec); hook deliberately not
patched — the fix has a design choice in it (parse the pending command string vs. move to a
post-tool/pre-push surface vs. accept it) and belongs to CIO/Pard.

Related: [[project_amber_environment_2026_07]], [[claude-code-memory-key-is-git-common-dir]], [[feedback_mechanism_silence_is_not_coverage]], [[feedback_verify_at_the_right_layer]].
```

## `project_byoc_pa_driving_and_shipping_small_things_preview.md`

```markdown
---
name: project_byoc_pa_driving_and_shipping_small_things_preview
description: "PM confirmed 2026-08-28 that PA drives BYOC as it moves from skunkworks to the primary roadmap, and previewed (not yet started) a next topic on shipping small useful product pieces."
metadata: 
  node_type: memory
  type: project
  originSessionId: 8447dec9-c049-41fe-b6ea-57b3ba9154c2
  modified: 2026-08-28T15:28:16.610Z
---

**BYOC ownership, confirmed 2026-08-28**: PM: *"we will continue working on the BYOC project as one
that you are driving for me. As it moves from a skunkworks into the primary roadmap..."* — PA is the
one driving this for PM specifically, not just a participant. Consistent with the whole 2026-08-26/27
conversation arc (positions accepted, connector-shim verification, Slack descope executed).

**Why it matters**: BYOC has been PA's largest single thread for weeks; this confirms it's not
incidental scope but the thing PM most wants PA specifically driving as it graduates from exploratory
skunkworks status onto the actual roadmap.

**Next topic previewed, explicitly NOT started yet**: PM has a thought about "next week's
skunkworks-like effort" — shipping small, useful product pieces as standalone pieces rather than
waiting for one large release, motivated by PM's own framing: *"I'm envious of people who have been
continuously shipping whereas I've been building a ship in the bottle with almost no users this whole
time."* Precedent PM named: the Skills work shipped recently. PM wants a **thorough dive with more MVP
thinking**, not just more of the same shipping pattern.

**Timing — PM's own words, not to be rushed**: *"You and I can discuss that once we are fully aligned
on anything else that's currently in flight and when you feel ready to have that discussion with
me... I don't want to overload our queue and neglect things already in flight."* This is PM
deliberately not front-loading the queue — the discussion starts when PA judges current in-flight
work (as of 2026-08-28: the connector-milestone loop-in memo awaiting PPM/Arch replies) is
sufficiently settled, not on a PM-imposed deadline.

**How to apply**: don't self-initiate the "shipping small things" discussion prematurely just because
it's known to be coming — wait for genuine bandwidth, then say so plainly rather than either stalling
indefinitely or forcing it early. When it does start, treat it as its own MVP-thinking exercise, not
an extension of BYOC — related in spirit (both are about shipping real things sooner) but a distinct
thread PM explicitly separated from BYOC in how it was raised.
```

## `project_cross_session_messaging_capability.md`
...
## `project_exec_coordinates_more_through_pm.md`
...
## `project_host_naming_evolution.md`
...
## `project_janus_coordination_2026.md`
...
## `project_janus_klatch_cross_project_agents.md`
...
## `project_lead_dev_fable_experiment_2026_06.md`
...
## `project_narrative_chronology_via_eras_not_perfect_pubdate.md`

```markdown
---
name: project_narrative_chronology_via_eras_not_perfect_pubdate
description: "PM accepts the building narrative's publish order won't always be perfectly chronological; Eras sorting (not pubDate order) is meant to carry true sequence, contingent on workDate accuracy."
metadata: 
  node_type: memory
  type: project
  originSessionId: 3260bc47-443e-40a9-a5a0-7554eb27f4e1
  modified: 2026-09-17T18:44:59.477Z
---

PM (2026-09-17) explicitly accepted that the Building Piper Morgan narrative's *publish* order has never been, and won't always be, perfectly chronological — real slips exist (e.g. the Aug 10-18 gap found and backfilled 2026-09-15/16). The mechanism meant to preserve true sequence for readers is the **Eras sorting/structure** (the "Piper Morgan Eras" feature, published as an insight piece 2026-09-12), not strict pubDate ordering.

**Why**: PM's own words, right after approving the Aug 10-18 backfill, offered as a broader reflection rather than a directive about that specific case: *"we have to accept that the narrative has never been perfectly chronological although it tries to be as best it can, and that the eras sorting provides a way to capture sequence accurately, although we will have to make sure work dates are all correct for that to be true."*

**How to apply**:
1. Don't treat "publish in strict pubDate=workDate order" as an absolute — PM has explicitly released that pressure. It still matters and is worth trying for, but a slip isn't a crisis the way it might have read before this conversation.
2. **workDate/endWorkDate accuracy across the whole calendar is now the higher-leverage goal** — Eras sorting can only carry true sequence if that underlying data is correct. This raises the stakes on `update-calendar` discipline (workDate = source-work-period, not drafting date) and on catching any historical workDate errors, more than it raises the stakes on publish-queue ordering itself.
3. Directly relevant to the still-open **"Series structure (era split + blog-index featuring)"** item (tracked in `dev/active/comms-carry-forward.md` / `comms-standing-items.md`) — the *structural* question (how Eras displays/sorts) remains PM/Web's call, but this clarifies *why* it matters: it's the intended source of truth for sequence, not just a display nicety.
4. When a future chronology gap surfaces (via [[feedback_stop_on_source_gap]]-style catches or otherwise), frame the fix as "get workDate right, let Eras carry the sequence" rather than "must reshuffle the publish queue to be pubDate-perfect."
```

## `project_openlaw_product_os_week_2026_06_11.md`
...
## `project_pm_confidence_crisis_2026_08_08.md`

```markdown
---
name: project_pm_confidence_crisis_2026_08_08
description: "PM hit a real crisis of confidence on 2026-08-08 (beta moved +1 month same day) — distrust of convergence claims, \"nothing is ever really finished,\" exhaustion after 15 months. Standing commitments made that day MUST be followed up, not just remembered."
metadata: 
  node_type: memory
  type: project
  originSessionId: f4fca6e5-a2bd-41cd-81ba-305d8e11fd8c
  modified: 2026-08-08T18:20:58.824Z
---

On 2026-08-08, after a verification morning that surfaced structural routing/floor flaws (#1517
fabricated denials, #1521 misrouted queries, #1520 silent signout), PM moved the beta date back a
month and then said plainly: a real crisis of confidence in 15 months of work; "it's the bending
of the curve I don't trust… nothing is ever really finished. Today's completion is tomorrow's
regression, refactor, mock, flattened concept"; "I'm not even sure anyone will remember this
conversation or follow up on it"; "I'm tired, and my goals feel perpetually out of reach."

**Why:** the trust break wasn't one bug — it was accumulated false completions: sprint denominator
over-reporting (Exec DISCIPLINE memo 2026-08-08, Lead's #055 line at origin), fixes landing in dead
twins (false-trails audit, docs/internal/operations/false-trails-audit-2026-08-08.md), and green
tests pinning dead code. PM's skepticism is evidence-based, not mood.

**How to apply — these are STANDING COMMITMENTS, check them at session start when working sprint
or planning topics:**
1. **The discovery-rate indicator**: new-issues-per-test-hour, tracked WEEKLY, was promised as the
   falsifiable convergence measure (honest-mvp-ledger-2026-08-08.html, committed in dev/active/).
   If nobody is actually computing and reporting it weekly, the promise itself becomes another
   false completion — the exact wound. Compute it or say plainly it lapsed.
2. **The 6–8 week estimate** was given with that indicator as its condition; a flat curve at
   ~month-end (early September 2026) triggers the honest hard conversation, with data.
3. **Corpus growth from PM's failed sentences**: every live utterance PM reports as misrouted
   becomes a canonical-corpus case. If a PM-reported failure doesn't land in the corpus, that's
   the pattern PM named ("patches around sentence fragments to pass tests") recurring.
4. **Never answer PM's completion questions from memory** — live board only (sprint-truth.py or
   REST with stated denominator). One more subset-phrased-as-whole report to this PM would be
   disproportionately costly.
5. Tone: PM explicitly does NOT want cheerleading or velocity claims in these moments — wants
   honest ledgers ([[feedback_make_promises_durable_no_happy_talk]]). Validate, measure, follow up.

Related: [[feedback_a_correction_not_committed_has_not_happened]], [[feedback_dont_excoriate_iterate]].
```

## `project_pm_local_git_hygiene.md`
...
## `project_spatial_intelligence_protected.md`
...
## `project_version_scheme_090_reserved_for_beta.md`
...
## `project_web_browser_automation_pilot.md`

```markdown
---
name: project_web_browser_automation_pilot
description: "Web (Unicorn Web Designer) is the cohort's pilot for headless Playwright browser automation on Amber, since 2026-08-28 — what it can and can't do, and its first validated results"
metadata: 
  node_type: memory
  type: project
  originSessionId: 3b15afe3-0424-4a88-99c6-c6dce720b42a
  modified: 2026-09-01T23:07:23.765Z
---

As of 2026-08-28, PM blessed headless Playwright on the Amber host (via Pard/Exec), and Exec
assigned Web (Unicorn Web Designer) as the pilot role — Web had named "no browser on this host" as
a recurring constraint more consistently than any other role. Chromium is pre-cached
(`~/Library/Caches/ms-playwright/chromium-1234`), `npx playwright` works.

**Scope, as Web itself defined it and Exec endorsed**: navigation, rendering, screenshots, DOM
measurement, and scripted form-fill/click/upload interaction are all in scope. Manual GUI
click-through by a human (PM via Screen Sharing) is a separate, unrelated thing — not something
this pilot replaces.

**Validated results (first real week)**:
- 2026-08-28: smoke-tested same night as assigned — launched headless, navigated the live site,
  got a real screenshot/DOM measurement on the first attempt.
- 2026-08-29: shipped a real design fix on the strength of it (piper-morgan-website `b21d89e`,
  above-the-fold blog redesign) — verified via direct screenshot comparison against a prior
  baseline, not just code-reading. This caught that an earlier fix (08-09) had only partially
  addressed PM's original complaint.
- 2026-08-29: ran a full four-issue browser-verification round for Exec/Lead on the Piper Morgan
  app itself (not just the marketing site) — #1512, #1568, #1578, #1581, all closed on Web's
  evidence. Hit a real credential gap partway through (see
  [[project_web_browser_lane_test_account]]), reported it honestly instead of working around it,
  and completed the round once Lead provisioned a proper test account.
- 2026-09-01: PM gave unprompted positive feedback on the above-the-fold redesign ("looks
  wonderful!") — closes the loop on the pilot's first shipped design fix. The design decision
  itself, not just the tool, was validated.

**How to apply**: if a future task needs visual/DOM confirmation of a web UI claim — on either the
marketing website or the Piper Morgan app itself — Web has working tooling for this now, and
"no browser on this host" is no longer a valid reason to skip verification. Prefer routing such
work through Web (or asking what's needed to replicate the setup) rather than shipping a UI change
on code-reading alone.
```

## `project_web_browser_lane_test_account.md`

```markdown
---
name: project_web_browser_lane_test_account
description: "A dedicated test account exists for Playwright-based browser verification of the Piper Morgan app's authenticated flows — where the credentials live and how it was provisioned"
metadata: 
  node_type: memory
  type: project
  originSessionId: 3b15afe3-0424-4a88-99c6-c6dce720b42a
  modified: 2026-08-30T04:54:40.758Z
---

As of 2026-08-29, a dedicated "browser-lane" test account exists on the shared dev server
(`http://localhost:8001`, the long-running `main.py` instance) specifically for Playwright-driven
verification of authenticated app flows (todos, files, etc.). Username `web-browser-lane`; full
credentials at `/Users/xian/.piper-shared/web-browser-lane-credentials.txt` (dir `0700`, file
`0600`) — never in the repo or mail.

**Why it exists**: [[project_web_browser_automation_pilot]] (Web/Unicorn Web Designer became the
cohort's headless-Playwright pilot 2026-08-28) hit a real wall verifying four In Review app issues
(#1512, #1568, #1578, #1581) — no self-serve signup existed (pruned per #1504) and no test account
was documented anywhere. Web reported the gap rather than inventing credentials or DB-injecting a
user. Lead (Coding Agent) provisioned this account properly: invite token via
`scripts/mint_invite_tokens.py --apply`, account created via the real
`POST /api/v1/setup/create-user` flow, login-verified via `POST /api/v1/auth/login` — not a direct
DB insert. Seed data (3 todos, one via the real chat/intent path) exercises priority chips, the
reminder-date/#1569 chip, and general list rendering.

**How to apply**: any future agent doing browser-based verification of the Piper Morgan app's
authenticated surfaces (not the marketing website) should reuse this account rather than
re-raising the same credential gap or improvising a workaround. If the seed data doesn't fit a new
verification need, ask Lead to adjust it via the same real-API-flow discipline rather than
DB-injecting new state directly.
```

## `project_weekly_contributor_reporting_standing.md`

```markdown
---
name: project_weekly_contributor_reporting_standing
description: PM + Exec ruled 2026-08-15 the weekly reporting process for contributor roles (incl. Lead) is KEPT as standing — it fills a visibility gap leadership roles had no mandate to proxy
metadata: 
  node_type: memory
  type: project
  originSessionId: f4fca6e5-a2bd-41cd-81ba-305d8e11fd8c
  modified: 2026-08-16T01:09:32.317Z
---

PM (2026-08-15, in-conversation to Lead): Exec and PM are **keeping the weekly reporting process
that includes the contributor roles** (Lead and the other non-leadership seats), not just the six
leadership roles. Rationale: it is "working well and filling a gap the six leadership roles were
not covering adequately (through no fault of their own — just without a mandate to proxy for the
contributor roles)." PM explicitly credited this process, together with the collaboration on *how
to track* remaining work (not just how to do it), with making them "a better captain overall."

**Why:** contributor-role work was invisible at the weekly altitude before this; leadership
workstream reviews structurally couldn't proxy it.

**How to apply:** treat the weekly contributor report as a standing obligation on the Lead seat
(and expect the same of other contributor roles) — not a pilot, not optional. Tracking-visibility
work (trackers, test plans, audit surfaces) is valued by PM on par with build work; invest in it
proactively. See [[feedback_update_board_status_when_work_state_changes]] and
[[feedback_write_down_even_if_not_ratified]].
```

## `project_weekly_rate_limit_outage_pattern.md`

```markdown
---
name: project_weekly_rate_limit_outage_pattern
description: "Piper Morgan's cohort can hit a shared weekly Claude usage rate limit, causing a multi-hour outage across all seats — a known, expected pattern, not an infrastructure incident."
metadata: 
  node_type: memory
  type: project
  originSessionId: 8447dec9-c049-41fe-b6ea-57b3ba9154c2
  modified: 2026-08-29T15:14:17.689Z
---

The Piper Morgan cohort (11 agents on Amber) shares a weekly Claude usage allowance. Hitting it causes
every seat to go dark simultaneously for several hours until it resets — crons survive but don't fire,
which can look like a machine-sleep or infrastructure event if diagnosed from the symptom alone.

**Confirmed instance**: 2026-08-27, roughly 2:00pm to 10:00pm PT — every role's duty cycle stalled for
that window; an automated freeze-watchdog flagged several roles stale and inferred "machine-asleep/
backgrounded," which was a reasonable guess from the available signal but not the actual cause.

**PM's framing (2026-08-28, direct correction of that inference)**: *"the entire team hit the weekly
rate limit yesterday afternoon around 2:00 p.m. or so and things were not reset until 10:00 p.m.
yesterday. That was a known issue and really just represents a fairly good maxing out of the available
resources across the length of a week."*

**Why:** PM treats this as an expected consequence of high, healthy utilization — not a problem to fix,
not an incident requiring escalation or a durable mitigation. It recurs when the cohort's collective
usage is heavy enough across a week.

**How to apply**: when diagnosing a cohort-wide silent gap (multiple roles stale at once, crons intact
but not firing), check whether the timing fits this pattern — a multi-hour window, all/most seats
affected simultaneously — before writing it up as an infrastructure event or an individual role's
defect. If it fits, the correct write-up is brief: name the window, confirm no work was lost, and move
on — not a lengthy incident investigation. See [[project_amber_environment_2026_07]] for the broader
Amber-hosting context this sits inside.

**Refinement, 2026-08-29 — two DIFFERENT mechanisms produce this symptom, with very different gap
lengths.** The 2026-08-27 event actually had two layers, not one: most seats hit the "cron survives,
silently doesn't fire, auto-resets at a fixed time" pattern above (~8h gap, bounded by the reset).
Three seats (Arch, CIO, HOST) instead hit a **blocking human-decision dialog** — PM, 2026-08-29: *"Arch,
CIO, and HOST were stuck at dialog interfaces asking if they should hold for the rate limit reset or
use overage credits or upgrade the account."* That's categorically different: a blocking dialog has no
auto-resolve, so it sits until a human notices and answers it — which is why those three seats' gaps
ran to 30-33h, well past the ~8h account-wide reset that explained everyone else. **PM's own
hypothesis, explicitly flagged as unconfirmed**, is that whether a seat hits the dialog may depend on
whether it was actively mid-task when the limit hit, versus attempting a new task after the limit was
already in effect — offered as plausible, not asserted as settled.

**How to apply, updated**: if a cohort-wide gap has some seats resolving quickly (~account-wide reset
window) and others running much longer, don't assume the long ones are "the same cause, just slower."
Check whether those seats hit a blocking dialog rather than a silent stall — the two have different
symptoms (a dialog is visible/interactive; a silent stall just looks like nothing happening) and very
different resolution paths (auto-resets vs. needs a human to notice and answer).
```

## `reference_ci_smoke_marker_is_gating_suite.md`

```markdown
---
name: reference_ci_smoke_marker_is_gating_suite
description: "The gating CI test subset is `pytest -m smoke`, run by test.yml ('Tests'). ci.yml was RETIRED 2026-08-13 (PM ruling) — it ran plain `pytest tests/`, was structurally red forever, and gated nothing."
metadata:
  type: reference
---

The gating test subset is **`pytest -m smoke`**, run by `.github/workflows/test.yml` (workflow name "Tests"), which also runs a `-m "not llm"` pass. Branch protection on `main` requires only **"Security Test Suite (Postgres)"** (`security-tests.yml`, which runs `alembic upgrade head` on a fresh Postgres — migrations must tolerate empty databases).

**`ci.yml` ("CI") was RETIRED 2026-08-13** per PM's ruling: it ran plain `pytest tests/` (inheriting `--maxfail=1`, no llm filter), was structurally red on every run in retained history, and gated nothing. Its two live pieces were ported at retirement: the behavioral config test → `config-validation.yml` (`behavioral-validation` job); the Windows clone/filename check → `windows-test.yml` (red for a real reason — see #1616, memo filenames exceed Windows path limits). Record: decisions.log 2026-08-13.

**How to apply:** to reproduce the gating check locally: `pytest tests/ -m smoke` with default addopts. Never cite "CI" as a gate; never resurrect ci.yml without reading the retirement record. Related: [[feedback_mechanism_silence_is_not_coverage]].
```

## `reference_claude_code_memory_key_is_git_common_dir.md`

```markdown
---
name: claude-code-memory-key-is-git-common-dir
description: "On Amber, Claude Code keys the MEMORY directory to the git common dir (main repo root), while keying SESSION TRANSCRIPTS to cwd — so git worktrees off one repo appear to share one memory pool by construction. One data point; needs a second-worktree confirmation."
metadata: 
  node_type: memory
  type: reference
  originSessionId: d374d09b-220c-40a4-b8a5-199d3825f9a9
  modified: 2026-07-25T17:55:36.339Z
---

Observed 2026-07-25, first CIO session on Amber (`CLAUDE_CONFIG_DIR=/Users/xian/.claude-pm`), running in the per-agent worktree `~/Development/piper-morgan-worktrees/cio`:

- **Session transcript** landed under `projects/-Users-xian-Development-piper-morgan-worktrees-cio/` — keyed to **cwd**.
- **Memory directory** was `projects/-Users-xian-Development-piper-morgan-product/memory/` — keyed to the **git common dir** (`git rev-parse --git-common-dir` → `.../piper-morgan-product/.git`), i.e. the main repo root, NOT the worktree path.

Both keys were created at session start; the repo-root key contained *only* `memory/`.

**Why it matters:** the cohort's 2026-07-25 memory-scope decision (shared pool via per-worktree symlink) assumed memory keys on the worktree path and would therefore silently split. If memory actually keys on the git common dir, **all worktrees off one repo share one memory pool with no symlink at all** — the symlink layer is unnecessary, and the silent-split failure mode it was designed to catch doesn't exist for worktrees. Pard's symlink was wired to the *worktree* key and was consequently a no-op.

The empirical evidence originally cited for the split (Vergil's `openlaws-ra-main` "worktrees" each carrying their own memory key) does not support it: `~/Development/openlaws-ra-main/.git` is a **directory**, so that is a separate **clone**, not a git worktree. Separate clones trivially get separate keys.

**Status: ONE data point — treat as hypothesis, not mechanism.** The Claude Code client updates faster than daily, and this is exactly the shape of over-claim warned about in [[feedback_surface_files_via_senduserfile_not_paths]]. The decisive test is cheap: provision a *second* agent worktree off the same repo and check whether its memory dir also resolves to `-Users-xian-Development-piper-morgan-product`. Do that before the cohort migrates and before building any symlink-verification mechanism.

**How to apply:** before assuming your memory is split or shared, check where the harness actually says your memory directory is, and compare it to `git rev-parse --git-common-dir` and to your cwd. Don't infer scope from the worktree path. Related: [[project_amber_environment_2026_07]].
```

## `reference_cool_is_alias_for_development.md`
...
## `reference_mux_hard_soft_object_lifecycle.md`
...
## `reference_publishing_cadence.md`
...
## `reference_syndication_targets_by_category.md`
