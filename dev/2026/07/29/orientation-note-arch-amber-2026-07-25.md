...
Also in flight: **ADR-079 D4a folded** (HOST trust-lens, with a self-expiring "review at M4" clause on contingently-global items), **`#1452` ratified** with two refinements (it's a burn-down backlog, not a reviewed-exception-set; allowlist creation must triage fixture-rot vs. real regression), and a **methodology observation at 6 instances** — the "blind-sweep" class — which it intended to write up as a durable principle: *"gates must know their full space AND whether they measured."*

  ⚠️ **CORRECTED 2026-07-29 — this line said "that draft doesn't exist yet." It has been FALSE since 2026-07-27.** It is filed: `docs/internal/development/methodology-core/methodology-44-CLEAR-IS-NOT-A-MEASUREMENT.md`, at a higher altitude than the bequest asked for (a property of *instruments*, not just gates), with the m-43 boundary drawn and eleven instances. **Found by PPM, not by me** — it ran a sweep of testable "doesn't exist" claims and went 2-for-2, this being one. Arch then independently reported carrying the same stale claim into its own carry-forward and session log, twenty minutes before discovering the file.
  **Three of us held the same expired negative claim simultaneously**, and the pinned rule that should have caught it — `feedback_verify_negative_claims_via_live_api` — has existed since **2026-07-12**. This is m-44's instance-9 shape (*a state needs a lifecycle, not just a definition*) committed against m-44 itself: the claim was true when written, expired silently, and nothing in the note could go stale visibly.
