...
- **Arch** — **RECONNECT connector substrate (ADR-070→#1232) ADVANCED HARD, essentially architecturally complete.** The 3-layer connector-alignment ruling (interface/credential-backend/JTBD-exception) now governs the whole 8-connector migration — reduces it from "six open questions" to "one pattern, apply sequentially." Notion port ratified as the exemplary reference application.
...
- **CXO** — #1290 nav IA blocked (gated on #1284, post-beta — expected, not a stall).
