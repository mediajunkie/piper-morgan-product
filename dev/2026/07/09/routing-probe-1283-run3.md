# #1283 behavioral routing probe — 2026-07-09 07:45 PT

Corpus: `tests/fixtures/routing_corpus_1283.yaml` (29 rows). Rail size at probe time: 89 registered actions.
**Result: 26 PASS / 0 FAIL / 2 REVIEW**

| phrase | expected | got action | got category | conf | on rail | verdict |
|---|---|---|---|---|---|---|
| give me my standup | `action:show_standup` | `get_project_status` | STATUS | 1.0 | ✗ | SEAM(pre_classifier owns this phrase in production; classifier-only result is informational) |
...
| who am I? | `action:get_identity` | `get_identity` | IDENTITY | 0.95 | ✗ | PASS(surface: pre-classifier/floor) |
...
| any stale PRs? | `action:stale_prs_query` | `stale_prs_query` | QUERY | 0.88 | ✓ | PASS |
...
| how productive was I this week? | `action:productivity_query` | `analyze_productivity` | ANALYSIS | 0.88 | ✓ | PASS(alias-equivalent: analyze_productivity -> same handler) |
| list my projects | `action:manage_portfolio` | `manage_portfolio` | PORTFOLIO | 1.0 | ✗ | PASS(surface: pre-classifier/floor) |
...
| what have you learned about my workstyle? | `action:pull_insights` | `pull_insights` | MEMORY | 1.0 | ✗ | PASS(surface: pre-classifier/floor) |
...
| write a short update for the CEO on where we are | `action:write_stakeholder_update` | `write_stakeholder_update` | QUERY | 1.0 | ✗ | PASS(surface: pre-classifier/floor) |
