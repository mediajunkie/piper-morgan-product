# Memory pool — VERBATIM EXPORT (post-prune)

**Taken 2026-07-27 by HOST, immediately after the merge/prune.** Pairs with
`memory-export-2026-07-27-pre-prune.md` — diff the two to see exactly what changed.

**167 files** (includes MEMORY.md).
...
**166 memories on disk.** Generated from the actual directory listing, never from a prior index.
...
**The line limit cannot be fixed by shortening text.** One entry = one line, so with 166 memories on disk the floor is 166 lines before any header — the real options are prune/merge, per-type index files with a router, or a denser entry format (cheapest, and worst for recall, since the description is what makes an index useful). **That is a governance decision about the whole cohort's shared pool, not a formatting choice for whoever trips the limit.** ⚠️ **Memory files are NOT under version control — deletion is irreversible. Export to a git-tracked file BEFORE pruning anything.**
...
## Feedback — how this cohort works, and the corrections behind it (143)
...
metadata: 
  ...
  modified: 2026-07-26T00:31:18.187Z
---
...
Related: [[feedback_verify_at_the_right_layer]] (the substance of that day's corrections — worth keeping; the tallying wasn't).
````
