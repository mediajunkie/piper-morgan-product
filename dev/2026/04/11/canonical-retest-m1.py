#!/usr/bin/env python3
"""
M1 Canonical Retest — Post-M1 Floor-First Validation

Evolved from canonical-retest-884.py (M0 runner) with these v3 additions:

1. Dual scoring: routing + quality (Colleague Test rubric)
2. Floor-aware expected destinations (per canonical-query-test-matrix-v3.md)
3. LLM-as-judge quality scoring with confidence + escalation
4. Honest failure tagging (known issues run anyway)
5. M0 vs M1 comparison
...
Plan:
    dev/2026/04/11/canonical-retest-m1-plan.md

Usage:
    # Ensure server is running on localhost:8001
    # Ensure ANTHROPIC_API_KEY is set in env (for the judge)
    POSTGRES_PORT=5433 ./venv/bin/python dev/2026/04/11/canonical-retest-m1.py

Outputs:
    dev/2026/04/11/canonical-retest-m1-results.csv
    dev/2026/04/11/canonical-retest-m1-report.md
"""
...
PASS: total >= 7 AND no dimension scored 0
...
def judge_response(query_text: str, response_text: str, anthropic_client) -> dict:
...
Score this response per the rubric. Return only the JSON object."""
    ...
    try:
        ...
        if parsed.get("relevance") == 0 or parsed.get("context") == 0 or parsed.get("tone") == 0:
            if parsed.get("verdict") != "FAIL":
                parsed["verdict"] = "FAIL"
                parsed["rationale"] = parsed.get("rationale", {})
                parsed["rationale"]["override"] = "auto-fail rule (one dimension scored 0)"
...
def run_query(session, query_num, query_text, category, expected_routing, known_issue, pathological_reason, anthropic_client) -> dict:
    ...
    try:
        ...
        if JUDGE_ENABLED and anthropic_client and result["tier_a_verdict"] in ("OK", "FAIL"):
            judge_result = judge_response(query_text, message, anthropic_client)
            if "error" in judge_result:
                ...
                if conf < JUDGE_CONFIDENCE_THRESHOLD:
                    ...
                    result["escalate_to_human"] = True
                    result["escalate_reason"] = "auto-fail (dimension scored 0)"
...
def main():
    ...
    if JUDGE_ENABLED:
        try:
            ...
            if api_key:
                ...
                print("\nLLM-as-judge: enabled (claude-sonnet-4-20250514)")
            else:
                print("\nLLM-as-judge: DISABLED (no ANTHROPIC_API_KEY)")
        except Exception as e:
    ...
    # Write outputs
    csv_path = OUTPUT_DIR / "canonical-retest-m1-results.csv"
    report_path = OUTPUT_DIR / "canonical-retest-m1-report.md"
