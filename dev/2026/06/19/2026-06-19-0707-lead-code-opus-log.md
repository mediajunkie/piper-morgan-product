...
## D1 gate cleared — #1293 fixed + closed; canonical regression CLEAN
- **#1293 fix** (`023f05d3f`): floor-prompt orientation carve-out (the flat "don't list capabilities" rule over-fired on Identity queries — the name rule already had an "unless asked" carve-out; capabilities didn't). CI guard added (`9eb8f50ef`, `test_addendum_answers_orientation_queries_1293` — the guard caught a line-wrap brittleness in my own assert before it false-greened; fixed by whitespace-normalizing). 42 floor unit tests pass.
- **Verified:** Identity canonical Q1–Q5 = 25/25 (judge); **full canonical regression = 252/252, 0 failed/skipped/deselected** (16 min; suite grew 222→252 during the day, all pass). Q4 fixed in full context. **#1293 CLOSED.**
- **Inbox** (read, no action; formal move-to-read deferred to next clean mail-loop): CXO #1290 (PM nav-instinct sound → hold for D2 w/ #1284+#1286); CIO #1259 (bridge swap live, already adopted). No #1270 alignment reply → supports → D2.
- **D1 closeable.** Board: #1090 ✅ Production · #1164 ✅ beta-hide · #1270 → D2 (PM move) · #1293 ✅ closed. Gate evidence (fold-in): G1 persistence = #1238/#1252/#1250 + radar/standup test coverage; G2 anti-flattening = the 252/252 regression; G3 multi-tenancy = #1252/#1250 (real-principal + owner-scoped reads + cross-owner tests). **Pending 2 PM actions: (1) move #1270→D2 on board, (2) nod gate fold-in. Then post formal D1-close + cut alpha.**

## #1270 beta band-aid SHIPPED + D1-close ready (both PM actions done)
PM moved #1270 → D2 (+ Production milestone), conditioned on making files-vs-documents coherent for beta today; and approved the gate **FOLD-IN** (a thorough forensic close comment, not a separate gate issue). PM will cut the 0.8.8 alpha via Piper after D1 closes (not Lead's task).
- **Band-aid shipped** (`a15012f33`, server restarted, health 200): `/documents` → redirects to `/files` (ui.py); nav_rail collapsed to one "Documents" item → /files (Files dup dropped, id=nav-files kept for nav.js active-state); command_palette "View documents" → /files. **Completeness diff drove the direction** (PM "thanks for the diff idea"): files.html is the working browser (22 JS fns) vs documents.html's 7 (the Stage-4+ #422 Q&A perspective — preserved in git, deferred to the #1270 D2 refactor).
- Tests: test_nav_rail_1280 updated (Files collapsed → 5 "Your work" items + assert Files-item gone); function-level redirect guard added to test_smoke_static (calls `documents_ui` directly — the auth middleware 401s unauthed TestClient, so a direct call verifies the redirect). 116 green. `test_template_renders` = PRE-EXISTING unauthed-/ env failure (untouched). `navigation.html` is DEAD (nothing includes it; test_navigation.py tests it) → spawned a cleanup task.
