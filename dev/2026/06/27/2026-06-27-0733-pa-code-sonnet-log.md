...
- Resume (post-compaction) — Corrected MCPB terminology (MCPB ≠ plugin; installs via Connectors or double-click, not Personal plugins). Built v0.1.8.mcpb — both zip structure and install instructions now correct. TESTER-QUICKSTART.md updated. Committed `f9358c6` on skunkworks main. Saved MCPB/plugin terminology distinction to memory. MEMORY.md was over 24.4KB limit (25.6KB) — compacted to ~16KB with shorter entries and merged sections. Inbox triage: 0 new items (all Jun 19–26 items confirmed in read/ from prior session). Cleared stale PA inbox MANIFEST. Synced worktree from origin/main.
