...
## Fire 28 (19:4x PDT — "someone" investigation + #1236 Phase 2 FRONTEND done + STALE-SERVER catch & restart)
PM: "frontend next please. i have been waiting for this makeover for months! please investigate the 'someone' issue first."
- **"Someone" investigation** (dev/ files reverting mid-session): root = **concurrency contention** — ~26 concurrent claude instances + 18 worktrees sharing one `origin/main` + shared `dev/active/`; frequent `git merge origin/main` syncs by other agents race against my uncommitted working-tree writes → my dev/ appends get clobbered before commit (Fire 26 + carry-forward B1 were the casualties). **Mitigation (adopted locally, holding):** commit dev/ writes IMMEDIATELY on append + verify-by-content (`git show HEAD:<file> | grep`); commit-before-sync. **To flag to Arch/CIO as a cohort scaling problem** (parked — not mine to unilaterally fix cross-lane).
- **#1236 Phase 2 FRONTEND DONE** (commit `98f460cdc` → on `origin/main` `8f8994e52`, verified by content). `history_sidebar.html`: self-contained radar-card CSS (token-with-fallback); `renderRadarCard()` (**XSS-safe — all user data via textContent**) + `renderRadar()` (empty-state teaching explainer + attention-first cards + **honest provenance** ● observed / ○ example per #1214/#1216); `updateRadar` on the HistorySidebar namespace; **frame-agnostic** so F2's page-shell (#1171) re-homes it. `home.html`: `loadRadar()` behind **`?radar=1` feature flag (default off → zero regression, PM-UAT-able)** + graceful fallback to the conversation list on fetch failure.
- **TDD (test-after for the JS-render layer, content-assertion convention** matching `test_home_recently_module_1194`): `test_history_sidebar.py::TestRadarSurface` (12) + `test_home_radar_wiring_1236.py` (5). **Full #1236 suite: 84 green** (8 domain + 3 route + 17 frontend + 56 sidebar).
- **STALE-SERVER catch (Pattern-045 / my own "curl-200 ≠ works" memory):** running server (PID 95577) started **06:59 — BEFORE tonight's Phase 2 backend commit (19:22)**. `/api/v1/radar` returned 401, but a **nonexistent path ALSO returned 401** → the 401 is a **catch-all auth middleware before routing**, NOT route confirmation. `/openapi.json` (200, unauth) **lacked `/api/v1/radar`** → route genuinely NOT mounted in the running server. Had I told PM "go to ?radar=1" it would've 404'd → silent fallback → PM sees the plain history list, NOT the Radar (broken months-awaited UAT). **Restarted** the dev server from the worktree with the canonical env-strip (`env -u ANTHROPIC_API_KEY …`) → new PID 30306, health 200, **`/api/v1/radar` NOW in OpenAPI schema (route mounted ✓)**, no `APIConnectionError`, no startup tracebacks. Server runs from MY worktree (cwd-confirmed) so it serves the fresh templates.
- **UAT-ready**: `http://localhost:8001/?radar=1` shows the Radar surface in the sidebar slot; default (no flag) unchanged. Live authenticated end-to-end smoke = PM's UAT gate (route auth-gated; everything testable-without-auth is verified: route mounted, templates fresh, 84 tests green, graceful fallback proven).
- **NEXT**: progress-comment #1236; then on PM UAT → Phase 3 (entity-search + richer types as PPM lands #706) + close-properly w/ Layer-2 gate (PDR-002 + mockup). D1 sequence after: F3 #1172 → F2 #1171 (~20-min CXO primitives-sync) → F1 #1170 (sync) → C1 #1173 → #1202/#1184 → #1048.

## Fire 29 (PM conversation, ~21:5x PDT — anti-flattening catch: Radar = 4 entity types; PM "ship it all" for beta; backend recon + PPM memo)
PM anti-flattening Q ("radar is more than conversations, right?") + directive: **"there is no partial ship. we are in alpha headed for beta. we need to ship it all"** → full 4-type Radar (**WorkItems / Documents / People / Conversations** per PDR-002) is a **BETA-SHIP REQUIREMENT**, not slot-in-later.
- **Verify-First backend recon**: Conversation ✅ live; **Document** ⚠️ partial (`DocumentService` `services/knowledge_graph/document_service.py` exists, public surface = `get_relevant_context(timeframe)`, NO per-user list → small add, **Lead-solo buildable**); **WorkItem** ⚠️ partial (GitHub `list_issues(repository,…)` `github_integration_router.py:237` is REPO-scoped → needs identity map **#1233** RECONNECT-WS9); **People** ❌ no backend — but per **CXO #1217 memo (2026-06-14, PPM cc'd)** it's **PPM's entity-model lane** (typed human/agent/stakeholder **relationship-network map**) AND **backs the #1217 personhood ethics floor** (double-duty infra, not just a card). PDR-002 names all 4.
- **"How we overlook this" (PM's wondering)**: NOT a total blind spot — People was being designed *today* (#1217 thread). The gap = **integration tracking between two parallel workstreams** (Lead's Radar *surface* vs PPM/CXO's entity *model*); no umbrella tying "Radar ships when all 4 EntitySources exist". The `EntitySource` seam let the surface build on conversations alone (good decoupling, masked the missing backends). + my own slip: cited **closed** discovery epic 706 as "the dependency" without verifying — the trace-every-referent discipline applied to *product scope*, not just code.
- **PPM memo SENT** (cc CXO) via bridge → on main `6b8d372a7`: backend-reality table + `EntitySource` contract (`async fetch(user_id)->list[RadarEntity]`; facets entity_type/title/lifecycle_state/provenance/meta/attention/ref) + ship-it-all directive; asked People-tracking confirmation + 4-backend sequencing + umbrella-tracker proposal.
- **Plan to PM (awaiting steer)**: (1) start **Document** EntitySource solo (unblocked, pre-authorized); (2) carve umbrella "Radar: the 4 EntitySources" tracker (child #1090 — **PM-gated board op**); (3) sequence WorkItem (w/ #1233) + People (PPM entity-model) — freeze `EntitySource` contract so theirs drops in. **Honest framing: "ship it all" = right beta bar, but a sequenced multi-lane effort (2 of 3 remaining have cross-lane deps), not a tonight-wrap.**

## Fire 30 (PM conversation, ~22:2x PDT — proper flywheel planning: 4 issues WRITTEN + audit-cascade RUN per PM directive)
PM: "no need to rush it or cram it in. plan it properly. write issues. run the audit cascade skill (noting that initial audit missed this dimension) and then we can assess if any can be started tonight."
- **4 issues WRITTEN + on the board** (PM "write issues" = the per-instance board authorization): umbrella **#1237** (RADAR-ENTITY-SOURCES — all 4 types beta-ship, child of #1090) + **#1238** Document (Lead, unblocked) + **#1239** WorkItem (gated #1233 identity) + **#1240** People (gated PPM entity-model). Conversation = done under #1236. Cross-linked (umbrella children-list; notes on #1090 + #1236). Milestone MVP. (Bug caught+fixed: child bodies created EMPTY from a sed `#`-delimiter clash → re-set with `|` delimiter; verified 148-151 lines each + reference #1237.)
- **Audit-cascade ISSUE gate RUN** (fresh-eyes Explore subagent vs `feature.md` — the skill's create-vs-audit asymmetry): gaps in all 4 (Example UX, Success Metrics, standard STOP set, Related Docs, Notes; umbrella Testing Strategy) → **ALL fixed** (no self-granted N/A per skill rule) → re-verified conformant. Matrix: `dev/2026/06/14/radar-entity-sources-issue-phase-audit.md`.
- **THE MISS documented (PM-requested "noting initial audit missed this dimension")**: initial #1236 audit checked **conformance**, not **dependency-completeness** — accepted "richer types later, gated on #706" without verifying #706 was open (it's **CLOSED**, a discovery epic) or that the deferral had a durable home; Phase-0.5/0.6 contract verification was scoped to the conversation slice, not the 4-type capability. Root: conformance ≠ soundness; investigate-before-extending not applied to *product scope*. **Proposed audit-cascade amendment** (→ CIO/skill-owner): add a **Referent-Verification / Dependency-Completeness check** — trace each claimed capability to a concrete/verified backend or an OPEN tracked dep; deferrals must name a durable home.
- **All durable**: drafts + audit doc on main `2da2ade72`; issues #1237-1240 live.
- **Assessment → PM**: only **#1238 Document** is tonight-startable (unblocked); #1239/#1240 gated. Board Sprint placement (D1 vs MVP-backlog) deferred to PM. Awaiting go/no-go on starting Document tonight + Sprint placement.

## Fire 31 (PM conversation, ~22:4x PDT — board #1237-1240→D1; #1238 Document Phase-0 STOP — the gate did its job)
PM: "Yes, tackle document tonight. We can resume the rest tomorrow. Yes put #1237 to #1240 in D1."
- **Board op (PM-authorized)**: #1237/#1238/#1239/#1240 → Sprint=**D1** (verified field `PVTSSF_lAHOADE-8s4A-JwAzg2hWcg` + option `a1babe2b` "D1 - Beta design quality"; added each to project + set). ✓ all 4.
- **#1238 Document → Phase-0 contract verification → STOP (NO code written; gate did its job)**: `DocumentService` (`services/knowledge_graph/document_service.py`) = single **GLOBAL** ChromaDB `pm_knowledge` collection; every method filters by `analysis_timestamp` only; **NO user_id/owner anywhere** — not in methods, not stamped by the ingester (`ingestion.py`), not passed by any upload caller (none found in web/ or services/). → cannot honestly list "the user's documents"; returning global-per-user would **LEAK documents across users** (ADR-058 multi-tenancy; Radar route is per-user via get_current_user). **The STOP condition #1238 explicitly named has fired.** Reclassified #1238 → **BLOCKED** on a **document-store user-scoping prerequisite** (data-layer: owner on ingest metadata + upload path + backfill existing docs + scope ChromaDB queries — Arch/PPM-flavored, NOT a "small wrap"). Documented on #1238 + #1237.
- **Vindicates tonight's dependency-completeness lesson**: even the "unblocked, small" source had an unverified backend assumption (DocumentService user-scopable) → FALSE. The proposed audit-cascade Referent-Verification check would have caught this at issue-time, not build-time.
- **Net**: ALL 3 remaining sources now have hard prerequisites (Document = doc-store user-scoping; WorkItem = #1233 identity; People = PPM entity-model). **Nothing in the entity-source workstream is cleanly buildable tonight.** → reported to PM; recommend banking tonight (the planning + the caught blocker IS the value); options surfaced (carve the doc-store-scoping prerequisite; or pivot to F3 #1172 tokens — CXO-build-ready, different workstream — but PM said rest-tomorrow).

## Fire 32 (PM conversation, ~23:0x PDT — systemic user-auth-anchoring gap FLAGGED + F3 #1172 token-lint gate BUILT)
PM: "That's a definite stop. Flag it as a gap and also flag the need to investigate the extent of parallel gaps. How systematic is it?... Is there any unblocked work in D1? If so, tackle it tonight."
- **Flagged the systemic gap (PM-directed)**: filed **#1241** "ARCH-AUDIT: content not anchored to user auth — extent + remediation (multi-tenancy completeness)" (Arch-lens; documents = confirmed instance #1; deliverables = inventory every content store, classify user-anchored vs global, root-cause the *recurring* failure, propose a **canonical anchoring pattern + ADR**). **#1238 linked blocked-on-#1241.** **Arch memo** (cc CIO, PM) routing the lens-request → on main `21bc3fe32`. Captures PM's "not our first attempt… deep plumbing-out."
- **Unblocked D1 work = F3 #1172** (VERIFIED not assumed — applied the Document lesson): CXO spec (`dev/active/design-floor-component-specs-2026-06-14.md`) confirms F3 spec-complete + "build-first, fast"; Phase-0 confirms `web/static/css/tokens.css` exists, `.github/workflows/lint.yml` CI present, no hidden dep.
- **Built the F3 token-lint gate (TDD)**: `scripts/token_lint.py` + `tests/unit/test_token_lint.py` (**16 green**; commit `9baa53aeb`). Catches hex/rgb/hsl colors, spacing px (not 0/1/2px), border-radius literals, font-size/weight/line-height literals; allow-list (relative units, hairlines, unitless line-height, inline `/* token-lint-allow */`). **Inventory: 70 violations** — color 30 (23 bare-drift + **7 var(--token,hexfallback)**), type 25, spacing 12, radius 3. **Radius convergence effectively done** (duplicate `--radius-*` = 0 uses; `--border-radius-sm/md/lg` canonical).
