...
## Fire 26 (19:0x PDT — D1 scope expanded (+4 issues) + design-floor UNBLOCKED (CXO specs); re-plan)
PM moved 4 formerly-M5 issues into D1 (#1048/#1202/#1164/#1184) + nudged CXO → **CXO design-floor specs DELIVERED** (`dev/active/design-floor-component-specs-2026-06-14.md`; memo → read/ `78d0a185c`).
- **Design-floor #1170-1173 UNBLOCKED.** CXO order: **F3 #1172 (token-lint, build FIRST — locks the token vocab) → F2 #1171 (page-shell) → F1 #1170 (Dialog) → C1 #1173 (chat-page)**. F3/C1 build-ready; F1/F2 want a ~20-min primitives-sync (Dialog API shape `dialog.open({...})` + page-declaration mechanism — my call, CXO input).
- **⚠ KEY COHERENCE (CXO)**: F2 page-shell = the start-screen app-frame = #1090's home frame; **#1236's Radar = the L2 aside in that frame** → sequence so F2 + #1236 don't double-build the frame. Handling: #1236 Radar = a **frame-agnostic component** (fed `/api/v1/radar`) that F2 re-homes into its aside. (My Phase-1 DDD split already supports this — the domain is frame-agnostic.)
- **4 new triaged** (mostly Lead lane): **#1202** files-follow-on (artifact export/share/versions + tagging — substantive/flywheel); **#1164** privacy-toggle-stub (small bug; **OVERLAPS #1236's `history_sidebar.html`** → fold into #1236 — preserve + wire the toggle to #1021 `PATCH …/privacy`); **#1184** artifact-rename/format (#355 follow-on, small-med); **#1048** stage-visual (a **DESIGN QUESTION** → CXO decides, then maybe small Lead build).
- **Proposed sequence**: finish **#1236** (Phase 2-3, folding #1164) → **F3 #1172** → **F2 #1171** (re-homes #1236 Radar aside; primitives-sync) → **F1 #1170** (sync) → **C1 #1173** → **#1202 / #1184** → **#1048** (post CXO design decision). Non-quick-wins all full-flywheel + DDD + TDD.
- CONTINUING #1236 Phase 2 (build in flight; no puzzlers).
