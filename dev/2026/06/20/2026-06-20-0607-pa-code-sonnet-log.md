...
- Fire (~14:05 PT, SKUNK track) — PM confirmed skills-only wave already sent Thursday. MCP wave is next. Context: v0.1.1.mcpb was stale (old server.py, no connect tool). Dispatched coding agent to sync source→dist and repack. Commit `adec32d` on `mediajunkie/piper-morgan-skunkworks` origin/main:
  - **v0.1.2.mcpb**: `server.py` synced from poc/dinp source (byte-for-byte identical); manifest bumped 0.1.1→0.1.2; `connect` tool added to tools list; `piper_base_url` default changed to `https://alpha.pipermorgan.ai`; repacked at 34KB (~3KB larger than v0.1.1 — accounts for new server.py code)
  - **Next**: PM to AirDrop v0.1.2.mcpb to other Mac → double-click install → confirm 6 tools appear → run `connect` with alpha pw → test `ask-piper`. Watch for: uv availability on other Mac (manifest uses `"command": "uv"`, not bundled binary — failure here = useful signal); `anthropic_api_key` user_config field may confuse testers (flag for removal before wider send)
