...
### #358 Phase 1 DONE — FieldEncryptionService (TDD green)
`services/security/field_encryption.py`: AES-256-GCM (authenticated) + HKDF-SHA256 per-field subkeys + random 12-byte nonce + `DecryptionError` on any auth fail + key-safe `repr` + `from_env` (returns None when unset → keychain fallback for local dev; the hosted box MUST set the env). **9 security tests green** (`tests/unit/services/security/test_field_encryption.py`): round-trip, not-plaintext, tamper-detect, per-field isolation, nonce-uniqueness, wrong-key, short-key, no-leak-repr, unicode. `docs/security/key-management.md` written (rotation + #482 KMS path). Next: **Phase 2** (encrypted user-secret store: `encrypted_secret` column on user_api_keys + UserAPIKeyService integration).

### #358 Phase 2 DONE — encrypted user-secret store (TDD green) + a pre-existing fixture bug fixed
- Model: `user_api_keys.encrypted_secret` (Text, nullable, additive). Migration **`a358encsecret`** (down_revision a1273coretables; idempotent add_column) — **applied + validated on the test DB** (a1273coretables → a358encsecret).
- `UserAPIKeyService`: `__init__` takes a `FieldEncryptionService` (from_env → None ⇒ local-dev keychain-only); `store_user_key` writes `encrypted_secret` (ctx "user_api_keys.secret"); `retrieve_user_key` prefers decrypt → keychain fallback. Additive + None-safe (existing flow unchanged when no master key).
...
- **Phase 1 DONE** (`services/security/encrypted_types.py` + `tests/unit/services/security/test_encrypted_types_358b.py`): `EncryptedString` TypeDecorator, **13 tests green**. Marker-prefix `PMENC1:`; bind=encrypt+marker, result=marked?decrypt(raise-on-tamper):passthrough; None-safe; injected-or-fresh-`from_env` encryptor (no cache → env changes take effect, negligible vs DB round-trip); keyless→plaintext-passthrough on write (warn-once) but marked-value-without-key→**raise** (never a silent token). Per-context isolation + from_env path verified. Next: Phase 2 (apply to the 4 columns).
- **Phase 2 DONE** (`services/database/models.py` — 4 columns → `EncryptedString`; `tests/unit/services/security/test_content_encryption_358b.py`): `conversation_turns.user_message` + `assistant_response`, `artifacts.content`, `conversations.preview` now `EncryptedString(context=...)`. 3 P2 tests green (4-column wiring + turn/artifact round-trips: raw=ciphertext `PMENC1:`, ORM=plaintext). **No DDL** (impl=Text). **Regression CLEAN: 112 passed** (existing content-column tests + full security suite); existing tests stay green via no-key passthrough. (Pre-existing cosmetic `KeyRotationService` async-teardown warning, not from this change.) Next: Phase 3 (zero-downtime backfill).
- **Phase 3 DONE** (`scripts/backfill_encrypt_content_358b.py` + `tests/unit/services/security/test_backfill_encrypt_content_358b.py`): async zero-downtime backfill — raw-read unmarked rows → encrypt (MARKER+context) → raw-write; batched/idempotent/resumable (`NOT LIKE marker` advances the loop), **refuses without key**. 3 tests green (scope-guard on the 4 TARGETS, key-refusal `SystemExit`, idempotent-encrypt + mixed-state read + count-preserved). No DDL. **Prod backfill RUN against alpha is a separate ops step** (needs `ENCRYPTION_MASTER_KEY` set on the box). Next: Phase 4 (perf + close-out + 2 deferred follow-ups).
- **Phase 4 DONE — #358-B CODE-COMPLETE**: perf measured (~14.7µs/field decrypt, ~29µs/turn-row; sub-ms per realistic history read — DB round-trip dominates; HKDF subkey-derivation is the cost lever if ever needed). Deferred follow-ups filed: **#1305** (JSON/JSONB structured cols — queryability tradeoff), **#1306** (on-disk file content — storage-layer). Posted #358 evidence comment (A + B code-complete on main; remaining = ops: set `ENCRYPTION_MASTER_KEY` on the box + run backfill on next deploy). **#358 close is PM's call** (code-complete vs hold-for-deploy) — surfacing. #358-B all green: 13 (P1) + 3 (P2) + 3 (P3) = 19 new tests; 112 regression.

### Gate-removal-safety investigation (task #35) — DONE: CONDITIONAL GO
Findings: `dev/2026/06/20/gate-removal-safety-investigation.md`. **Key**: the app has its OWN global `AuthMiddleware` (JWT) gating all routes except a categorized exempt list (171 required-auth vs 3 optional `Depends`) → **removing Caddy does NOT open everything**. Exposure = the exempt list, mostly benign (health/static/oauth/docs/bootstrap; `dev_trust`+`dev_composting` 404 in prod; `/auth/register` has no handler).
**BLOCKER found** → **#1307**: `admin_compose` (`/api/v1/admin/compose`) is auth-exempt + **WRITABLE** (`POST /save`) + **not env-gated** → open writable admin UI in prod, protected only by Caddy. Filed (blocks #1162). Plus: **no rate-limiting anywhere** (SHOULD add before public). `/intent` unauthenticated is LLM-gated by BYO-key.
**Go/no-go: CONDITIONAL GO** — sound architecture; fix #1307 + add rate-limiting before removal. Surfacing to PM + Arch.

### RECONNECT review + pivot to #1232 (WS-5 connector protocol) — keystone contract SHIPPED
PM (6/20) asked why no RECONNECT issues have closed. Honest finding (**2 self-corrections**): (1) #1185+#358 were NOT a sidetrack — they're RECONNECT **Phase-0** (identity core + enabler) per scope §12; (2) #1230 is NOT the unblocked quick win — I misread it off its title; it + #1229/1231/1232/1233 all wait on ADR-070, and the one ADR-independent quick win (#1227) is already closed. **Real reason nothing's closing**: RECONNECT is foundation-gated (ADR-070 + #1185); the foundation doesn't produce WS closes. (Also: #998/admin_compose is Web's lane / website — PM clarified; #1307 fix = remove the misplaced product-app router, pending PM confirm.)
PM chose **option (2): activate the ADR-070 build.**
- Read ADR-070 v0.1 (Arch's; complete D1-D9 — Piper-as-consumer, MCP-server owns OAuth, DB-backed config, **Connector protocol D5**, tier migration D6, identity-first D8).
- Scoped #1232 to the buildable-now slice: the **contract** (protocol + 4 types + AST-guard + github structural proof); the real ports deferred per D8. Gameplan + audit (`dev/2026/06/20/1232-gameplan*.md`), gate-2 cleared.
- Mailed Arch the activation + 2 confirms (v0.1 stable? contract-now/ports-later split?) + Open-Q-4/5 consults (`66f4d8f54`).
- **P1 DONE+committed (`d400c733a`)**: `services/mcp/consumer/connector.py` — `Connector` protocol (connect/status/resolve/degrade) + ConnectResult/ConnectorStatus/ResolveResult/DegradationResponse/ResourceQuery + **9 tests**. **The keystone — WS-3 (`resolve`) + WS-4 (`degrade`) now have a contract to build to.**
- **P2 (AST-guard) + P3 (github proof) DONE**: `tests/unit/services/mcp/consumer/test_connector_contract_1232.py` (5 tests). AST-guard enforces the 4 methods on `IMPLEMENTS_CONNECTOR`-declaring adapters (declared-conformer-scoped so the 5 un-ported ones don't break the build); `github_adapter` declares + conforms (runtime `isinstance(Connector)` + AST). **14 #1232 tests total**; regression clean (68 mcp + 66 arch/query). **#1232 contract slice COMPLETE — WS-3 (`resolve`) + WS-4 (`degrade`) unblocked.** Result-type shapes are v1 pending Arch's Open-Q-4 review. Next: P4 close-out + #1232 disposition (contract done; the real ports deferred per D8).

### #1307 (admin_compose) DELETED + CLOSED + Arch gate-removal concur
- PM: "delete it." **Removed** admin_compose — router + 2 templates + `app.py` mount + the `EXEMPT_LOCALHOST_SCAFFOLD_PATHS` entry + the stale route-prefix test + 2 dangling refs (`0466fd09d`). App imports clean (233 routes; `/api/v1/admin/compose` gone); no dangling refs. **#1307 CLOSED**. Deliberate exception to comment-out-dead-code (PM-directed misplaced-feature removal, git-recoverable — belongs in Web's website repo).
- **Arch replied** (gate-removal concur memo): CONCUR AuthMiddleware-as-sole-gate; the load-bearing add = treat the auth-exempt list as a **security boundary** via an enforcement lint (every exempt route read-only / env-gated / justified, fail-closed) → makes #1307's class impossible. Rate-limiting = global ASGI fail-closed default + slowapi per-route + Redis. **GO on gate-removal once #1307[done] + the lint land.** Filed the lint = **#1308** (gate-removal prereq, M5).
- Discovered (regression sweep, NOT mine — my commits touch zero onboarding code): **#1309** — stale onboarding test (flow added GATHERING_REPOS; test still expects COMPLETE). Filed.
- **#1232 P4 close-out**: contract slice complete; evidence comment posted; kept OPEN (ports deferred per D8).
