...
- **NEXT (unblocked marquee): DRAFT ADR-071** (Lead-author, grounded in the audit + Arch's D1-D7 guidance) → loop Arch at v0.1 → then the consolidating refactor (doc-store first). F3 remainder = CXO-gated (now explicitly routed).
