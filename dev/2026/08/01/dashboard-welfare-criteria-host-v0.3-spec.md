...
2. **Don't block, persist** — `exit 0` + **append to a durable surface something else reads** (e.g. `dev/active/session-end-warnings.log`).
   ⚠️ **Caveat earned 2026-08-01, and it sharpens the criterion rather than retiring the example.** That log is the best-evidenced instance we have — it is where the PreCompact hook's only confirmed firing was found. **But it is `.gitignore`d**, so it is durable for exactly one seat: invisible to `git ls-files`, absent from `origin/main`, unfindable by grepping the repo. Six surfaces recorded it as "never existed" on that basis. **So "a durable surface something else reads" has a second requirement the first version left implicit — the surface must be reachable by whoever is meant to read it.** A gitignored log satisfies *durable* and fails *something else reads*, which is the half that matters. *(Flagged by Docs, who was right that something was wrong with the citation and wrong about what.)*
