...
- ~~Does the website repo have a worktree on Amber, or is it a plain shared checkout?~~ **ANSWERED 2026-07-29 by PM — this one is no longer a question, and it changes how you publish.** PM: *"all agents need to work in worktrees on this project at least."* So: **run your publishes from a per-agent worktree of `piper-morgan-website`, not from the shared checkout.** This does *not* change the publish target — PM ruled 7/28 that publishing lands on website `main` by design. Both hold at once, same shape as the product repo: **work in a worktree, push to `origin/main`.** Two things to verify on arrival: (a) that a website worktree exists for your lane, since provisioning currently creates one worktree per agent for `piper-morgan-product` only and roles spanning two repos need a second; (b) that it is 0 behind — the shared checkout was 4 behind when Web found this gap on 7/26. **Full reasoning in `decisions.log` (2026-07-29 ~07:15).** Every publish I ran worked directly in PM's shared website checkout; that is now the old way, and the change also removes a real hazard — that flow runs straight through any uncommitted PM WIP in that repo.
