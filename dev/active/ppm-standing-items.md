...
| 3 | **PDR-005 v0.5 → v1.0 path** | medium | **advancing** | partial | **EC-2 RESOLVED 6/3**: flag-back → Arch + CXO both qualifier-needed (genuine platform-forced examples: Slack threads, voice, file surfaces) → **PPM synthesized the unified "platform-affordance-bounded" qualifier** (zero-tolerance still binds capability *behavior*; *whether claimed at all* is conditionally-surfaced-per-host; invisible-by-default + honest-boundary-on-demand; Colleague-Test-verified) → re-circulated `memo-ppm-...ec2-qualifier-synthesized-recirculate-2026-06-03.md`. **Holding ~1-2 cycles for Lead's integration-constraint read + cohort no-objection**, then fold qualifier into PDR-005 (EC-2 + paired AC-1 surface-presence-detection mechanism + Q7 capability-claim-map note) → take v1.0 to PM. Remaining v1.0 gates: Comms external-language frame + PM ratification. CT v2.5 sub-dim deferrable to v1.1. |
