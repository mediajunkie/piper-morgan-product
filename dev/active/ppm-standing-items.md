...
| 9 | **#1158 summarize floor-vs-handler product position** | low | open | YES | CXO/Lead consult (CC PPM): summarize stays on conversational floor by default; promote to structured handler ONLY if PPM product-spec surfaces a persistent/exportable-artifact need. PPM input = the source/output spec deciding floor-vs-handler. **Folds into the design-leadership "not being bad / being good" working session** (CXO↔Lead↔PM arc, kicked off 6/6; frame `design-leadership-framing-web-ui-2026-06-03.md`); non-urgent. |
| 10 | **#1166 Type-2-Dreaming roadmap-fit + PDR** | medium | open | YES | CXO filed #1166 6/6 (PM-flagged parked decision; methodology-27 concept absorbed, operational/roadmap call deferred + never opened). **PPM lens = roadmap-fit + priority: does it belong on the roadmap, at what depth (explore / discovery-spike / build), and when? PPM owns the resulting PDR if yes.** Three-way (PPM roadmap + Arch design-surface + CXO user-facing). Non-urgent (no deadline) but a real distinctive-PPM item (roadmap stewardship + PDR craft) — **next focused-fire candidate after Ship #046**. Acceptance: three-way input → roadmap-fit call → open PDR or record explicit-defer + reopen-trigger. | **Next PPM action**: on Comms frame landing, fold it + take PDR-005 v1.0 to PM. **CT-version reconcile DONE 6/3**: CXO confirmed canonical = v2.3.2 (no committed v2.4 — was a May-10 proposal that never landed). Reconciled all my citations: roadmap v18 §Methodology + PDR-005 v0.6 (5 citations) → v2.3.2; Layer B cites by file. v2.5 (proposal) references left as-is. |
