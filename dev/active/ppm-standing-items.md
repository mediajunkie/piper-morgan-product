...
|---|---|---|---|---|---|
| 1 | **#1128 ROADMAP-REFRESH** | medium | **RATIFIED 6/3 — PPM-complete; Docs swapping** | done (PPM) | roadmap.md stale. delta-assessment (5/28) → v17 draft (5/30, `00cee8d47`, distributed `15f8a05ae`) → **v18 draft (6/2, `roadmap-v18-draft-2026-06-02.md`): PA §M5/BYOC review ABSORBED** (Daedalus referent explicit; Outcomes date corrected; PoC PASSED-5/19 sharpened; Janus meta-coordinator line). **CIO §Methodology review ABSORBED 6/3** (corpus 29→37, patterns 62→74, methodology-as-operational-capability prose). **Both section reviews now in (PA + CIO) → v18 READY FOR PM RATIFICATION** (escalated to attention doc 6/3). Comms external-language frame = parallel polish (v18.1-able), not gating internal canonical. **Packaging correction folded 6/3** (PA relay of PM 6/1): plugin is the canonical Anthropic package (not MCPB) — §Distribution build sequence + §Timeline corrected; CT reconciled to v2.3.2. v18 still ratification-ready, now packaging-correct. **v18 RATIFIED by PM 6/3 evening** (both flagged confirmations ratified: Comms frame non-gating; ratification on the packaging-correct version). **Docs is doing the canonical swap** (v18 draft → `roadmap.md` + archive v16.0), per PA relay. PPM substantive work on #1128 = COMPLETE; #1128 closes on Docs swap-confirmation. (HTML canonical re-render available on PM request — not auto-done.) |
