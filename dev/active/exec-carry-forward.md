...
### 🔄 RE-OPENED 6/27 — github-mcp provisioning A→C
Morning: PM ratified **A (hosted-OAuth)**, relayed (`1ecb66eda`). **Then REVERSED**: Lead found A's hosted endpoint **requires every tester to have Copilot** → collides w/ PM's constraint (tester-Copilot = non-starter for alpha). A BLOCKED. B (PAT) still rejected. **Lead's new Option C: self-host `github-mcp-server` + per-user OAuth** (Droplet/Mac Mini; per-request OAuth via Auth header; no Copilot; ~zero cost; testers authorize *our* OAuth App). Lead recommends C, memo'd Arch cc PM/exec (`...redecide-A-to-C-selfhosted`). **Decision now ARCH's**: is OAuth-token custody (Piper holds/forwards scoped revocable #358-encrypted OAuth tokens — better than B's PAT, more than A) D3-acceptable? Arch to re-rule A→C. PM's constraint already drove it; no further PM input needed unless PM wants to weigh custody. **Watching for Arch's re-ruling.**

### 🔄 IN RATIFICATION — inbox-proxy (tally)
ACKs in (6/27): **comms, docs, host, ppm = 4/10.** Awaiting: lead, arch, cio, cxo, web, pa. Backstop Mon 6/29.

### ▶ Ship #049 workstream — COLLECTING
Reports received: **comms, host = 2/6.** Awaiting: cio, cxo, ppm, arch. Tue 6/30 last-call → synthesize → Wed 7/1.
