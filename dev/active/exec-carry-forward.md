...
> **★ 6/14 ~19:00 PM UPDATE (current — supersedes stale bullets below).** Resumed after a **~29.5h Gap-C dormancy** (session died 6/13 ~10:30 → PM manual resume 6/14 15:56; 6/13 retroactively closed). Cron expr `32 6,9,12,15,18,21` (id rotates per-fire; if CronList zero, re-arm). Current state:
> - **Ship #047 → READY FOR PM VOICE-PASS** (Comms editorial pass done; accuracy fix applied = six-of-nine cumulative). `docs/public/comms/drafts/weekly-ship-047-draft-2026-06-12.md` → PM voice-pass → Docs publish Wed. 6 lenses still held in inbox.
> - **Role-portfolio framework → RATIFIED by PM 6/14.** HOST notified; next phase = cohort self-authors → HOST reviews → Exec coordinates. Awaiting HOST's sequencing preference, then Exec drafts the per-role kickoff.
> - **BYO-colleague:** Q1 + Q2 PM-ratified; Q3 team-authored (HOST/CIO/Comms) pending ratification (no longer 3 open questions).
> - **Gap-C cure ≠ scheduled-tasks** (= the persona-fork PM vetoed 6/14). Real cure = **wake-this-session (`ScheduleWakeup`)**, CIO/Docs designing + UNVERIFIED. Exec NOT migrating; queued to adopt; sent CIO the ~29.5h evidence (`8dd266bf3`). CronCreate meanwhile. **PM-pending: drive-vs-coordinate (a/b)** on whether Exec helps drive the design.
> - **Attention board = TWO surfaces**: inline `show_widget` + static `.html` (`dev/active/exec-attention-board.html`) auto-surfacing in PM's preview pane (preview-pane technique RESOLVED, PA+CIO). Rendered/corrected several times 6/14 — live-state verification caught stale Routines + BYO-colleague items (lesson: never render from a stale source without the verification pass).
> - m-41 Proven; coverage gaps both concurred (CXO/Web + PPM/PA, #048); BYOC Phase 2 cohort-ratified 8/9 (PA synthesizes for PM).
>
