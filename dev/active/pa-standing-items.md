...
| # | Item | Filed | Notes |
|---|---|---|---|
| 2 | **PM wants to discuss the architecture diagram** — ✅ **REDRAWN 2026-08-01**, PM-requested, still awaiting a time | 2026-07-26 | PM 7/26: *"I've been meaning to discuss the architecture diagram you (earlier-PA) put together."* ⚠️ **The July artifact is UNREACHABLE** — it was created under the designinproduct.com account and artifacts are account-scoped like memory, so it did not survive the switch. **Redrawn 8/1 rather than re-shared**, because four things had moved under it: the Web client is largely deleted by the pivot; the capability split needs a *surface* column (web chat has no hooks); caller identity is now a boundary the whole diagram crosses; and the return arrow matters — the client's model composes what the user reads. **Source lives in git first** (`dev/active/pdr-006-architecture-2026-08-01.html`) precisely because the last one vanished with an account. Artifact: https://claude.ai/code/artifact/92ce8bc9-23d4-4590-b121-dacc0ab72e17 |
