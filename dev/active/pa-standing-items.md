...
| # | Item | Filed | Notes |
|---|---|---|---|
| 1 | **ALPHA_FEATURE_GUIDE refresh — draft landed, blocked on live verification** | 2026-08-13 | Draft at `dev/active/draft-alpha-feature-guide-v0.8.11-for-pa-review.md` (85 tagged claims + 11-item "PA: please check" list). **Real blocker found**: no Chrome/Chromium exists on this Amber worktree, so PA cannot actually click through `alpha.pipermorgan.ai` as the split assumed. Did a code-level pass on `production` branch instead (different, weaker layer — resolves *what's shipped*, not *how it renders*). **Resolved at code level**: GitHub Settings shows OAuth first (recommended, #1317/ADR-070 C) + PAT fallback — settles Docs's flagged RN-0.8.9-vs-briefing contradiction; Calendar OAuth flow confirmed present; Notion connection method is a pasted API key (not OAuth); "Test All" health dashboard confirmed absent from app code; `/standup` page route still renders despite "legacy" framing. **Everything UI/interaction-behavior (chat tone, drag-drop, autosave, honesty claims) still needs an actual browser session** — proposed PM or another seat with working Chrome do a short, now-targeted click-through. Reply: `mailboxes/pa/sent/reply-pa-to-docs-...-blocked-on-live-verification-...-2026-08-13.md`. **Pending Docs/PM response on how to close the live-verification gap.** |
