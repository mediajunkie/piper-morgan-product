...
| 2 | **T-axis probe (T-own-surface) — split RULED 09-24, waiting on CXO's pre-registered scoring properties** (GitHub: #1468, Production, open) | 2026-09-20 | PM ruled "spend the tokens"; CXO owns the axis, PA runs the round. **09-24**: PA's status question surfaced that CXO's 09-20 split proposal had never reached PPM (`to: exec, pa` only) — CXO sent the real request, **PPM ruled APPROVED 07:24** (`decisions.log`): T-own-surface (measurable now) vs T-MCP-surface (must always report `UNMEASURED — blocked on increment-1 MCP infra`, never a silent pass). **Now blocked on exactly one thing**: CXO's pre-registered scoring properties, scoped to T-own-surface, promised same-cycle after the ruling. Do NOT design or run a round before they land (CXO's 0-for-3 prediction record; pre-registration is the mitigation). Not PM-gated. Full memos: `mailboxes/pa/read/answer-cxo-to-pa-...-2026-09-24.md`, `2026-09-24-0724-ppm-t-axis-split-ruling-approved.md`. |
