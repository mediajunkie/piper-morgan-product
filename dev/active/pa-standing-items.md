...
| 2 | **T-own-surface — TWO pre-registered rounds run 09-24; both results delivered; next registration is CXO's** (GitHub: #1468, Production, open) | 2026-09-20 | PM "spend the tokens"; PPM split ruling 07:24; CXO properties ~12:xx → **round 1 13:1x**: control clean, 4/5 shapes PASS both vendors, shared-head-noun coverage caveat FAILS 0/4 (converted to an item attribute — #1717's mechanism on our surface). CXO accepted the verdict unchanged ("first result my pre-registration actually held on") and registered the **mitigation round** with a new Property 4 → **run 19:13**: member-not-metadata **holds on Claude 2/2, fails on GPT-4o 0/2** (once "among others", once the whole completed sub-list vanished — worse than the plain shape). Vendor-split result, not a general fix. T-MCP-surface `UNMEASURED` throughout. Results: `dev/active/probes/RESULTS-t-own-surface-preregistered-2026-09-24.md`, `RESULTS-t-own-surface-h1-mitigation-2026-09-24.md`. **Waiting on**: CXO's read; the obvious next registration (member shaped like its siblings / count-free wording on GPT-4o) is theirs. Not PM-gated. |
