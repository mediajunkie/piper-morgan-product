...
| # | Item | Filed | Notes |
|---|---|---|---|
| 1 | **BYOC — active focus. Phase A TWO PASSES run (incl. independent-author control); Phase B recommendation sent, Exec agrees, awaiting PM's decision** | 2026-07-26, converged 08-18, restarted 09-22 | **Current PM-attention state lives in `pa-carry-forward.md`, not here.** **Phase B, 09-22**: recommendation sent (reuse this week's PM-granted-Pard-Fly-access pattern over Arch-supervising-a-fresh-instance); Exec replied agreeing with the reasoning, relayed to PM (`mailboxes/pa/read/relay-exec-to-pm-cc-pa-pard-mcp-recommendation-agree-2026-09-22.md`) — still PM's call, not yet ruled. **Phase A**: naming-test pass 1 (13:12, `dev/active/probes/RESULTS-naming-test-first-pass-2026-09-22.md`) found situation-shaped 12/12, object-shaped 10/12, but flagged a same-author disambiguation confound. **Pass 2 (19:13, independent-author control, `dev/active/probes/RESULTS-naming-test-control-2026-09-22.md`)** — run once PM's usage-directive lifted the cost-caution that had deferred it: a fresh isolated author's object descriptions fixed one failure (confirming the confound) but not the other (`list_projects` vs. `attention_query`, persisted even under a good independent description) — object rose to 11/12, situation stayed 12/12. **Net finding, honestly uncertain**: real but small (1-point) residual gap, could be a genuine naming effect or one hard utterance — a targeted follow-up (vary just that utterance) is named, not executed. Full plan: `dev/active/byoc-parallel-work-plan-2026-09-15.md`; readiness checklist: `dev/active/byoc-hosted-alpha-readiness-checklist-2026-09-15.md`. |
...
| 3 | **Usage-correlation model — spec+dispatch approved, awaiting seat→account mapping from PM; Anthropic-dashboard avenue CLOSED** | 2026-09-20 | PM ruled live 09-22: build Lead's `usage-per-account-capture-2026-09-19.md` proposal via an outcome-oriented spec + `prog` subagent dispatch, not diverting Lead. Two blocking unknowns per Lead's own doc are PM's/Pard's knowledge, not Lead's: **(1) seat→account mapping — asked PM directly, awaiting answer.** **(2) where the usage number is readable / can Dispatch read it — sent to Pard 09-22, awaiting answer** (`mailboxes/pard/inbox/question-pa-to-pard-cc-pm-exec-where-is-usage-readable-and-can-dispatch-read-it-2026-09-22.md`). **Separately, PM's Anthropic-dashboard tip investigated and CLOSED 09-22**: both accounts are personal Max x20, not Organization/Team/Enterprise, which rules out the Admin API and Claude Code's team-analytics dashboard — the two surfaces that could have beaten Lead's proposal outright. No change to the plan: Lead's proposal and Exec's transcript-parsing method both read local data already, unaffected by account type. Full writeup: `dev/active/anthropic-usage-data-research-2026-09-22.md`. **Next step once (1)+(2) land**: write the spec, dispatch to `prog` at Sonnet tier. **PM explicit instruction 09-22: focus shifts to BYOC now** — advance what's ready, batch what needs PM's attention through Exec's rollup rather than ad hoc memos; this item continues in the background whenever (1)/(2) answers arrive, not the active focus. |
