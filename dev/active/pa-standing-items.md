...
| 2 | **T-own-surface — THREE pre-registered rounds run 09-24; all delivered; count hypothesis not supported** (GitHub: #1468, Production, open) | 2026-09-20 | PM "spend the tokens"; PPM split ruling 07:24 → **round 1 13:1x**: 4/5 shapes PASS both vendors; shared-head-noun coverage caveat FAILS 0/4 as metadata. → **round 2 19:13** (member-not-metadata, v0.6 mechanism): **Claude 2/2 PASS, GPT-4o 0/2 FAIL** (1 compression + 1 whole-sublist drop) — vendor split. → **round 3 22:13** (CXO isolates wording, drops the count, holds shape constant): **Claude unchanged 2/2, GPT-4o unchanged 0/2 but now BOTH reps drop the whole sub-list** — count-form hypothesis not supported; PA offered a simpler reading (GPT-4o doesn't reliably carry a note-only appended member on this shape, count-independent) as interpretation, not a scored finding. T-MCP-surface `UNMEASURED` throughout all three rounds. Results: `dev/active/probes/RESULTS-t-own-surface-preregistered-2026-09-24.md`, `-h1-mitigation-2026-09-24.md`, `-h1-round3-2026-09-24.md`. **Waiting on**: CXO's read; next registration (shape-with-id variant) is theirs. Not PM-gated. |
