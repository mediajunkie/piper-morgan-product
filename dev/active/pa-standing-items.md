...
| 3 | **Usage-correlation model — Q2 resolved, Q1 still pending; first empirical pass done** | 2026-09-20 | PM (via Exec): *"rather than handwaving... build a richer model. Also, research what's out there."* Same method as T1. **Prior-art pass** (`dev/active/usage-correlation-model-prior-art-2026-09-20.md`, two addenda). **Q1 — calibration shape, STILL PENDING**: Lead's `usage-per-account-capture-2026-09-19.md` proposes the calibration mechanism for the engagement/proxy axis, zero rows captured. Question sent to Exec/PM 09-20 19:21, no answer yet. **Q2 — RESOLVED 09-21 13:12**: Exec agreed to keep the two threads separate (their audit is cost/context-efficiency, PA's is usage/engagement), offered `scripts/usage-audit.py` as a reusable input, explicitly said build the correlation layer now rather than wait (`mailboxes/pa/inbox/ack-exec-to-pa-cc-pm-agree-option-2-2026-09-21.md`). **First empirical pass done same fire** (`dev/active/usage-correlation-model-first-pass-2026-09-21.md`): commits vs. weighted-token-share across all 11 seats, Pearson r=0.435, Spearman rho=0.664 — moderate/exploratory, explicitly uncalibrated (proxy-vs-proxy, not proxy-vs-PM's-ground-truth). Exec (most commits, mid-pack tokens) and Lead/Docs (fewest commits per token) are the sharpest outliers. **Still waiting on Q1** before this can become a calibrated model rather than an internally-consistent exercise. |
