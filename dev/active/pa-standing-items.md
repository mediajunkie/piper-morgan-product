...
| # | Item | Filed | Notes |
|---|---|---|---|
| 1 | **BYOC — active focus. Phase A FOUR PASSES run; Phase B APPROVED by PM 09-23, execution notice delivered directly to Pard** | 2026-07-26, converged 08-18, restarted 09-22 | **Phase B, APPROVED 09-23**: PM approved the recommendation directly in conversation (reuse the PM-granted-Pard-Fly-access pattern). Notice first sent to Exec for relay, then **PM instructed direct delivery** — re-sent straight into Pard's real inbox (`mediajunkie` commit `26f4c03`, pushed); decisions.log entry confirmed landed by Exec — nothing further needed from PA; watch for Pard/PM to actually execute. **Phase A, pass 3 (targeted follow-up, PM-approved "Please proceed!" 09-23)**: varied the utterance for the one persisting failure — 2 unambiguous phrasings resolved correctly under BOTH naming schemes, the one genuinely ambiguous phrasing failed under object-shaped a THIRD time (new utterance) while situation-shaped succeeded again. **Phase A, pass 4 (09-23, second pair, generalization check, PA-initiated per "continue working on byoc")**: added `create_reminder`/`create_todo` (a time-cue ambiguity, verified against the real handler, not the first pair's purpose-ambiguity) — **6/6 under BOTH naming schemes, no difference.** **Net across all 4 passes**: the disambiguation benefit is real and replicated for purpose-shaped ambiguity (list_projects/attention_query) but does NOT generalize to every ambiguous pair — working hypothesis is it helps specifically where the distinguishing signal is hard to compress into a short object-shaped description, not where a concrete feature (like "time-anchored") can just be named directly. Full writeup: `dev/active/probes/RESULTS-naming-test-secondpair-2026-09-23.md`. Passes 1-3: `dev/active/probes/RESULTS-naming-test-first-pass-2026-09-22.md`, `-control-2026-09-22.md`, `-followup-2026-09-23.md`. Full plan: `dev/active/byoc-parallel-work-plan-2026-09-15.md`; readiness checklist: `dev/active/byoc-hosted-alpha-readiness-checklist-2026-09-15.md` (re-verified current 09-23 against #1462/#1458 live via `gh api`). |
| 2 | **T-axis probe execution — PM ruled "spend the tokens," CXO owns the axis, PA runs it** (GitHub: #1468, Production milestone, open — surfaced by the new criteria line 09-23) | 2026-09-20 | PM ruled (relayed by Exec) to spend tokens on the T-axis probe now. CXO (axis owner, no harness on their own seat) handed execution to PA directly: *"PA — I'm not handing you a deadline (there isn't one — the framed urgency was CXO's own now-retracted error) and I'm not specifying your round design."* **Blocked on CXO sending pre-registered scoring properties "this cycle"** — do not design or run a round before that lands (CXO's own prediction record on this instrument is 0-for-3, and pre-registration is the stated mitigation). Separately: CXO is proposing to PM/PPM a split of the T axis into T-own-surface (measurable now, tokens can close it) vs. T-MCP-surface (blocked on increment-1 infra, stays honestly unmeasured) — not PA's call, just context for why "spend now" doesn't necessarily mean the axis fully closes. Full memo: `mailboxes/pa/inbox/response-cxo-to-exec-pa-cc-pm-t-axis-spending-the-tokens-what-it-can-and-cannot-buy-2026-09-20.md`. |
| 3 | **Usage-per-account capture — UNBLOCKED 09-23, IN BUILD (#1862, prog/Sonnet dispatched)** | 2026-09-20 | PM ruled 09-22 (re-confirmed 09-23): build Lead's proposal via PA-written spec + `prog` subagent, not Lead. **Both unknowns closed by Pard's one answer, 09-23 11:2x** (direct delivery to `mediajunkie` worked — reply in ~1h): **(1) seat→account = function of `CLAUDE_CONFIG_DIR`**, all 11 PM seats → `~/.claude-pm` → pipermorgan.ai account (week resets Thu 22:00 PT); Pard/Janus/small projects → `~/.claude` → designinproduct.com. The question I'd routed to PM is moot. **(2) usage IS script-readable** — `GET api.anthropic.com/api/oauth/usage` (unpublished, same call the CLI's `/usage` makes), reader written+tested: `mediajunkie/scripts/usage-read.sh`. Spec written (`dev/active/usage-per-account-capture-build-spec-2026-09-23.md`, 5 deliverables), issue #1862 filed, subagent dispatched at Sonnet 13:1x. **Deviation I imposed**: script does NOT self-commit in this build (TODO marker) — review append logic first. **After it lands**: review → explicit-path stage → commit → mail Pard the crontab line (external driver; install is host-level, not ours). Then the correlation model (`usage-correlation-model-overview-2026-09-22.md`) becomes calibratable once a few real rows exist. Anthropic-dashboard avenue stays closed (personal Max x20). |
