...
| 3 | **Usage-correlation model — PM tasking, no deadline, prior-art pass DONE** | 2026-09-20 | PM (via Exec): *"rather than handwaving... build a richer model. Also, research what's out there."* Same method as T1. **Prior-art pass complete this fire** (`dev/active/usage-correlation-model-prior-art-2026-09-20.md`): four converging fields (SW-productivity SPACE framework, errors-in-variables/regression-calibration statistics, LLM-fleet cost observability tooling, North-Star/leading-indicator product analytics) all say the same thing — a proxy-only model with zero ground-truth readings is uncalibrated and potentially biased, not just weaker. **The load-bearing finding is internal**: Lead's `usage-per-account-capture-2026-09-19.md` (PM-reaffirmed 09-15) already proposes the exact calibration mechanism needed — a daily manual capture of PM's real usage reading — and **it has zero rows captured, unimplemented** (`dev/heartbeats/usage-per-account.tsv` doesn't exist, no `decisions.log` ratification). **Next step, not yet done**: draft the calibration-shape question to PM, folding in Lead's proposal's status rather than treating it as a separate thread. Model design itself (dimensions, functional form) also not yet started — deliberately, per the prior-art finding that building it before any paired readings exist would be an uncalibrated exercise. |
