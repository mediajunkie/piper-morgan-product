...
| 3 | **Usage-correlation model — PM answered Q1 in conversation 09-22: BUILD IT, via spec+subagent, not Lead** | 2026-09-20 | PM (via Exec): *"rather than handwaving... build a richer model. Also, research what's out there."* Prior-art + first empirical pass done (`dev/active/usage-correlation-model-prior-art-2026-09-20.md`, `dev/active/usage-correlation-model-first-pass-2026-09-21.md`, consolidated in `dev/active/usage-correlation-model-overview-2026-09-22.md`). **PM ruled live 09-22 (direct conversation, not mail)**: yes, build Lead's `usage-per-account-capture-2026-09-19.md` proposal — but via an outcome-oriented spec + `prog` subagent dispatch, explicitly NOT diverting Lead's attention. **Re-read Lead's proposal and found the two blocking unknowns are named in Lead's own doc as PM's/Pard's knowledge, not Lead's**: (1) seat→account mapping, (2) where the authoritative usage number is readable + whether Dispatch can read it. **Asked PM directly for (1) in the same conversation — awaiting answer.** (2) needs routing to Pard, not yet sent. **Next step once (1) lands**: write the outcome-oriented spec myself (no further Lead involvement needed) and dispatch a `prog` subagent, Sonnet tier (bounded, well-specified build — TSV writer, seat↔account companion block, wiring into freeze-watchdog + Exec's rollups as readers), matching the same PA-doesn't-touch-Lead's-queue pattern BYOC's Phase C already established. |
