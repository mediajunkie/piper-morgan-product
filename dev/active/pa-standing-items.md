...
| 3 | **Usage-correlation model — PM tasking, no deadline, prior art first** | 2026-09-20 | PM (via Exec): *"rather than handwaving... build a richer model. Also, research what's out there."* Same method as T1 (read outside sources honestly, state what transfers). **Hard constraint Exec flagged**: the only ground truth is PM's usage dashboard, which no agent can see — everything measurable from inside (commits, session-log growth, mail volume, fire counts, dispatch tier) is a proxy, and a model built purely from proxies correlating with each other proves nothing about the thing they're proxying for. **Design implication**: agree a calibration shape with PM (periodic real-usage readings to anchor against) before building the model itself, not after. Dimensions Exec offered (not prescribed): commits/role/day, session-log growth, mail volume, scheduled-vs-substantive fires, sub-agent dispatch tier (least-studied, may matter most), issue closures, conversation turns. **No deadline** — "a well-grounded answer in a week" is fine, and "the proxies can't support this without PM's numbers" is itself a complete, useful answer. Started 16:01 fire: prior-art research pass in progress. |
