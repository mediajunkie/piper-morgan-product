...
| # | Item | Filed | Notes |
|---|---|---|---|
| 1 | **BYOC/MCP — PM handed PA full ownership of the MCP testing program 09-26 (PM verbatim: "let's let Piper Alpha drive the MCP testing program as part of skunkworks and free you up to work on MVP critical-path epics"). mcp.pipermorgan.ai units 0–2 LIVE.** | 2026-07-26, converged 08-18, restarted 09-22, ownership handoff 09-26 | **Live now** (release v5, `65740438ae`): `/health` 200; identity (unit 1) fail-closed, hash-only tokens, garbage bearer refuses via real lookup; resources (unit 2, no tools/prompts) — `piper://me/profile`, `.../colleague-model`, `.../github/issues`, honest-empty shapes. 92/92 tests. Runbook: `docs/internal/architecture/current/mcp/server-README.md`. Mint: `scripts/mint_mcp_token.sh` (treat like an invite token, never the repo). **Unit 4 (OAuth AS) decision made 09-26**: Lead builds it as the last bounded lane (~1 day, identity-boundary-adjacent to what Lead already verified), then hands off in full — Arch's technical case, PA ratified, with an explicit required test (minted token binds the SAME identity through the whole `authorize`→`exchange_authorization_code` flow, not assumed). Memo: `mailboxes/pa/sent/decision-pa-to-lead-arch-cc-exec-pm-unit4-oauth-yours-review-condition-ratified-2026-09-26.md`. **PA's next owed action**: draft the tester-facing named-gaps document before PM (tester #1) first connects — #1458 (cross-caller isolation, OPEN, safe here since exactly one caller), T-MCP-surface (`UNMEASURED`, this build is what unblocks measuring it), #1510 store near-empty for most users. Not written yet — do it once unit 4 lands and a connection is imminent, not before it's needed. **Superseded/closed by this handoff, preserved for history not re-litigated**: Phase A naming-test (4 passes, `dev/active/probes/RESULTS-naming-test-*.md`), Phase B DNS/TLS (approved+executed 09-23). readiness checklist: `dev/active/byoc-hosted-alpha-readiness-checklist-2026-09-15.md` (now stale against this handoff — next touch should rewrite it around the live-units reality, not the pre-deploy planning framing). |
