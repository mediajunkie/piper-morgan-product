...
- **Worktree collision** (`mystifying-lumiere-8bebd3`, shared with Exec, confirmed via reflog, real data loss already happened once via a *different, now-resolved* bug that got conflated with this one before being corrected). Detection fix shipped (`duty-cycle-tick` v1.14, Step 2a). Root cause (harness-level provisioning) still unfixed. **Next action**: this specific directory stops being CIO's problem once this migration completes, but it doesn't stop being *a* problem — check with Exec/Docs whether it's been resolved by the time you read this, and if not, it's still worth a PM decision (end the colliding sessions) rather than continued careful workarounds indefinitely.
...
- **Read the memory export first, deliberately — it won't surface itself.** `dev/active/cio-memory-export-2026-07-24.md` is a full verbatim export of all 162 memory files from the designinproduct.com account, built because a different Anthropic account has zero access to the old account's native Claude Code memory, regardless of device or repo path. This file has the *content*; it does not restore native per-memory retrieval behavior. Reading it is a first-orientation action you need to take explicitly, not something that happens ambiently the way ordinary memory does.
- **Three different portability boundaries, not one.** Memory is account-scoped (needs the export above). A live watchdog/launchd job is device-scoped (doesn't exist on Amber as of this handoff — confirm with Pard, don't assume). Skills/scripts/docs/session logs are repo-scoped and already portable via git — no action needed there.
- **The worktree model may not translate directly.** Everything in this handoff assumes Claude Desktop's ephemeral-auto-worktree-per-session model (Model B). Amber's setup (per `amber-agent.sh`) runs a persistent tmux session with Claude Code launched directly in the repo directory — closer to a long-lived CLI session than a fresh-worktree-per-fire pattern. Whether `duty-cycle-tick`'s Step 2 sync procedure (and the new Step 2a collision check) needs adaptation for that shape is an open question — work it out early rather than assume the skill applies unchanged.
- **Verify git commit identity on the new account** before assuming it's unchanged. This repo currently has every role committing under one shared `mediajunkie` identity (attribution via message prefix, not git author field) — confirm that's still true from the pipermorgan.ai account rather than assume.
