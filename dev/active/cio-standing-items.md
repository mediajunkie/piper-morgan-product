...
| 12i | **Worktree-path consistency convention** — methodology codification routing to Docs | May 11 | Per Pattern-068 filing: the path-fragmentation child (P-17) was caught only because the next session noticed cross-tree diff. Codification ask routed to Docs for inclusion in `branch-worktree-mailbox-discipline.md`. ~30 min Docs work. Surface area for fragmentation drift continues until codified. |
| 12j | **Possible Lead-Dev shell wrapper / hook for cross-tree edit detection** | May 11 | Per Pattern-068 filing: Lead Dev assessment ask for tooling that warns when an agent is editing files outside its current working-directory's git tree. Cost-benefit unevaluated. Routed to Lead Dev for feasibility read; no urgency, can sit until Lead Dev has bandwidth. Convention (12i) is the higher-leverage first step regardless. |
...
| 12m | **methodology-27 Type 2 Dreaming (Anxiety Dreams) entry** — drafting Mon–Tue May 18–19 | May 15 | Per PA May 12 Anthropic Dreams Phase 3 routing + CEO May 12 ratification of "claim Type 2 publicly." Methodology-core entry first; PDR deferred. Citation strategy: Revonsuo's Threat Simulation Theory as primary academic ground; Matthew Walker (UC Berkeley *Why We Sleep*) as targeted-search candidate for the Bay Area radio researcher CEO mentioned. Cross-pollination distribution to Janus / Klatch / OpenLaws during Ship #043 publication week (May 20). Cadence-committed in CIO disposition memo May 15. ~60–90 min focused work unit. |
