...
| 7v | **PM's work-queue ruling implementation** (carried + mail + newly-observed GitHub issues, idle only when all three empty) + retiring "next fire"/`## Fire N` vocabulary from `duty-cycle-tick` SKILL.md | Sept 11 | **CIO's call, per Exec's explicit routing.** Not started this fire — deliberately deferred to a fresh session/compaction (named trigger) rather than rushed in the tail of an already-dense fire that also handled a real data-loss incident (see resolved-items below). Exec's concrete proposal: retire "next fire" from agent vocabulary, stop making `## Fire N` the session-log organizing unit (log by work unit instead), leave the anti-bite-sizing prose as-is. CXO already has a concrete per-role criteria-line proposal for herself (`label:UX state:open`, denominator 3) as a worked example of the pattern to generalize. |
