...
| 8a | **HOST 360 v0.3 questionnaire draft review** | May 24 | CIO committed (memo 2026-05-24) to review HOST's v0.3 draft before fielding. HOST will share draft ~May 26-27 (target Wed May 27 latest). Cycle-experience module per shape 2; CIO offered five-question starter shape. Trigger: HOST inbound delivery of v0.3 draft. Capture-here per Pattern-074-interaction-finding (autogenerated MANIFEST clobbers inline annotations; standing-items tracker = task-list surface per v0.5). |
...
| 12ee | **MEM-* cluster (#972-#975) CIO Janus coordination touch-point** | May 17 | Per Lead Dev MEM-cluster Phase 0 audit memo May 17 (CC). CIO question Q4 = Janus field-name coordination on #972 (MEM-TEMPORAL: `valid_from` / `ended` frontmatter fields). Two options: align with Janus's chosen names OR ship our spec first and let Janus adopt. Deferable until PM ratifies cluster sequencing (PM-primary decision). CIO action ~30 min once PM ratifies. |
...
| MEM-975: implement-script (slot 12pp) | **Implement delta-generator script** per design doc | May 25 (queued; deferred pending drain-until-IDLE design fix) | `scripts/generate-delta.sh` (bash per design decision 1+5); output to `dev/active/delta-{role-slug}-{date}.md`. Read-only generation. ~30-45 min. **DEFERRED 5:05 PM EDT**: pending drain-until-IDLE semantics fix to v0.5 design + cron prompt (see escalations doc); resume next session. |
| MEM-975: implement-hook (slot 12qq) | **Implement SessionStart hook extension (signal layer)** | May 25 | Following implement-script. Add modular function block to `.claude/hooks/session-start.sh`. ~15-20 min. **DEFERRED 5:05 PM EDT**: pending drain-until-IDLE fix; resume next session. |
| MEM-975: test (slot 12rr) | **Test (manual run + cross-session validation)** | May 25 | Following implement-hook. Manual run verify file; trigger hook verify signal; test edges (empty + long delta). ~15 min. **DEFERRED 5:05 PM EDT**: pending drain-until-IDLE fix; resume next session. |
| MEM-975: close-and-memo (slot 12ss) | **Close #975 + completion memo to Lead Dev** | May 25 | Following test. Update #975 description checkboxes per `close-issue-properly` skill (NOT comment-only close — see [[feedback_close_issue_properly_skill_recurring_miss]]). Closing comment with implementation evidence + completion memo to Lead Dev CC PA, CEO. ~15 min. **DEFERRED 5:05 PM EDT**: pending drain-until-IDLE fix; resume next session. |
