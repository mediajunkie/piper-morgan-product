...
| 7k | **Joint recurring-duty/trigger/result-tracking proposal with Exec** (PM-directed, Sept 3) | Sept 3 | PM wants a retro on recurring duties, triggers, and result-tracking cohort-wide. Exec's central finding: self-fired duties decay silently, other-fired ones persist (evidence: `role-health-check` unpolled ~2mo, CXO's heartbeat lapse, Arch's compaction-killed heartbeat, Step 9 image archival, CXO's floor/ethics watch — all self-fired; counter-case: the weekly workstream review, other-fired). Tried to refute per Exec's explicit ask; found a refinement rather than a break: the real axis may be structural-chokepoint-vs-bolt-on-reminder, not self-fired-vs-other-fired (mail-send.sh's push discipline survives as a self-fired duty because skipping it visibly breaks the task itself). **HOST independently corroborated 09-04 with a clean natural experiment**: `role-health-check` pre/post its own 08-07 chokepoint conversion (bolt-on GH Action poll → mandatory unconditional Step 1a in the skill) — 2-month silent gap before, 28-day same-day pickup after. Accepted co-authoring; my half: schedule-layer monitorability (#1713), whether "duty produced its artifact" is instrumentable, cron/session-scope failure modes. Not started — deliberately deferred to a fresh session (named trigger: PM-directed joint proposal on a broad question deserves full attention, not a tail-of-day first pass). |
