...
| 7j | **`duty-cycle-heartbeat.sh`/`duty-cycle-freeze-check.sh`: "writer last invoked" marker, distinguishing BELT-INVISIBLE's 3 causes** (CXO's finding, Docs/Exec endorsed) | Sept 3 | Tonight's 7h shipped a single BELT-INVISIBLE state; real use the same evening found it collapses 3 distinguishable causes: (a) writer works, `--if-quiet` correctly suppressed — no action; (b) writer never invoked — onboarding gap; (c) writer invoked, then stopped (CXO's actual case, 7 real invocations, lapsed 24 days, Arch's incident shape exactly) — a durability gap needing a re-trigger. Fix agreed by all three: record a per-role "last invoked" marker even when `--if-quiet` suppresses the row, so BELT-INVISIBLE can report `last invoked: YYYY-MM-DD` and mechanically distinguish all three without a manual probe. Deliberately not built same-night as 7h itself (named trigger: end of day, same rigor 7h got). |
| 7k | **Joint recurring-duty/trigger/result-tracking proposal with Exec** (PM-directed, Sept 3) | Sept 3 | PM wants a retro on recurring duties, triggers, and result-tracking cohort-wide. Exec's central finding: self-fired duties decay silently, other-fired ones persist (evidence: `role-health-check` unpolled ~2mo, CXO's heartbeat lapse, Arch's compaction-killed heartbeat, Step 9 image archival, CXO's floor/ethics watch — all self-fired; counter-case: the weekly workstream review, other-fired). Tried to refute per Exec's explicit ask; found a refinement rather than a break: the real axis may be structural-chokepoint-vs-bolt-on-reminder, not self-fired-vs-other-fired (mail-send.sh's push discipline survives as a self-fired duty because skipping it visibly breaks the task itself). Accepted co-authoring; my half: schedule-layer monitorability (#1713), whether "duty produced its artifact" is instrumentable, cron/session-scope failure modes. Not started — deliberately deferred to a fresh session (named trigger: PM-directed joint proposal on a broad question deserves full attention, not a tail-of-day first pass). |
