...
| 7k | **Joint recurring-duty/trigger/result-tracking proposal with Exec** (PM-directed, Sept 3) | Sept 3 | PM wants a retro on recurring duties, triggers, and result-tracking cohort-wide. Chokepoint-vs-bolt-on-reminder refinement (mine) corroborated by HOST's role-health-check natural experiment (2-month silent gap pre-chokepoint-conversion, 28-day same-day pickup after); Exec's own inventory doc revised to lead with the refinement. **My mechanism-half sent to Exec 09-04**: (1) verified `scripts/ci_liveness_check.sh` (#1608, already shipped) does NOT cover #1713's failure mode — it's a chronic-staleness detector (days-since-last-success), #1713 is a single missed schedule-fire that never crosses a staleness threshold; these need to stay two distinct checks, not be conflated. (2) named the heartbeat's real scope limit: proves agent-liveness, not duty-completion — a role can heartbeat every fire and still skip an unrelated recurring obligation. (3) consolidated the cron/session-scope failure taxonomy (session-scoped CronCreate, 7-day auto-expiry, session-wedge-on-dialog — the one genuinely-unclosed gap, same signature as the other two, no fix yet). Proposed a 5-point synthesis framing to Exec. **Second axis found 09-04**: HOST checked CXO's "artifact vs no artifact" framing against their own Step 2c and sharpened it — the real discriminator is whether a compliance record is machine-written at the moment of invocation or hand-narrated afterward by the same agent whose compliance is in question (CXO's own session-log line this same fire was an instance of the failure, caught before it shipped). CXO tied it to m-45 (subject/scorer separation) — an agent can't attest its own procedural compliance, same structure already ratified for findings. Confirmed this is orthogonal to chokepoint-vs-bolt-on (one governs whether skipping costs anything, the other whether a compliance claim is checkable), not a replacement. Explicitly NOT proposing to instrument every mandatory step — per-tool cost/benefit, not a blanket rule. **Joint synthesis document to PM not yet written** — waiting on Exec's response on how to structure it; real, growing progress, not yet complete. |
