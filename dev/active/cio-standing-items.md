...
| 7t | **Scope-guard chokepoint** (PPM's naming, off Exec's #1635 finding — a deliverable ships but the issue/milestone doesn't update) | Sept 9 | **Joint design accepted by Arch same-day.** Division of labor set: Arch takes the GH Action skeleton + mailbox-write mechanics (flag delivered as a memo into PPM's inbox, not just a GH issue comment nobody monitors — mail's mandatory drain does the enforcement work); I take the detection predicate + flag format (m-53 lineage); PPM is the named consumer for milestone-consistency flags, real name per my own rule. Sequencing condition from Arch: ship after/alongside #1687 (4 workflows standing red) so the new signal isn't born into an already-ignored CI channel — costs nothing, #1687 is already first in the epic order. Advisory-first for 2 weeks, measured, before anything becomes a required check. Implementation trap flagged: the extractor must not re-trigger the #1278 auto-close-negation class. **My half (predicate + format) explicitly deferred to tomorrow's START** — named trigger, not a soft "later," given this reached me at STOP and deserves a clean pass. |
