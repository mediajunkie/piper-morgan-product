...
| 12nn | **MEM-975 step 1 — read precondition (issue body + Lead Dev routing memo)** | May 25 (pulled forward from week of May 26-30 per PM 10:17 AM EDT directive) | Read `gh issue view 975` for MEM-DELTA AC details + re-read `mailboxes/cio/read/memo-lead-to-cio-cc-pa-mem-975-delta-hybrid-mechanism-routing-2026-05-24.md`. Confirms hybrid mechanism shape ratified Sunday: script generates detail to `dev/active/delta-{role}-{date}.md` + SessionStart hook adds one-line signal (~50 tokens). Implementer discretion noted (hook integration; signal format; invocation cadence; "since-last-session" scope). ~10 min. |
| 12oo | **MEM-975 step 2 — design pass (delta-generator script shape + signal format)** | May 25 | Following 12nn. Decide: (a) script invocation cadence (manual? scheduled? hooked into SessionStart?); (b) what "since-last-session" means concretely (last commit timestamp? last session log timestamp? explicit checkpoint file?); (c) signal format (Markdown header? plain line? icon prefix per existing hook conventions). Output: design note in this log + Read of `.claude/hooks/session-start.sh` to understand modular integration shape. ~20 min. |
| 12pp | **MEM-975 step 3 — implement delta-generator script** | May 25 | Following 12oo. Script lives at `scripts/generate-delta.{py,sh}`; output to `dev/active/delta-{role}-{date}.md` per spec. Read-only generation (does not mutate state). Per the v0.5 SessionStart-hook-extension overlap note: keep separable from CHECK trigger (that's a Phase C+ item). ~30-60 min depending on language + cross-session-bridge sophistication. |
| 12qq | **MEM-975 step 4 — implement SessionStart hook extension (signal layer)** | May 25 | Following 12pp. Add to `.claude/hooks/session-start.sh` (modular, per Sunday memo "keep modular so the two extensions don't entangle"). Signal format: one line, ~50 tokens, with counts + pointer to detail file. Verify hook exits 0 even if script fails (safety net layer per existing patterns). ~15-30 min. |
| 12rr | **MEM-975 step 5 — test (manual run + cross-session validation)** | May 25 | Following 12qq. Run script manually → verify detail file generated. Trigger SessionStart hook → verify one-line signal appears + points at detail. Test edge: empty-delta (no work since last session). Test edge: long delta (many commits). ~15 min. |
| 12ss | **MEM-975 step 6 — close #975 + completion memo to Lead Dev** | May 25 | Following 12rr. Update #975 description checkboxes per `close-issue-properly` skill (NOT comment-only close — see [[feedback_close_issue_properly_skill_recurring_miss]]). Closing comment with implementation evidence (commit refs + file paths + verification output). Completion memo to Lead Dev CC PA, CEO. ~15 min. |
