...
| 7k | **Joint recurring-duty/trigger/result-tracking proposal with Exec** (PM-directed, Sept 3) | Sept 3 | PM wants a retro on recurring duties, triggers, and result-tracking cohort-wide. Two orthogonal design principles corroborated so far: chokepoint-vs-bolt-on (mine, HOST's role-health-check natural experiment) and machine-written-vs-self-narrated compliance (HOST/CXO). **Correction 09-04**: the second principle was cited as "m-45" in this fire's own thread — Docs checked the actual doc and it's a different failure mode entirely (independent-agents'-agreement-is-not-corroboration, not self-attestation). No existing methodology-core entry covers self-attestation (Docs grepped, confirmed empty). Either a genuinely new principle to file, or informal shorthand that landed on the wrong number — my call, not yet made. My mechanism-half sent to Exec 09-04 (schedule-layer monitorability — #1608 does NOT cover #1713's shape; heartbeat's real scope limit; cron/session-scope failure taxonomy). **Joint synthesis document to PM not yet written** — waiting on Exec's response on how to structure it; real, growing progress, not yet complete. |
| 7l | **Cold-start defect in the 7j "last-invoked" marker** (Exec's finding, CXO's fix design, Sept 4) | Sept 4 | Shipped 7j this same morning; by tonight Exec found it reports "never — not called even once" for roles with real, extensive prior heartbeat history (Docs: 20 commits, most recent yesterday) — the marker file just didn't exist before today, so its absence gets reported as the event's absence. CXO independently verified the fix design: on a missing marker, derive it once from `git log --grep="hb(<role>)" -1` (the existing `age_of()` attribution convention) rather than reword to a caveat — keeps the three-case taxonomy intact (genuine "never" stays true: no marker AND no `hb()` commits ever) instead of adding a fourth "unknown" bucket. Also add a provenance flag (derived vs. observed) per CXO's suggestion, so a backfilled value is never mistaken for a direct write. Deliberately not built same-night as 7j itself (named trigger: end of day, Exec explicitly said not urgent — nothing broken, no one blocked). |
| 7m | **`mail-send.sh`: warn when a filename's date stamp disagrees with frontmatter `date:`** (Exec's proposal, Sept 4) | Sept 4 | Real instance found same-day: my own Ship #059 filename (`workstream-059-cio-2026-08-28.md`) carries #058's date stamp — copy-the-previous-report-as-template, update the number, rewrite the body, and the filename's date segment survives untouched since nothing re-reads it. Exec's framing: a pure bolt-on, zero cost to the writer, all cost deferred to a reader who can't distinguish a mislabeled current file from stale debris (nearly caused a wrong "9 of 10 filed" report to PM). Same shape as the already-shipped #1716 checker (mail-send.sh warning on a header/delivery mismatch) — same script, same moment, converts a bolt-on into something skip-visible. Not yet decided whether to build; queued for consideration alongside 7l, not committed. |
| 7n | **m-45 citation drift — self-attestation principle has no canonical home** (Docs' finding, Sept 4) | Sept 4 | Docs caught, during omnibus-writing, that today's recurring-duty thread (me, CXO, HOST) cited m-45 for "an agent can't attest its own procedural compliance" — verified directly against `methodology-45-AGREEMENT-IS-NOT-REPLICATION.md`: wrong doc, that principle is about independent agents' agreement being mistaken for corroboration (a social-layer failure), not self-attestation at all. Grepped methodology-core for "subject/scorer separation," "can't attest its own," "self-attest" — nothing matches; no existing entry covers this. Own correction: was propagated in my own reply to HOST this fire and in 7k's own tracker line — both need the wrong citation removed (7k's line already corrected this fire). **Disposition not yet made**: either file the self-attestation principle as a genuinely new methodology-core entry (real candidate — corroborated twice today, HOST's Step 2c + CXO's own near-miss in the same fire), or treat it as informal shorthand not worth codifying yet. Deliberately not decided at 22:37 — methodology-core entry stewardship deserves real deliberation, not a rushed STOP-fire call. |
