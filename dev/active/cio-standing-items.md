...
| 12g | ~~"Silent State Mutation in Shared Working Tree" parent meta-pattern candidate~~ — **RESOLVED May 11** (filed as Pattern-067; see R18 in Resolved) | May 10 → May 11 | Promoted to Emerging per PM May 11 directive. |
| 12h | ~~"Coarse Triggers Causing False-Positive Triage Cost" hook-design meta-pattern candidate~~ — **RESOLVED May 11** (filed as Pattern-068; see R19 in Resolved) | May 10 → May 11 | Promoted to Emerging per PM May 11 "close the loop" directive. |
| 12i | **Worktree-path consistency convention** — methodology codification routing to Docs | May 11 | Per Pattern-067 filing: the path-fragmentation child (P-17) was caught only because the next session noticed cross-tree diff. Codification ask routed to Docs for inclusion in `branch-worktree-mailbox-discipline.md`. ~30 min Docs work. Surface area for fragmentation drift continues until codified. |
| 12j | **Possible Lead-Dev shell wrapper / hook for cross-tree edit detection** | May 11 | Per Pattern-067 filing: Lead Dev assessment ask for tooling that warns when an agent is editing files outside its current working-directory's git tree. Cost-benefit unevaluated. Routed to Lead Dev for feasibility read; no urgency, can sit until Lead Dev has bandwidth. Convention (12i) is the higher-leverage first step regardless. |
| 12k | **PreCompact hook refinement** — Docs lane per HOST May 10 framing | May 11 | Per Pattern-068 filing: Docs owns hook script (`.claude/hooks/precompact-signoff-warning.sh`). Three refinement options ranked by leverage: locality differentiation (highest), explicit "safe to compact" path (docs-only), severity reduction for known-safe patterns. CIO no longer owns disposition; tracking for visibility. |
...
| R18 | **Pattern-067 (Silent State Mutation in Shared Working Tree) Emerging filing** | May 11 | Filed Emerging per PM May 11 directive ("we need to solve these issues to avoid a real problem occurring or loss of valuable effort"). Parent meta-pattern subsuming P-13/P-15/P-16/P-17 anti-patterns. Anti-pattern index updated with cross-refs + P-17 (working-tree-path fragmentation) added as fourth child. Innovation Backlog: was Operational #44, now Emerging #46. |
| R19 | **Pattern-068 (Coarse Triggers Causing False-Positive Triage Cost) Emerging filing** | May 11 | Filed Emerging per PM May 11 "close the loop" directive. Hook-design meta-pattern; companion to P-067. Innovation Backlog: was Operational #45, now Emerging #47. Promotion to Proven requires cross-mechanism recurrence (not PreCompact-only). |
