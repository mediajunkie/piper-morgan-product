...
| 7p | **Methodology candidate: "bounded search reported as total"** (CXO's finding, Sept 5) | Sept 5 | CXO surfaced three same-week instances of one shape: a real, correctly-run command whose scope (`--since` window, a reproduction under narrower conditions, a `head -4` truncation) was chosen and then not stated, each time reported as if it covered the full answer — plus the sharper point that CXO's own honest-sounding hedge ("I'm not claiming it isn't, I'm claiming I couldn't establish it") was formally correct and still misleading, because it named the wrong cause of the uncertainty (the truncation, not the source). Adjacent to m-44 ("state the denominator") but distinct — m-44 is about omitting a denominator; this is about a hedge that states one and still misattributes it. Deliberately NOT filed tonight (STOP fire, already dense) — **explicit trigger: pick up first at tomorrow's START**, before other work. Evidence fully gathered in CXO's two memos; nothing left to collect, only the write-up. |
