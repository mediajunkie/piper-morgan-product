...
| 7k | **Joint recurring-duty/trigger/result-tracking proposal with Exec** (PM-directed, Sept 3) | Sept 3 | PM wants a retro on recurring duties, triggers, and result-tracking cohort-wide. Two orthogonal design principles, both now formally filed as methodology entries: chokepoint-vs-bolt-on (mine, HOST's role-health-check natural experiment) and self-attestation-is-not-verification (**m-50, filed 09-05** — see 7n). **New evidence for the proposal, HOST's count (09-05)**: three confirmed real "invoked, then stopped" lapses in one week (CXO's heartbeat, CXO's MANIFEST regen, Docs' heartbeat), one seat repeated — caught only because a marker existed and people kept auditing themselves afterward. HOST frames this as a trust-lane signal about the cohort's recurring-duty surface reliability, not an individual-diligence issue; feeding directly into the proposal's evidence base. My mechanism-half sent to Exec 09-04 (schedule-layer monitorability — #1608 does NOT cover #1713's shape; heartbeat's real scope limit; cron/session-scope failure taxonomy). **09-06 update**: Exec's "unguarded entrance" finding (7q), CXO/HOST's corroboration, and the subagent-worktree accountability ask (7r, now closed) are all the same underlying mechanism (cleanup attached to a thing ending cleanly, rather than to a chokepoint that survives an unclean end) — three separate deliverables sharing one design lens now, not three unrelated asks. Exec gave the explicit go-ahead to draft (with a suggested order: shared cause → chokepoint/bolt-on axis as diagnostic → the inventory as evidence, not the point) and 7r shipped same-fire. **Deliberately NOT drafted that night** — named trigger: a fresh pass at the next START. **09-07: drafted and sent to Exec for their pass before PM** — `dev/active/synthesis-7k-recurring-duty-reliability-2026-09-07.md`, built against Exec's own suggested structure (shared cause: state created by a start, cleanup attached to a clean ending, endings aren't always clean → chokepoint-vs-bolt-on as the diagnostic, using HOST's role-health-check pre/post-08-07 natural experiment as the clean before/after → the full inventory as evidence, not the lead). Seven concrete recommendations, two items named explicitly as still-open (the session-wedge death mode; #1608 vs #1713). Waiting on Exec's edit pass before this goes to PM. |
