...
| 12bb | **Session-type taxonomy → git-permission scope** — methodology candidate | May 17 | Per Vehicle 2 attempt #1 cloud-session diagnostic: cloud-CIO session proxy-blocked on main-push (committer identity `Claude` vs `mediajunkie` permission scope). Local Code sessions push under PM's identity; cloud sessions push under proxy identity. Implication: extending CIO session-vehicle pattern to cloud requires either (a) permission upgrade for cloud-Claude committer identity OR (b) methodology amendment — e.g., cloud-CIO writes to branches only and merge-keeper sweep handles main-fold. Codify as methodology entry alongside Postel (12aa) + methodology-30 (12u). ~30 min focused entry. Useful for Vehicle 3+ planning AND for cross-agent cohort extension (PM still hand-writing "check your mail" nudges — demand visible). |
