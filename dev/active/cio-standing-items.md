...
| 7z | **`pre-commit-broad-staging-warn.sh` — two real fixes found while executing the WARN ruling, deliberately deferred**: (1) migrate to a PostToolUse hook — the WARN ruling (Arch's reasons 1+2, HOST-concurred) is correct, but a PreToolUse exit-0 implementation was tested live and found to produce ZERO visible output to the agent (confirmed via the hook's own log firing with nothing surfacing) — currently reverted to BLOCK (exit 2, confirmed working) as the safe interim state, since a "warning" nobody sees is worse than useless. `memory-index-overlimit-warn.sh` is the working PostToolUse precedent to follow. (2) Move to the common-dir git-native `.git/hooks/pre-commit`, same as `check-branch.sh` — found this hook has the identical time-of-check/time-of-use bug: a compound `git add && git commit` in one Bash call silently bypasses it entirely (empty index at PreToolUse-fire time), independent of block vs. warn. | Sept 13 | Both real, both deserve a careful pass rather than a rushed follow-up in the tail of an already-eventful investigation. Current live state (BLOCK) is safe and matches what every agent has actually experienced all along — nothing regresses by deferring. |
