...
| 12g | **"Silent State Mutation in Shared Working Tree"** parent meta-pattern candidate | May 10 | Per Code agent's May 10 staging-race memo + HOST May 10 concurrence on shelf-placement. Parent shape subsuming branch-drift (P-13 / Lead Dev May 9 #5), index-drift (today's PPM staging-race), residue-drift (P-16 candidate Cross-Agent Residue Accumulation). Common mechanism: shared `.git` + concurrent agent activity → silent mutation of stable-looking state. Filed in Innovation Backlog Operational #44. Candidate for Emerging filing at next Pattern Sweep. HOST stance: name the parent for cohort vocabulary; don't codify as discipline gate. |
| 12h | **"Coarse Triggers Causing False-Positive Triage Cost"** hook-design meta-pattern candidate | May 10 | Per Code agent's May 10 PreCompact-hook second-incident addendum + HOST concurrence. Distinct from triggering-failure-mode patterns: failure is in *how* mechanism weights what it detects, not in *what* it detects. PreCompact hook two-fire-one-day example. Filed in Innovation Backlog Operational #45. Disposition: hold for one more incident before filing; smaller surface than #44 may stay tactical observation. |
