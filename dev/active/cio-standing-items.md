...
| 7v | **PM's work-queue ruling implementation** (carried + mail + newly-observed GitHub issues, idle only when all three empty) + retiring "next fire"/`## Fire N` vocabulary from `duty-cycle-tick` SKILL.md + **CXO's structural NO-SESSION-LOG fix** (move Step 0's session-log commit before the mail loop, making the race zero by construction — added 16:37 fire) | Sept 11 | **CIO's call, per Exec's explicit routing.** Not started — deliberately deferred to a fresh session/compaction (named trigger) rather than rushed piecemeal across three same-day fires. Three things now bundled into one skill-text pass: (1) work-queue redefinition + vocabulary retirement, (2) Step 0 reordering, (3) Exec's 3-item process proposal below (7x). CXO's per-role criteria-line example (`label:UX state:open`, denominator 3) is the worked pattern to generalize for (1). |
...
| 7x | **Exec's 3-item process proposal**: archive `mailboxes/*/read/` (11,510 files, quarterly `read/archive/YYYY-QN/`, MANIFEST-regen-safe, exercise on one seat first), change the cc rule (PM cc'd only for (a) PM-only decisions, (b) relaying PM's rulings, (c) something PM would contradict — everything else via attention rollup), adopt "re-check anomalies before reporting" as a one-line procedural rule. | Sept 11 | Mine as skill/process owner per Exec. Not actioned this fire — bundling with 7v since the "re-check anomalies" rule and the cc-rule change both plausibly belong in the same skill-text pass as the work-queue/vocabulary rewrite, rather than three uncoordinated edits to the same document in one day. |
