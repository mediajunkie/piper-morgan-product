...
## PM-engaged ~20:30 — Michelle up a tier + recurring-workflow owner-routing reminder
- **Michelle Hertzfeld → Tier 2 (next wave)** per PM (relationship value as first external tester outweighs the weaker easier-setup hook). Updated the (uncommitted) tiering doc.
- **Recurring-workflow reminder** (PM: "add that reminder... for all the recurring workflows honestly"): the role-health-check auto-issue assigns to PM (= a literal PM-as-catch instance; GH doesn't notify agents). **Two-half fix**, role-health-check done as exemplar: (1) workflow-side **owner-routing banner** in the generated issue (`role-health-check.yml`, `497be28dc` — "Owner: HOST; PM can disregard; HOST's cycle polls"); (2) owner-side — **HOST cycle now polls** `gh issue list --label sapient-trust --state open` (carry-forward standing responsibility). **Generalized to a proposal → CIO** (cc Docs/Lead/PM, `f60fe8e24`): recurring auto-issues name their owner + the owner's cycle polls its label; composes m-39 (PM-as-catch) + the mail-vs-GH norm + m-36. Other workflows = Docs (audit-calendar) + Lead (CI) at convenience.
- PM acknowledged the dev/alpha privacy finding (thanks) but hasn't yet decided keep-tracked-vs-gitignore → still holding tiering doc + roster as-is.
