...
- **Role→model map — RATIFIED 6/13** (`docs/operations/duty-cycle design/role-model-map.md`). PM **recovered the original map from old-CIO's transcript** (my proposal was too conservative — actual decision is more Sonnet-aggressive + a Haiku tier). Canonical: **Opus** = Architect, CIO, Exec; **Sonnet** = CXO, PPM, Comms, Docs, HOST, Web (+ LD **Sonnet-default / Opus-burst**); **Haiku** = PA-option + mail-only fires. **OPEN — PM reconcile**: LD migrated to Opus 6/12 but map says Sonnet-default (artifact of the lost map) → flip LD to Sonnet, or override-for-LD? Queued roles migrate per map (HOST→Sonnet ✓). Plan-of-record + memory updated. **Capability**: `search_session_transcripts` (approval-gated) finds transcript-only content that committed-docs search misses — that's why I missed this map earlier.
