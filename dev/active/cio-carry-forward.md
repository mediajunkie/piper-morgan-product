...
- **Role→model map — PROPOSAL WRITTEN 6/13, awaiting PM ratify** (`docs/operations/duty-cycle design/role-model-map.md`, STATUS: PROPOSED). Redid the per-role model spec lost to a logging lapse. Tiers: **Opus-firm** = Exec/CIO/LD/Arch; **swing** = PPM/CXO; **Sonnet** = PA/HOST/Comms/Docs (HOST/Comms/Docs = the Opus→Sonnet cost win). PM migrating HOST → proposed **Sonnet** for HOST. **On PM ratify**: status → RATIFIED + fill plan-of-record model column + pin the map in memory. (Lesson: `feedback_write_down_even_if_not_ratified`.)
