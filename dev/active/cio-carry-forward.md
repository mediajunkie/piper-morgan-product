...
- **Gap-C still UNSOLVED (deployed)**: CronCreate dies on resume/compaction (afb1da90 died on the `/model` [1m] switch 6/13; re-armed `16d19ac8`). Candidate real cure = `mcp__scheduled-tasks` (disk-persistent, survives restart, LIVE on account) — NOT yet prototyped as the duty-cycle backbone. Offered to prototype (PM approval). The skill self-heal (re-arm-if-zero) is only a partial mitigation (needs a session turn to fire).
