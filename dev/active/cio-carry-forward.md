...
- 🔴 **COHORT MIGRATION GATED ON HOST'S RULING — user-level hooks (2026-07-25).** Project hooks (`check-branch`, `log-maintenance-reminder`, PreCompact sign-off) **do not fire in a Model-A worktree**. Config is present and correct; the harness isn't activating it for a sibling-path worktree. *Trust was a red herring — my first diagnosis, corrected by Pard from the docs.* Only docs-supported fix is moving hooks to user-level `~/.claude-pm/settings.json`, which takes enforcement config out of the repo where it's reviewable → **governance call, filed to HOST, response-requested**. Until it lands, **every agent (including me) is running with NO enforcement** — mailbox discipline and log maintenance are manual. **Bulk migration deliberately gated**: one agent migrates as a paired behavioral test (stage a `mailboxes/` file on a non-main branch, confirm the commit is blocked) before the rest roll.
- 🟡 **One thing needs PM's keystroke**: run **`/hooks`** in the CIO worktree session and report whether `check-branch` appears. It's a REPL command an agent can't invoke. Confirms *not-listed* (expected) vs *listed-but-not-firing* (surprising, escalates to `claude --debug hooks`).
- ✅ **Worktree lifecycle v0.1 RATIFIED** (7-day grace + two-phase adopted; `.agent-session` lockfile declined; collision = tmux-cwd, basename fingerprint retires). Implemented + tested by Pard; the design caught two real bugs in his build, one of which would have deleted a standing worktree.
- ✅ **CLAUDE.md worktree model revised, PM-approved** — host-dependent, Model A current on Amber. Docs + HOST notified.
- **Duty cycle**: `a645461c` at `7,27,47 * * * *` (20-min COLLABORATION cadence, bumped from LEAN for the active Pard window). **Revert to LEAN `7 10,16,22` when the migration collaboration closes** — don't let it persist by inertia.

- ✅ **CIO migration to Amber — DONE 2026-07-25 ~10:55. Successor session live in `~/Development/piper-morgan-worktrees/cio`, cron `d250236f` armed.** Full account: `dev/2026/07/25/2026-07-25-1053-cio-code-log.md`. **The old-machine cron `d854c9be` can now be deleted** (its migration-pending guard is satisfied — a verified Amber CIO exists). **Two items need PM/Pard, both routed to Pard 2026-07-25 (`mediajunkie` `86c634c`)**: (1) the memory symlink Pard wired is attached to the *worktree* key, but Claude Code resolves memory to the *git common dir* key — so it's a no-op, and worktrees may share memory by construction, making the symlink layer unnecessary. Held as ONE data point; the second agent's worktree is the decisive test and should run before the cohort migrates. (2) **The shared memory pool is empty — the 162-file export was never seeded anywhere**, so the cohort currently has no live memory on Amber. Seeding is held only until (1) resolves so it happens once into the right directory. Also fixed at arrival: this worktree was provisioned from a June-12 branch **5,393 commits behind main** — provisioning must assert currency before handing a worktree to an agent.
- ⚪ *(superseded, kept for provenance)* **CIO migration to Amber + pipermorgan.ai — was ACTIVE.** PM initiated 7/24 after a genuine multi-day outage (all terminal sessions ended 7/19). I'm first to migrate; Pard partners on the mechanics. Full context gathered this session (Pard's `amber-agent.sh`/`amber-harbor-status.md`, the ratified `migration-checklist.md` v1.2, Janus's synthesis memo to Pard) — see today's dated log for the complete picture. **Real risk flagged, not yet mitigated**: my new path on Amber (`~/Development/piper-morgan-product`, flat) differs from my current path (`~/Development/piper-morgan/piper-morgan-product`, nested) — Claude Code encodes the full filesystem path in memory-directory keys, so this WILL silently orphan my accumulated memory unless deliberately handled. Needs to be part of the handoff-package discussion with PM, not discovered after the fact.
