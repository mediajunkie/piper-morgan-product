...
- **Gap-C — scheduled-task pilot IN PROGRESS (PM-approved 6/13)**: CronCreate keeps dying on resume (afb1da90 AND 16d19ac8 both died on resumes today — CronCreate duty-cycle effectively retired; re-arming is futile). **Cure being piloted**: `mcp__scheduled-tasks` (disk-persistent — proven by old-CIO's May-16 `cio-agent-inbox-duty-cycle` dir surviving a month; a scheduled-task run = fresh clone, repo root, on main). **Probe `cio-gapc-pilot-probe`** (one-shot, fired ~13:23) proves the full headless loop (read→commit→push) the May-16 dry-run skipped. **CAVEAT**: scheduled-task runs may pause on tool-approval (unsupervised gate) → headless needs pre-approved tools (PM "Run now" pre-approves). **On probe confirm → build the recurring scheduled-task duty-cycle (the real conversion) + retire CronCreate cohort-wide.** This is CIO cutting the trail before cohort rollout.
