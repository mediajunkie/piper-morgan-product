...
- **#975 MEM-DELTA — close?** (PM flagged for discussion). Implementer + cohort-rollout DONE (hook auto-runs cohort-wide since 5/26); **validated 6/12** — live generator emits accurate deltas (CIO run: correct 20-commit/1-memo delta anchored on the session-log timestamp, clean truncation). Only unmet AC = before/after session-start-time measurement (unmeasurable retroactively). **CIO recommends CLOSE**; awaiting PM nod ("your call to close").
- **#972 MEM-TEMPORAL — scope when PM has bandwidth** (PM flagged; CIO-owned). Two drivers now: (a) migration staleness incidents (stale §5 / Model-A line / cron-prompt); (b) BYOC multi-user write-governance gate (flagged in the PA phase-2 reply). Work = convention change (define `valid_from`/`ended`/`superseded` fields; stamp briefings/memos/prompts; templates; 3 examples; Janus field-spec coord). CIO drafts a concrete scoping plan on PM go.
- **LD worktree-exception — VERIFY + reconcile**: the 6/12 delta surfaced LD's bootstrap log saying "no Model-A exception needed." If confirmed, the "exception: e.g. Lead Dev" examples just written into CLAUDE.md + BRIEFING-CURRENT-STATE + the plan-of-record exception rubric should flip to "no current exceptions" (Model A fully deprecated, zero carve-outs). **Verify against LD's actual session log first — don't act on the one-line delta summary.**
