...
- **Migration wave — HOST done, Comms next** (6/13). HOST MIGRATED on **Sonnet 4.8** (PM; current Sonnet release = 4.8 → map version note added: tier fixed, version tracks current). **Comms pair DRAFTED** (`dev/active/comms-{migration-handoff,bootstrap-brief}-2026-06-13.md`; leaner than HOST's — patterns established, points to plan-of-record + HOST pair as templates; Comms cron `12 6,9,12,15,18,21`, daytime-only). **Recommended order: Comms → CXO → PPM → Arch → Docs** (Docs last = keep the merge-keeper safety-net stable through the others; Arch = Opus-no-change, lowest-risk, slots anytime; the rest are Sonnet-changers riding HOST's fresh path). I draft each pair as PM migrates one-at-a-time.
- **Gap-C — scheduled-task pilot SUCCEEDED ✓ (6/13)**: probe `cio-gapc-pilot-probe` **fired autonomously 13:23 + did the FULL headless loop** (read→write→commit→push, `e0de384e7`); disk-persistent (survives the resumes that keep killing CronCreate — afb1da90 + 16d19ac8 both died today); ran **main-checkout-direct** (repo root, on `main` — NOT a fresh clone, NOT an ephemeral worktree); **no tool-approval gate hit** (ran headless without "Run now"). **Scheduled-tasks = the working Gap-C cure.** CronCreate duty-cycle retired (futile — dies every resume). **NEXT: build the RECURRING scheduled-task duty-cycle for CIO** (main-checkout-direct prompt: commit/push straight to main, no worktree/bridge; reads carry-forward; runs duty-cycle-tick; logs to session log) + verify. Considerations: fires a FRESH agent per fire (not in-session → possible double-up if an in-session agent is active); verify 7-day auto-expiry. **Observe a few fires → THEN propose cohort rollout** (don't overgeneralize yet — PM's "cut the trail first").
