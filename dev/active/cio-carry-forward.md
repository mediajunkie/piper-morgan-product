...
- ⚪ **Finding #6 follow-through** — the immediate instance is closed (HOST registered its own row; skill v1.17 makes START own it). What remains with **Exec** is the registry row *shape*, and the alert's phrasing fix so it names its denominator rather than reporting a subset as a total.
