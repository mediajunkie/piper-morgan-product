...
**Duplicate-cron pattern — root-caused with PM, fixed, live-tested.** This morning's `13b5541f`+`772e045e` duplicate turned out to be a genuine instance of `methodology-35` (Asymmetric Discipline, Emerging status): `duty-cycle-tick`'s STOP instruction said "re-CronCreate" without ever saying to delete the existing job first — any agent following it literally while idle at STOP (the normal case) would produce the same duplicate. Fixed all 4 places in the skill using that ambiguous phrasing to require `CronList` → `CronDelete` existing → `CronCreate` → `CronList`-verify-exactly-one (commit `d2d1e9656`). **Tested live** (PM's explicit ask, not simulated): walked the new sequence against this session's real cron — `772e045e` deleted, `8094d7db` created, confirmed exactly one job survives. Docs's `f33227b7` is the SAME methodology instance but a harder sub-case (cross-mechanism visibility gap, not a procedure gap) — still needs a discipline-level answer, not yet resolved. Corrected one imprecision from Friday's Ship #051 review: Arch's cron-prompt issue is a *different* bug (stale content, not duplicate count) that shouldn't have been grouped in.
