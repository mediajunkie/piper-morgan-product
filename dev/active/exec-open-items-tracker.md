...
|---|------|-------|--------|-------|
| 1 | **CXO's 3 MUX branches + CIO's xpoll-hook — 5 stale unowned branches, disposition still pending** | CXO/CIO | **RECONCILED 7/23**: this is a distinct, older item from a completely different "stale branches" thread than the Janus/PM/Docs one (`fix-docker-migration-setup` + 5 others, which Docs correctly resolved 7/21 — see item below). Conflated the two on 7/22 and sent Docs an unneeded correction; retracted 7/23 with an explanation. This item's 5 branches (`cxo-mux-step-3-cluster-review`, `cxo-mux-surface-2/4/7`, `xpoll-brief-staleness-hook`) are still genuinely all present on origin, unowned, no reply from CXO/CIO despite both being active. | 9+ days silent. Worth a light, clearly-scoped nudge to CXO/CIO next pass — explicitly naming these 5 branches to avoid another cross-thread mixup. |
| 1b | ~~Janus/PM stale-branches thread (fix-docker-migration-setup + 5 others)~~ | Docs | **RESOLVED 7/21, reconfirmed 7/23.** Docs deleted 5 of 6 on 7/21 as originally reported; only `claude/fix-docker-migration-setup` remains, holding for explicit PM go-ahead per Lead's "safe to delete" read. Janus re-verified fresh 7/22, Exec re-verified again 7/23 — both confirm. | Dropped from Active except the single-branch go/no-go, which is PM's call whenever convenient, not urgent. |
