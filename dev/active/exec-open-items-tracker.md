...
|---|------|-------|--------|-------|
| 1 | **Four stale unowned branches — disposition still pending** | CXO/CIO/Docs | **NO MOVEMENT since 7/14, verified via `find` again this pass** — none of the three (CXO's 3 MUX branches, CIO's xpoll-hook, Docs' confirmed-safe branch) have a reply, despite CXO/CIO both being fully resurfaced and active since 7/19. | 6 days now with named owners and no reply from anyone who's been active in the meantime — this has crossed from "hasn't been seen" to "seen, not yet acted on." Worth a light second touch next pass if still silent. |
...
| 3 | **#1386 beta-close gate — HOLDING for Lead's re-probe, not yet convened** | PM + Arch + Lead + Exec (relay) | **Sequencing corrected 7/19 evening**: Arch stopped Lead's original #1394 fix (would have reversed ratified ADR-078 D4) and redirected to a re-probe — the live #1394 failure predates B3's build, so the case may already be fixed with no new code. Re-probe was pending as of this morning's check (no result in mail or on the issue yet). **Deliberately not convening the CXO/PPM window** the 7/18 kickoff was meant to schedule, since running it now risks testing a stale failure mode. | Check for Lead's re-probe result first thing; only then move the CXO/PPM coordination forward. Gate itself: still 6 of 15 criteria met as of last direct check (7/19), #1278 scope call still outstanding, PM go/no-go still the final box regardless of #1394's outcome. |
