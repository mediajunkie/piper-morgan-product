...
| 6 | **Weekly Ship #051 — awaiting PM fact-check/voice-pass** | PM | **NEW, 7/14.** Redrafted with all 6 workstream memos folded in (PPM's arrived late, revealed the Sprint-field wipe as the window's real headline). On `origin/main`, PM reviewing this evening, edits expected in the morning. | Hold — do not re-edit the draft until PM's notes land, so we're not working the same file concurrently. |
