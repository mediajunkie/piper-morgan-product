# Verbatim export of the shared memory pool — 2026-07-30, pre-any-prune
...
**168 memories on disk.** Generated from the actual directory listing, never from a prior index.

**Each entry is `<slug> — hook`. The slug IS the filename** — open `<slug>.md` in this directory. Markdown links are deliberately omitted: duplicating every slug in link text *and* target consumed ~15.7KB of a ~17KB budget, i.e. the entire space for descriptions.

⚠️ **This file has TWO independent read limits and is SILENTLY TRUNCATED past either** — trailing entries vanish for every agent that loads it, with no error and no sign anything is missing. **(1) ~24KB of bytes** — it had reached 41.4KB (~40% of entries invisible, including most of the `reference` bucket) before HOST caught it 2026-07-25. **(2) ~200 LINES** — a separate ceiling that the byte count does NOT imply, found by PA 2026-07-26 at 194 lines while the byte guard was reporting a comfortable green. `scripts/rebuild-memory-index.py` now refuses to write past either, and warns from 90%.

**The line limit cannot be fixed by shortening text.** One entry = one line, so with 168 memories on disk the floor is 168 lines before any header — the real options are prune/merge, per-type index files with a router, or a denser entry format (cheapest, and worst for recall, since the description is what makes an index useful). **That is a governance decision about the whole cohort's shared pool, not a formatting choice for whoever trips the limit.** ⚠️ **Memory files are NOT under version control — deletion is irreversible. Export to a git-tracked file BEFORE pruning anything.**

**Provenance**: seeded 2026-07-25 from `dev/active/cio-memory-export-2026-07-24.md`, the verbatim export of the designinproduct.com pool. Memory is scoped per (account × project), so none of it transferred automatically at the account switch. This is the whole cohort's shared pool, not one role's — Claude Code keys memory by account and project, not by role, and on Amber it resolves to the git common dir, so every agent worktree off this repo shares it by construction.

## User — who xian is (1)

- user_xian — PM and founder of Piper Morgan project — direct,…

## Project — ongoing work, goals, constraints (15)
...
## Feedback — how this cohort works, and the corrections behind it (146)
...
- feedback_agent_naming_convention_in_public_prose — How to name agents in public-facing blog prose — \"my…
...
- feedback_dont_excoriate_iterate — PM 2026-07-25: don't over-flagellate on mistakes.…
...
- feedback_outside_feedback_elicits_pms_latent_design_read — Fresh-eyes feedback is an elicitation…
...
- feedback_present_tense_note_goes_stale_when_artifact_changes — A note asserting a present-tense fact about an…
...
- feedback_verify_negative_claims_via_live_api — PM 2026-07-12 — before asserting a file/resource doesn't…
...
description: "PM 2026-07-25: don't over-flagellate on mistakes. Tracking and correcting them IS the process working. It often takes ~three passes to shake down a new process — expect that, don't treat it as failure."
metadata: 
  node_type: memory
  type: feedback
...
## Merged 2026-07-27 — absorbed `feedback_deadlines_as_latest_acceptable_not_scheduled_windows` (same sender-side rule, recorded twice)

**CEO 2026-05-15 verbatim**: *"I would much rather have the presumption that any agent who gets a memo with an unblocked task does the task immediately or as soon as they are available to do it. We don't schedule and postpone things because that's the way to leave them to the last minute or even fail to do them in time."*

**The concrete wrong/right pair** — the most usable thing the absorbed pin carried:

- ❌ *"Target EOD Sun May 17 (~48–60-hr filing window over the weekend)"* — reads as "you have all weekend"; legitimizes deferral. **CEO corrected this exact phrasing.**
- ✅ *"as soon as you have a window"* + optionally *"latest acceptable: EOD Sun May 17"*.

**Boundary worth keeping**: process-cadence anchors (Wed publish, Fri kickoff) are fine as project-level rhythm; **per-task framing must not invoke them as deferral license.**

*Merge rationale: see the receiver-side pin. Four "deadline" files were two rules × two records; merged 4 → 2 along the sender/receiver seam this file already named.*
````
...
description: "A note asserting a present-tense fact about an artifact becomes FALSE the moment the artifact changes under it — write what you did and when, or re-check the note after any later edit lands."
metadata: 
  node_type: memory
  type: feedback
