"""
T-own-surface — H1 round 4, registered by CXO 2026-09-24 evening BEFORE any output:
  mailboxes/pa/read/register-cxo-to-pa-cc-ppm-h1-round-4-shape-with-id-2026-09-24.md
Follows RESULTS-t-own-surface-h1-round3-2026-09-24.md, where dropping the count did NOT help
GPT-4o (0/2, both reps now drop the whole completed_todos sub-list) — the counted-claim hypothesis
was not supported. PA offered, but did not register, a simpler reading: GPT-4o may not reliably
carry a bare {"note": "..."} member appended to a list, independent of wording.

CXO registers that reading as this round's test: hold wording at round 3's form
("...plus a few other completed items not shown"), change the member SHAPE to match its siblings
in completed_todos (id + title, like {"id": "T-38", "title": "Send the alpha invite"}) instead of
a bare {"note": ...} dict.

Properties 1-4 unchanged (Property 4: distinguishable list member; vanished = FAIL). CXO has said
four rounds is likely enough regardless of this one's result -- this is expected to be the last
round in the current series.
...
CELLS = [
    ("H1s_siblingshape", "coverage caveat as the LAST MEMBER, SIBLING-SHAPED (round 4: isolate shape, wording held at round 3's form)", True,
     "a distinguishable list item with id+title like its siblings, saying ~'and a few other completed items not shown' — Property 4",
     "piper_list_todos", "What's on my todo list?",
     {"pending_todos": TODOS_PENDING,
      "completed_todos": TODOS_DONE + [{"id": "T-more", "title": "...and a few other completed items not shown"}]}),
...
if __name__ == "__main__":
    ...
    path = "dev/active/probes/probe_t_h1_round4_%s_2026-09-24.json" % PROVIDER
