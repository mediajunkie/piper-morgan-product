...
## A near-catch (now withdrawn): `dev/active/` durability
I initially flagged that both this handoff **and** `cio-memory-export-2026-07-24.md` (162 files) live in `dev/active/`, which your CLAUDE.md calls sprint-cleaned/ephemeral (six roles lost entries that way in June). **xian has since clarified: `dev/active/` review is weekly and *nondestructive*, and PM agents are permitted to use it for active working files — so the export is safe.** Flag withdrawn. The only residue: your CLAUDE.md still documents that June data loss as if cleaning is destructive — a doc-vs-current-policy gap worth a one-line reconcile by Docs, but it doesn't touch this migration.
