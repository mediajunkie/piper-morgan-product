...
Executable plan + proof on **#1322 comment 4827173746**. The sim transport (`PiperMCPClient simulation_mode=True` / `MCPProtocolClient` / `MCPConsumerCore`) is DEAD CODE (chat GitHub = real REST always; sim reached only via an unserved `__main__` demo). **Arch CONCURRED 6/28 ("remove it", owns the live-path overstatement; strongly endorses the m-36 guard).** **Increments 1–2 DONE** (inc.1 `d61987a70`: `query_router_spatial_migration.py` + 8 federation/pm033c integration tests; inc.2: `query_router.federated_search` + dormant `mcp_consumer` wiring [import/param/2 init lines] + `test_pm033c_mcp_server_config.py` removed — 176 green, integration collects 890 clean). **Remaining (delete → full suite → commit, each its own increment):**
  - ~~inc.2~~ ✅ DONE.
  - **inc.3 — PARTIAL: inc.3a DONE** (`connect_to_mcp` removed, committed, github_adapter imports clean). **Remaining (for a fresh fire — investigation-heavy, do NOT rush in a bloated session):**
    - **Methods to remove** (both verified caller-less): `list_issues_via_mcp` (now ~L1089) + `get_issue_via_mcp` (now ~L1151). NOTE both *try sim then fall back to REAL REST* (`list_github_issues_direct`/`get_github_issue_direct`) — but they're dead (no callers), so remove whole; the real methods they fall back to STAY (used by the native chat path).
    - **mcp_consumer dependency to remove**: import `from .consumer_core import MCPConsumerCore` (L41) + attr `self.mcp_consumer = MCPConsumerCore()` (L97); and the **reference lines only** inside `get_mapping_stats` (the `"mcp_connected": self.mcp_consumer.is_connected()` key, ~L1237) + `disconnect` (the `await self.mcp_consumer.disconnect()` line, ~L1259) — **keep those 2 methods**.
    - **TEST BLAST RADIUS = 4 files** (bigger than first noted): DELETE (pure sim) `test_github_adapter_demo_fallback_1088.py` + `test_mcp_consumer_demo.py`; **INVESTIGATE real-vs-sim** `tests/integration/test_github_mcp_router_integration.py` + `tests/integration/test_github_integration.py` — if they exercise REAL REST through the `_via_mcp` wrappers, **update them to call the real methods directly** (don't lose real coverage); if pure-sim, delete.
    - Run full consumer suite + `pytest tests/integration/ --collect-only` after.
