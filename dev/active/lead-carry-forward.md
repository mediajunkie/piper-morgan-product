...
- **RECONNECT Phase-1 — WS-1 BUILD IN PROGRESS** (Arch RATIFIED #1232 + ruled WS-1-now, independent of #1185, 2026-06-21). **Guide: `dev/2026/06/21/1226-ws1-config-store-gameplan.md`** (P0→P5, audited PROCEED). **P0 (WS-9-collapse) DONE** `7436d1bef` — canonical owner_id = m1-test `009afc8c` (xian content absorbed; xian residual = audit_logs only). **P1 DONE** (`166efb6f1` — ConnectorConfig model + migration `000baa96d800` applied; table live; 7 tests). **▶ P2 (repo + service) NEXT** — mirror `services/repositories/document_repository.py` (async; `__init__(session: AsyncSession)`; get/upsert + the `_as_uuid_or_none` graceful-parse helper). Build `ConnectorConfigRepository.get(owner_id, connector)` / `.upsert(owner_id, connector, config)` and `ConnectorConfigService.get_default_repo(owner_id)` / `.set_default_repo(owner_id, value)` — a **drop-in** for `UserPreferenceManager` (`services/domain/user_preference_manager.py:877/885`, both async, `owner/name` string) so P3 can swap callers. github config key = `default_repository` in the JSON blob. TDD via the doc-repo's async-session test fixture. → **P3** wire the 3 readers (`repo_resolver._resolve_from_user_default` @ `repo_resolver.py:300`; standup `morning_standup.py:269` + `standup_workflow_skill.py:545`; `github/config_service`) to the service + backfill `data/github_preferences.json` → DB → **P4** honest-degrade + retire flat/in-memory → **P5** close #1226+#1199. #1185 stays parked (sibling). **DB↔model drift** spawned as a separate task (autogenerate unusable until reconciled).
