...
- **▶ NEXT: #1317 github inc.3 — `resolve()` via `MCPClient`.** Wire `GitHubMCPSpatialAdapter.resolve()` through `MCPClient.connect_stdio` (binding's server ref) → `ResourceHandle | ResolveMiss`; fold #1230 (resolution) + #1231 (honest-degrade) + #1229 per-connector cleanups. **Buildable+tested NOW against a FastMCP fixture** (like inc.2). **github-mcp-server provisioning** (stdio-local-process vs hosted-http) = the flagged infra decision — surface to PM when the live hookup is needed (the fixture proves resolve() logic without it).
