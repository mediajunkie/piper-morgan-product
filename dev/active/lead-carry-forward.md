...
**Updated**: 2026-06-14 19:0x PDT (Fire 26 — D1 +4 issues; design-floor UNBLOCKED (CXO specs); re-plan; continuing #1236)
...
- **Track B1 — #1090 Radar/entities slot-swap (CXO HANDED OFF 6/14 — BUILDING via full flywheel).** Mockup = binding spec (`dev/active/radar-entities-surfacing-mockup-2026-06-14.html`); PM-ratified + loved; #1090 now in **D1**. **#1090 is a scoping TRACKER** ("UI work lands as separate tickets") → flywheel issue-gate = **carve a focused implementation issue** (child of #1090) — **awaiting PM nod** (board op). Build: slot-swap into `templates/components/history_sidebar.html` (739L); Part-B Card (`cards.css`/`insight_card.html`); attention-first; entity-search-subsumes-chat; two states (real-only / empty+1 example); honest provenance (#1214/#1216). **PPM dep**: entity-catalog (#706) backend DOESN'T EXIST (only MUX design catalogs) → build surface + wire conversations (#1021 `user_history.py`) + home Radar modules; slot WorkItem/Person/Document as PPM lands the model. **Closure gate (CXO)**: AC must cite PDR-002 Layer-2 + the mockup. Flywheel: carve issue → gameplan+audit (DDD) → prompts+audit (if fan-out) → TDD → close-properly. **Progress**: ✅ **#1236 carved** (issue gate; D1/In-Progress/child-of-#1090); ✅ **gameplan written + self-audited** (`dev/2026/06/14/1236-radar-gameplan.md`; DDD = RadarEntity/EntitySource/RadarFeed). ✅ **Phase 0.5 contract read** (sidebar = client-JS-fetch → Radar render = JS + server endpoint; #1021 lacks lifecycle/open-questions → v1 derives lifecycle from recency + honest meta). ✅ **Phase 1 domain DONE** (TDD 8/8 green): `services/radar/` (models/sources/feed — RadarEntity/EntitySource/ConversationEntitySource/RadarFeed). **NEXT: Phase 2** — `/api/v1/radar` endpoint (RadarService wiring) + JS render in `history_sidebar.html` (feature-flagged) + render + wiring tests. Build the render as a **FRAME-AGNOSTIC component** (F2 #1171 re-homes it into the shell's aside — don't double-build the frame, per CXO coherence). **Fold #1164** (preserve + wire the privacy toggle to #1021's privacy API) — same file.
- **Track B2 — design-floor #1169–1173 (UNBLOCKED 6/14 — CXO specs delivered: `dev/active/design-floor-component-specs-2026-06-14.md`).** Lead builds, full flywheel + DDD + TDD. **CXO build order: F3 #1172 (token-lint — BUILD FIRST, locks the token vocab F1/F2 build against; radius convergence folds in) → F2 #1171 (page-shell) → F1 #1170 (Dialog) → C1 #1173 (chat-page conformance; real `template.render()` not curl-200).** F3/C1 build-ready; **F1/F2 need a ~20-min primitives-sync with CXO** (Dialog API shape `dialog.open({title,body,actions,dismissible})` + page-declaration mechanism — my call, their input). **⚠ COHERENCE (CXO)**: F2 page-shell = the start-screen app-frame = #1090's home frame; **#1236's Radar = the L2 aside IN that frame** → sequence so F2 + #1236 don't double-build the frame. My handling: #1236 Radar = a **frame-agnostic component** (fed by `/api/v1/radar`) that F2 re-homes into its aside slot.
- **D1 additions (PM moved from M5, 6/14): #1048, #1202, #1164, #1184** — triaged:
  - **#1202** FILES-FOLLOW-ON (artifact export md/pdf/docx + share-link + version history + tagging full release; split from #313) — **Lead, substantive → flywheel**.
  - **#1164** HISTORY-PRIVACY-TOGGLE-STUB (the slide-out "Start private session" toggle is UI-only; `handleHistoryPrivacyToggle` @ home.html:1918 is a console.log stub; wire to the existing #1021 `PATCH …/privacy`) — **Lead, small bug; OVERLAPS #1236's file** `history_sidebar.html` → fold into the #1236 sidebar work (design Q: where the toggle lives in the Radar surface — mockup doesn't show it; flag CXO).
  - **#1184** ARTIFACT-RENAME-FORMAT (#355 follow-on: rename via `PATCH /artifacts/{id}` + save/export format choice) — **Lead, small-med**.
  - **#1048** MUX-INSIGHT-STAGE-VISUAL (a **DESIGN QUESTION** — stage-specific visual treatment for the Insight Journal; CXO/design decides, then maybe a small Lead build). labels: question/UX.
