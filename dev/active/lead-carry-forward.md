...
- **🧭 Ports thread is at a clean boundary (2 connectors = rule of two):**
  - **Live path for github + calendar** = gated on the **provisioning A/B decision** (memo `64f3bfaec` to PM/Arch). Don't pre-build the OAuth callbacks (inc.2) until ratified.
  - **Connector #3 (notion; "Slack can wait" per PM)** = the trigger for **#1323** (extract a shared `BindingBackedConnector` mixin — rule of three; refactor github+calendar onto it + verify the m-41 guard's conformer-discovery handles a mixin). This is a *deliberate* refactor of just-shipped live code, NOT to be rushed; do it as connector #3's first step.
  - Other adapters (linear/cicd/gitbook/devenvironment) un-ported — porting them pre-#1323 would add copies before the dedup; do after the mixin.
- **▶ NEXT options (pick by what's unblocked when resumed):** (a) **provisioning decision lands** → wire the github (then calendar) OAuth-callback binding-creation (inc.2) + the concrete #1230 resolution against the real server; (b) **#1323 mixin extraction** + notion port (connector #3); (c) a different chunk if PM redirects. #1322 (legacy sim-transport cutover) is also unblocked-ish but needs real servers to migrate query_router onto.
