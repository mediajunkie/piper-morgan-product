...
- **▶ NEXT (unblocked, context-coherent, NOT premature)**: replicate the proven binding-aware + honest-degrade **rail** to the next connector (calendar → notion → slack) per gameplan inc.5 — the rail is structural/ADR-ruled (#1232 + #1231), so it's safe to replicate before the live path is proven (only each connector's LIVE resolution is provisioning-gated, like github). OR #1230's dead default-project-path cleanup (pre-prod cut-clean). Check each connector's adapter for `IMPLEMENTS_CONNECTOR` first (calendar may need full 4-method wiring, not just a rail).
