...
- **✅ PM RECONCILED A (6/27 ~09:30): "hosted OAuth it is" — A in principle for alpha**, nominal cost accepted (Copilot cost IS real — each tester draws on their Copilot/credits; watch-item: flip to self-hosted C if alpha needs zero tester-cost). Self-hosted C = intended future (Mac Mini incoming); `connect_http` makes A→C a contained change. **github inc.2 is GO.** (decisions.log `19d4aa928`.)
- **🔑 TOKEN-CUSTODY MODEL (flagged to Arch `0c6a09b5d`, proceeding on it):** MCP-client OAuth = Piper holds the MCP-*session* OAuth grant (access+refresh) — unavoidable for an OAuth client (SDK `TokenStorage`; mirrors existing calendar OAuth). D3-clean (session token ≠ raw PAT). **Binding stays token-free by REFERENCING a #358-encrypted secret** that holds the grant. Arch's "no token touches Piper" precised to "no raw vendor PAT." Hold the secret-storage wiring for Arch's nod; connect-redirect + callback-route parts are non-blocked.
- **inc.2 plan (mapped):** connect()=OAuth redirect-orchestrator (returns ConnectRequired w/ connect-URL action_hint) → web callback route → SDK `OAuthClientProvider` (PKCE; verify DCR vs register a Piper OAuth app against `api.githubcopilot.com`) → store grant in #358 secret → create #1229 binding (status=bound, refs the secret) → github adapter `_mcp_client_ctx` switches stdio-stub → `connect_http("https://api.githubcopilot.com/mcp/")`. Existing template to mirror: `services/integrations/calendar/oauth_handler.py` + the `/setup/calendar/oauth/callback` route.
