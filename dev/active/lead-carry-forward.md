...
- **🚦 THE GATE is now PM's ONE business checkpoint** (Arch handed PM): any cost / Copilot-seats / data-policy blocker on `api.githubcopilot.com`? **If PM says no-blocker → github inc.2 is GO.** No-blocker default; B only as an explicitly-temporary single-user dev affordance if PM hits a hard blocker (production is A either way, per D3).
