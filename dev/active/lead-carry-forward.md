...
Executable plan + proof on **#1322 comment 4827173746**. The sim transport (`PiperMCPClient simulation_mode=True` / `MCPProtocolClient` / `MCPConsumerCore`) is DEAD CODE (chat GitHub = real REST always; sim reached only via an unserved `__main__` demo). **Increment 1 DONE** (`d61987a70`: removed `query_router_spatial_migration.py` + 8 federation/pm033c integration tests). **Remaining (delete → full suite → commit, each its own increment):**
  - **inc.2**: `query_router.federated_search` (now caller-less post inc.1; it's the last method in the file) + the dormant `self.mcp_consumer` wiring (lines ~30 import, ~58 param, ~98/109 init — keep `github_adapter`, it's the real connector) + delete `tests/integration/test_pm033c_mcp_server_config.py`.
  - **inc.3**: `github_adapter` sim methods — `list_issues_via_mcp`, `connect_to_mcp`, the `self.mcp_consumer` attr + #1088 demo-fallback (+ `tests/unit/services/mcp/consumer/test_github_adapter_demo_fallback_1088.py`, `tests/integration/test_mcp_consumer_demo.py`).
