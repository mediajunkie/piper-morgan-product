...
- **NOT ratified as proposed — split, not uniform**: Lead's memo treated two "registry-category artifact" corpus rows (`create_issue`, `meeting_time`) as the same fix. They aren't: `create_issue`'s QUERY-filing is a real artifact (and symptomatic of a wider ≥4-instance mutation-under-read-only-category pattern — gated on whether category is ever a downstream safety signal before touching the corpus). `meeting_time`'s QUERY-filing is a **deliberate, cited decision** (#589, `pre_classifier.py:382,385-386` — calendar lookups are reads) that the corpus's TEMPORAL expectation disagrees with; re-expressing it would silently override #589 without anyone actually ruling on it. Sent back to Lead split: `mailboxes/lead/inbox/ruling-arch-to-lead-cc-pm-phase1-condition-1-ratified-condition-2-split-not-uniform-2026-08-14.md`. **Watch for Lead's reply** — either the downstream-safety-signal check on `create_issue`, or an explicit #589-citing ruling on `meeting_time` (not a blanket "artifact" label).
...
| Cron | **`27 6,9,12,15,18,21`** (6×/day, offset :27), job **`ee52b1fd`**, re-armed at 08-13 STOP via delete-then-create (prior `1ec65cec` → `3e79e19a`, the latter re-armed post-reboot 08-11 — see `docs/handoff-arch-2026-08-11.md` §1 for the reboot story). **Session-only — dies with the session.** Empty `CronList` → re-arm this expression |
