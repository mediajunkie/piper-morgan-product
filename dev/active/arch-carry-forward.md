...
| Cron | **`27 6,9,12,15,18,21`** (6×/day, offset :27), job **`8076ada0`**, re-armed at 08-26 STOP via delete-then-create (prior `7d306060`). **Session-only — dies with the session.** Empty `CronList` → re-arm this expression. Full reboot/park story (macOS 26.6, 08-11): `docs/handoff-arch-2026-08-11.md` §1 |
...
- ✅ **Phase 2.2 flip-1 named-WRITE question RULED 2026-08-25 — #1684 filed, ruling posted to #1677 + mailed to Lead.** Lead asked (mail) whether `create_todo` could flip individually through flip-1's `EffectClass.READ` guard, ahead of the write wave, framed as "WRITE-not-DESTRUCTIVE, no confirm tier at stake." **Verified before ruling, same discipline as #1663/#1633**: that framing was false — `create_todo` has **no `WorkflowEntry` at all**, the identical gap #1666 found for `delete_todo`, so it isn't rail-dispatchable via either router today regardless of the guard question. Filed **#1684** to close it (independent prerequisite, do regardless of the rest). The consent-gate-untouched-either-way claim WAS verified true (`intent_service.py:2161-2197` is the single convergence point for both routers). **Ruled**: yes, a named WRITE can flip individually, but as an explicit individually-reviewed allowlist (both `inversion_live.py:463` dispatch check + `workflow_dispatcher.py:149-157` constructor guard updated together) — never a blanket `EffectClass` relaxation, since the guard's specificity is what caught the real `create_issue`-filed-under-QUERY bug (#1663) in the first place. **Also caught**: Lead's own 08-22 comment on #1677 (the forcing issue) had named a third option — a deterministic pre-classifier pattern, Lead's own "strongest fix" — that Lead's mail to me had omitted from the framing. Flagged that PM's #1677 triage should see four options, not the two/three presented. Not blocked on me now — watch for PM's sequencing call on #1677 and whether #1684 lands before any flip-1 wiring.
