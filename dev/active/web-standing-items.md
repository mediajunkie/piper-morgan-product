...
| 4 | **FTUX interview render-check** (CXO ask, PIPER_FTUX_INTERVIEW live) | 2026-09-07 | **Hold for** Lead to provision a genuinely cold (never-onboarded) test account — self-serve `/register` is pruned per #1504, and the existing browser-lane test account from 08-29 has seed data/chat history/a bound connector, so it's not cold anymore. Not Web's to unblock; ready to run the moment a fresh invite-token account exists. When it does: log in, capture the first exchange verbatim (user turn, Piper's reply, connector-bound state), report whether the reply leads with CXO's opening line and whether it asks the question at all. No urgency stated by CXO. |
