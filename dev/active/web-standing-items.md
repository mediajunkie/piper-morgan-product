...
| 3 | **Buttondown** — ⬆️ **sub-task DELIVERED 2026-09-20**: PM's live ask inside the hold (*"make sure the options we offer at signup align with what we offer today"*) is **audited and sent**. Finding: the gap collapses to one PM-only question — **does anything actually get sent to Buttondown subscribers?** No send mechanism in either repo; runbook step 9 is the *LinkedIn* newsletter; signup collects fine. Also: cadence is ~4.9 posts/week vs "weekly" copy; "early access" points at the separate /try/beta waitlist; "576+" hardcoded twice and unverifiable (no Buttondown credential). **Waiting on PM's answer**; copy fix follows from it. The migration itself remains on hold. | 2026-08-15 | **Hold for** PM research; not Web's to close (explicitly long-term/not-urgent). Publish blog posts natively to Buttondown, possibly with subscriber choice (blog vs. Ship) — Buttondown may not support that granularity without multiple newsletters. **2026-08-31**: PM doing research when back at desk. |
| ~~4b~~ | ✅ **CLOSED 2026-09-20 — website#43 SHIPPED** | 2026-09-19 | `public/assets/blog-images/source/` (240.7 MB / 172 files = 74% of deployment payload, publicly served, needed by nothing at build or publish time) moved to `assets-source/blog-images/`; `SOURCE_DIR` updated in six scripts. **Move, not delete** — 172 basenames have **zero overlap** with the product repo's `drafts/{published,images-archive}/` (202), so these are the only copies in git. ⚠️ My original *"per website#37"* justification was **wrong** (different repo + directory, closed since 09-12) — corrected on the issue and to PM before either acted. PM: *"100% approve moving those huge pngs so they do not get deployed, ASAP!"*, explicitly *don't hold for measurement*. Shipped `300c89cd`, deploy success, live-verified: source PNG **404** (was 200/3.84 MB), served webp still **200**, `/blog` + `?page=16` render **0 unloaded images, 0 console errors, 0 4xx**. **Bracket for the storage reading**: commit `300c89cd`, landed **2026-09-20 09:09 PDT**. **No storage claim made** — dedup behaviour is dashboard-gated and unverified. |
| 4 | ⬆️ **Q1 ANSWERED 2026-09-19** — "100% of the **10 GB Deployment Storage** free tier," found in Exec's hosting-tasking memo **cc'd to me about something else**, 10 days after filing. Still open: what happens at the ceiling, and whether Vercel dedupes across deployments — both dashboard-gated, same access blocker. Actionable half is row 4b. Original filing follows. | 2026-09-09 | PM saw a Vercel plan/usage warning and wants it understood before it becomes an outage (Q2 already resolved by Exec: the beta app runs on Fly, not Vercel, so blast radius is one property). **Hold for** either the actual warning text/screenshot from PM, or Vercel dashboard/API credentials — checked thoroughly first: no `vercel` CLI, no auth token, no `~/.vercel/auth.json` in this environment, and no existing plan/usage documentation in either repo. Not a guessable answer; genuinely can't determine which resource/how close/what happens at the ceiling without one of those two things. Explicitly "no rush... next-working-day," per PM's own framing relayed by Exec. |
