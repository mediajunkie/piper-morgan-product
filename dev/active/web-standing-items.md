...
| 4b | **Vercel payload — website#43, fix ready, held for PM** | 2026-09-19 | `public/assets/blog-images/source/` = **240.7 MB / 172 files = 74% of deployment payload**, publicly served (verified live: `200, 3,841,557 bytes, image/png`), needed by **nothing** at build or publish time (six manual scripts, none wired into `prebuild`; no `.vercelignore`). Proposal: move out of `public/`, update `SOURCE_DIR` in the six scripts — **move not delete**, per website#37. **Held deliberately**, not deferred: PM may be actioning retention concurrently and two changes to one system make before/after unattributable; and the storage win depends on Vercel dedup, which is dashboard-gated. **Told PM I ship on the correctness argument alone if no reply** — serving build inputs from prod is wrong regardless of dedup. |
