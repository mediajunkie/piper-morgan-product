...
| 44 | "Silent State Mutation in Shared Working Tree" — parent meta-pattern | Code agent May 10 staging-race memo; HOST May 10 concurrence on shelf-placement | Subsumes branch-drift (Lead Dev May 9 #5 / Pattern Sweep P-13), index-drift (today's PPM-stranded staging-race), residue-drift (P-16 candidate Cross-Agent Residue Accumulation). Common shape: shared `.git`/index + concurrent agent activity → silent mutation of "stable-looking" state. | **Pending CIO judgment**: candidate parent pattern for next Pattern Sweep. HOST stance: name the parent for cohort vocabulary; don't codify as discipline gate (tolerated-risk acceptance). Capture-tier: file as Emerging candidate when next sweep runs; promotion to Proven would need ~2 more sub-instance recurrences across the three named children. |
| 45 | "Coarse Triggers Causing False-Positive Triage Cost" — hook-design meta-pattern | Code agent May 10 PreCompact-hook second-incident addendum; HOST May 10 concurrence on shelf-placement | Distinct from triggering-failure-mode patterns (062 family) — failure is in *how mechanism weights what it detects*, not *what it detects*. Two-fire-one-day example: PreCompact hook (correct catch on Docs stranded log first time; false-positive triage on PPM local-CLI same-tree session second time). | **Pending CIO judgment**: candidate proto-pattern. HOST stance: worth naming, CIO call on proto-pattern vs. tactical observation. Capture-tier: smaller surface than #44; may stay as tactical observation rather than pattern. Decision: hold for one more incident before filing. |
