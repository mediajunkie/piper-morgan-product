...
|---|---|---|---|
| **2026-08-30** | **#1463 both arms (GPT + deconfounder)** | 🔴 **STILL BLOCKED — OpenAI key returns `insufficient_quota`.** ⚠️ *I marked this UNBLOCKED at ~13:2x on PM's report of a top-up; PA tested the credential live twice, an hour apart, and it still fails. Authorizations are fine — the credential is not.* **Diagnostic put to PM**: check the billing page directly, which distinguishes propagation delay from a top-up posted to a different org/project. | A live API call succeeds. PA runs both immediately then. |
| **2026-08-30** | **#1463 two-call deconfounder** | ✅ **PM AUTHORIZED 08-31 ~13:3x** — *"agreed run it together"*. Relayed to PA as PM's decision, on the durable surface. ⚠️ **Not "too small to need approval"** — size isn't the criterion, scope is. And not mine to run (subject/scorer confound). | Rides the GPT arm. PA asks. |
| **2026-05-10** | **CT v2.4 → `context_requirement` corpus tag** | **Lead** — PA has ruled themselves out factually (their probe corpora are per-probe, hand-built, stood down; not #928's). ⚠️ **"Nobody, currently" is a live possible answer** — the tag job may sit behind an ownership vacuum rather than a routing question. Tag semantics are mine either way. | Ownership answer. |
