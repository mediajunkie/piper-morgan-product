...
|---|--------|-------|-----------|
| arc-1 | **Design arc — "not being bad" track** | **Standard v0.1 authored + delivered to Lead** (`design-system-and-conformance-standard-2026-06-07.md`). Forensic finding: tokens.css = complete WCAG-AA system → enforce-not-build. NEXT: 20-min primitives-sync w/ Lead (Dialog API + page-shell) when he surfaces from #1124 Phase 3 → then Lead executes floor-defect map (chat-page first). PM-decision still open: chat-page urgency vs Lead's #1124 lane. | Lead (review + primitives sync; he's mid-#1124) |
