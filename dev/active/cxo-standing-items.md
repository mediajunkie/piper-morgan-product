...
| **2026-09-09** | **Acceptance contract — CXO pass delivered** ⭐ **Structural correction**: the ask (`decide_consent`) scales on **effect AND outwardness**; the proposed acceptance predicate scaled on effect alone, so `OUTWARD/WRITE` collapses to the same bar as a private write — the exact case the 08-15 axis was ratified to prevent. **Ruled question-forms**: a different speech act, not a failed acceptance — answer, restate the arm, never re-prompt. **Per-verdict acceptance table** anchored to the four existing `ConsentDecision` verdicts, no new vocabulary. **Flagged arm-lifetime as a QUESTION, not a finding** — did not verify whether arms drop on topic change. | **Lead** — builds after my pass + Arch's (Arch's is in). | Lead builds, or comes back on the outwardness axis / arm lifetime. |
