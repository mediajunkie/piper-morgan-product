...
|---|---|---|---|
| **2026-09-01** | **#1463 class-discriminator test (2 calls)** — item 3's payload carrying BOTH a class-A caveat (cached read) and a class-B caveat (`coverage: partial`), one question. Holds → staleness survives, completeness vanishes, same reply. Both vanish → I'm wrong again. | **PM** — needs its own authorization; deliberately not folded into the run already approved. **Not asking yet**, flagged as cheap and available. | PM's word. |
