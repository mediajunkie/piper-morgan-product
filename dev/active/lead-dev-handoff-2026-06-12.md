...
- **Server disposition at session close (CIO tail-item, 17:25)**: PID **95175**, started **11:42 6/12**, running detached from THIS worktree with Slack inbound connected (confirmed in `/tmp/piper-server.log`). **KEEP IT RUNNING** across the migration — it's what keeps PM's Slack DMs answered. ⚠️ It predates the afternoon commits (notably the #1188 humanizer fix — server code), so successor: **restart it env-stripped from your own working location when you begin work** (picks up the afternoon's code); until then the stale-but-healthy instance is the right trade. Kill cmd if ever needed: `kill 95175` (verify port owner via `lsof -ti :8001` first — pkill-by-pattern missed it once, 6/12 lesson).
