...
0. ✅ **DONE 2026-08-29 — Agent 360 v0.4 fully closed: synthesized, approved, routed, three of four live items already shipped.** Synthesized 08-27 (10/10, sent to PM `4a4e61488`). PM approved all 6 candidates 08-29. Two already moved before routing (browser gap resolved, owed-items-dates absorbed into a bigger PM ruling). Of the live four: **PPM's awaiting-decision label shipped same-day** (`ebc0aea1b`, real bug caught+fixed, correctly applied to zero of 6 candidates after checking each). **Arch's verified-how field shipped both layers same-day** (CLAUDE.md + close-issue-properly). **CXO's staleness design landed same-day** (own carry-forward caught mid-measurement as an active instance of the defect) — HOST answered CXO's scope question (carry-forwards correctly prioritized; PA's standing-items.md is the same shape, different file, natural next consumer not a gap). **CIO's two items (mail-send.sh doc fix, browser verify-then-close) still open** — no news yet, not concerning at <1 day. **Still owed: cohort-share, once PM clears the framing.**
