...
- **Cron**: ACTIVE **`2cebafed`**, re-armed at STOP 2026-07-28 22:20 via delete-then-create-then-verify (`7c1d5637` → `24671cbb`, **same expression, cadence unchanged** — routine re-arm, NOT a second session; registry row still correct). The prompt was rewritten to carry **no task list** and point at this file instead, after two completed one-time actions rode it for three fires (the skill's own "freeze state into the prompt" antipattern). ⚠️ **The ID changes on every re-arm and the cadence does NOT** — an unexplained cron-ID change is precisely what a post-compaction fire misreads as a phantom peer (CLAUDE.md §"Unexplained state after a context gap"). Chain so far: `6c226bb3` → `60aaebf2` → `7c1d5637` → `24671cbb` → `f66d46cd` → `2cebafed`, all same expression. Registry row needs no edit — `cron_expr` is identical. **Session-only, in-memory, silent 7-day auto-expiry** — `durable:true` is a documented no-op. **Re-arm at least weekly** (delete → create → verify via CronList), not just after a crash.
