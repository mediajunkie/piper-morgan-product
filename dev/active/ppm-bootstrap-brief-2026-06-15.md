...
6. **Cron â cohort-standard CronCreate windowed cron.** â ï¸ The scheduled-task approach is **SUSPENDED** (it spawned concurrent *fresh* sessions that interleaved with the live one â persona fork; PM-rejected 6/14; see `docs/operations/duty-cycle design/scheduled-task-gap-c-cure-2026-06-14.md`). Use the cohort-standard CronCreate **daytime** windowed cron: **`52 6,9,12,15,18,21 * * *`** (06:52 START Â· daytime fires Â· 21:52 STOP; offset :52 is your per-lane slot), `durable:true`. CronCreate prods **THIS** session (no fork). Known limitation: it dies on a session resume (the freeze risk) â if your cycle goes quiet, PM re-prods; a proper *wake-this-session* watchdog is being designed cohort-wide â UPDATE: it is now LIVE (the launchd freeze-registry, shipped 6/15-17; alerts PM on a silent freeze incl. the closed→never-restarted overnight-dormancy case). To be watched, add your row to `dev/active/duty-cycle-registry.tsv` (role, cron_expr, threshold_h, wake_start/end, first_fire) once your windowed cron is settled. Prompt CONSTANTS must embed the windowed expression (the self-heal re-arms from the prompt).
