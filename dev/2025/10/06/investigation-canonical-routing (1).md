...
### 0. Check ADRs and Patterns (ADD THIS FIRST)

Review architectural decisions and patterns:
```bash
# Look for decisions about canonical handlers or dual routing
grep -i "canonical\|routing\|intent" docs/adrs/*.md
grep -i "simple.*complex\|fast.*path" docs/adrs/*.md

# Check patterns documentation
grep -i "canonical\|handler\|routing" docs/patterns/*.md
```

Look specifically for:
- Any mention of dual-path architecture
- Decisions about simple vs complex query handling
- Patterns for fast-path responses
- Intent routing strategies

If ADRs exist about this, they take precedence over code investigation.
