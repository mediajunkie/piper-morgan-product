...
*v1.5 — **Fixed a self-contradiction in the Common Updates section (2026-08-26, Docs).** The Field Reference table always said `canonicalSite=distributed` means "on blog + syndicated," but Common Updates instructed setting it at blog-first publish — before syndication. This is the root cause of both directions of #1683: the 2026-07-19 migration's 145-row undercount (rows genuinely fully-synced but never selected because `canonicalSite` was never reliably set at the right moment), and a same-day fresh instance on Weekly Ship #057 (published this skill's own prior instructions, `canonicalSite` set to `distributed` hours before its LinkedIn leg actually ran, caught by Dispatch-PM). Moved the `canonicalSite→distributed` action from the blog-first-publish bullet to the cross-post bullet, where it matches the field's own definition. Historical rows are NOT touched by this fix — see #1683 for the separate, deliberately-not-rushed remediation of existing data.*
