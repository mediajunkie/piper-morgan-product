...
version: 1.39
created: 2026-06-06
changelog: "Full history: docs/internal/operations/duty-cycle-tick-changelog.log (v1.0-present). Most recent: v1.39 (2026-09-23) — **Rule 1's cron delete is now book-ended, not vigilance-dependent.** Lead's 09-22 STOP was missed because the drain-protection delete's re-arm half depended on remembering to act at an idle moment that never arrived; PM ratified the framing (\"the delete and the restore are ONE obligation, not two\") and Lead's mechanism (a one-shot CronCreate at the STOP slot, created in the same breath as the delete, so the day-close survives even if the agent never gets back to idle to re-arm normally). Also records that CronCreate's own docs confirm jobs only fire while the REPL is idle -- the interruption hazard Rule 1's delete defends against may not exist at all on this runtime -- named as an open question, not resolved in this edit."
---
...
- **Cron — ONE rule (it's your idle self-wake, nothing more)**: keep it **ARMED by default**, *including throughout PM conversation*. It can't fire while the REPL is busy, so armed-during-work-or-convo is harmless; armed means it re-wakes you the **moment** you go idle — which is the whole point. (This is why "arm only on reaching idle" is wrong: if the session backgrounds *during* work or convo, you'd have no armed cron to self-wake — a stall. Armed-by-default closes that.) **The only time you DELETE it: while you're actively draining genuinely-substantive multi-step work** (so a queued fire can't interrupt mid-build) — then **re-arm the instant you return to idle** (incl. end of STOP), where "re-arm" always means **delete-then-create** (`CronList` → `CronDelete` the existing job → `CronCreate` fresh → `CronList` to confirm exactly one survives), never a bare `CronCreate` assumed safe because "the old one probably already died" — at STOP specifically it almost certainly hasn't (see STOP's re-arm step above; this is the same instruction, not a different one). A **pending PM question never deletes the cron and never blocks other unblocked work** — hold only the thread awaiting the answer; drain everything else. A trivial one-line fire needs no delete. *(This collapses the former Rule-1/Rule-2 into one: armed-by-default, deleted only mid-substantive-build, re-armed at idle. PM-ratified shape 2026-06-06; unified 2026-06-23 per the fire-as-timebox structural pass.)*
  ...
  ⚠️ **The delete is only valid BOOK-ENDED — PM-ratified 2026-09-23, from a real miss.** Lead deleted the recurring cron correctly at drain start (09-22), but the drain ended with the conversation going idle mid-evening with no ritual moment to hang the re-arm on — vigilance-dependent, and it failed. No cron → no 21:17 STOP fire → no overnight wakes → caught only the next morning by Step 0's retroactive-close backfill (which worked exactly as designed, but is the backstop, not the primary mechanism). PM, verbatim, on the framing to lead with: *"Rule 1 only works if it is book-ended by restoring a cron when finally ready to go idle."* **The delete and the restore are ONE obligation with two halves — a delete that isn't book-ended is not a valid invocation of this rule, it's an unarmed session.**

  **Mechanical form of the book-end (Lead's proposal, adopted)**: when you delete the recurring cron to protect a drain, in the *same breath* `CronCreate` a **one-shot at today's STOP slot** (`recurring: false`, cron pinned to that slot's minute/hour/day/month, prompt = your normal tick prompt). This makes the day-close structurally unloseable by a drain that never gets an idle moment to re-arm from: if you remember to re-arm normally, the one-shot is superseded by the delete-then-create ritual as usual (harmless, idempotent — the STOP branch already checks "not yet STOPped today"); if you don't, the one-shot still fires at the STOP slot and the flywheel picks the day-close up on schedule instead of losing it until next morning's backfill. If the drain runs past the STOP slot, the one-shot fires as the next-turn wake and STOP happens at first idle — still strictly better than never.

  **Why the underlying hazard may not exist at all, named but not yet resolved**: `CronCreate`'s own documented runtime behavior is *"jobs only fire while the REPL is idle (not mid-query)"* — confirmed 2026-09-23 directly from the tool's own description, not inferred. If a queued fire structurally cannot interrupt an in-progress turn, the delete-during-drain may be defending against a hazard the runtime already prevents, at the real cost of exactly the failure above. Lead's own framing: *"I'd happily drop to 'leave armed always, swap-to-one-shot never needed' if you conclude the interruption hazard is imaginary; the swap proposal is the conservative amendment that doesn't need that conclusion."* **Shipping the conservative book-end now, not resolving the deeper question in the same edit** — removing the delete entirely is a bigger behavior change than this fix needs to make, and deserves its own scrutiny rather than riding in on an incident fix.
