...
- ✗ **FAIL**: "the person who applied it went and checked" describing Docs, who applied and then verified a config change. Fix: "the agent who had applied it." Same piece, same audit pass, also had "checked by more than one person" for two agents (Docs + CIO) reviewing it — fix: "more than one agent." Two instances, one piece, both missed at the piece's original 2026-08-18 draft-time audit (see version-drift note below).
...
⚠️ **REPORT EVERY MATCH WITH AN EXPLICIT VERDICT — a holistic "clean" summary is not a checkable claim (2026-09-26, closing a real live miss).** "A Fix Needs the Same Rigor as the Claim It Fixes" published 2026-09-23/26 with two uncaught FAILs ("five more people," "four different people," both describing agents). Root-caused to two distinct mechanisms, not one — report whichever applies, don't default to the more comfortable one:
  1. **Version drift**: that piece (and "Described Is Not Running," same defect, same day) was drafted 2026-08-18, two weeks before this check existed (2026-09-01). Nothing re-sweeps a piece already marked clean from an earlier audit pass just because the audit gained a new check — a stale "clean" from before the check existed reads identically to a fresh clean.
  2. **Judgment miss on a live check**: "Three Silent Failures Became One Law," drafted 2026-09-18 — a full three weeks *after* this check existed — was audited that same day with the holistic claim "agent-as-'people' sweep clean (all legitimate generic/human uses)." That claim was wrong: one instance ("without someone doing the unglamorous work," describing Docs by name two paragraphs earlier) was a live FAIL the auditor read and passed.
  Fix, both mechanisms: **list every match with its own line + verdict + one-clause reason**, the same per-item ledger discipline `continue-narrative` uses for exactly this failure shape (an aggregate "rich everywhere"/"clean" claim hides a per-item miss that nothing downstream can check). "Zero matches" is a real, statable clean pass; "N matches, all judged PASS, here's each one" is a real, statable clean pass; "sweep clean" alone is neither — it's unfalsifiable. On any piece that's sat in the queue since before this check's own most recent version, re-run it explicitly rather than trust the original pass's summary line, whatever check number it cites.

⚠️ **THE FAILURE IS BIDIRECTIONAL — PM, 2026-09-26, sharpening the "why."** *"For reasons of agency and accountability, I feel it's important we not get sloppy about saying a human person did some thing when an agent did, and vice versa."* This check's grep only catches one direction (agent described as person). **The mirror direction — a human's actual action credited to an agent — is not keyword-detectable at all**; it requires cross-checking the claim's actual attribution against the primary source that recorded who did what, which is a fact-checking discipline this project already runs (see `[[feedback_primary_log_can_misattribute_a_named_person]]`), not a grep this check can absorb. Don't claim this check covers both directions — it covers one; the other stays a manual verification call.
...
*v1.15 — 2026-09-19. **Check #11 upgraded from style guidance to ruling, and gains `\bperson\b`.** HOST ruled (`decisions.log`, #1834, in response to PM's referral after Exec's own Ship-061 internal report called six named agents "people" in its thesis sentence): agents are never "person"/"people," in any register, published or internal — unlike "cohort"/"team," this isn't a tone call, because "people refused to lower the bar" asserts evidenced motive the agent's actual behavior doesn't show, where "a clearing condition held under pressure" is the accurate claim. The grep pattern was missing the singular "person" entirely — added it. This check is build-item 1 of #1834 (Comms owns it, since it already rides this exact chokepoint); build-item 2 (PM-facing internal reports) is Exec's/CIO's. Picked up same-day from HOST's memo rather than left as a standing item, since the fix was small, unblocked, and squarely in-lane. **(Frontmatter `version:` field was never bumped for this release — caught and fixed 2026-09-26, one small instance of the exact "a change landed but nothing marked the surface that would let a reader know" shape v1.16 exists to close at a larger scale.)***

*v1.16 — 2026-09-26. **Check #11 gains a mandatory per-match verdict requirement, and the bidirectional framing.** Docs found "A Fix Needs the Same Rigor as the Claim It Fixes" published with two agents-called-"people" instances (PM caught it post-publish). Prompted a proactive sweep of the entire current drafted/queued/ready-for-docs pool (not just the one flagged piece) rather than assume it was isolated — found 3 more real instances across 2 more pieces ("Described Is Not Running," "Three Silent Failures Became One Law"), all fixed same-day. Root-caused to two distinct mechanisms: **version drift** (both "A Fix Needs..." and "Described Is Not Running" were drafted 2026-08-18, two weeks before this check existed at all, and nothing re-sweeps a piece already marked clean from before a check existed) and a **live judgment miss** ("Three Silent Failures" was drafted 2026-09-18, three weeks *after* this check existed, and the same-day audit's own holistic claim — "sweep clean" — was wrong on one instance). The fix for both: report every grep match with its own verdict line, not one holistic summary claim — the same per-item ledger discipline `continue-narrative` already uses for the identical failure shape (an aggregate "clean" hides a per-item miss). Also added PM's 2026-09-26 bidirectional clarification (agent-credited-as-human is exactly as wrong as human-credited-as-agent) and named plainly that this check's grep only catches one direction — the other needs primary-source attribution verification, not a keyword scan.*
