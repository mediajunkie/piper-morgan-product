# Piper Morgan Roadmap v17.0 (DRAFT)

**Date**: May 30, 2026
**Author**: PPM, with leadership review (PA — §M5/BYOC + skunkworks; CIO — §Methodology; CXO — §Differentiator stack EC framework; Architect — §Architectural commitments AC framework; Lead Dev — §M2g + Phase 2 build; Comms — external-language frame pending)
**Status**: DRAFT — for cohort section review (PA, CIO, CXO, Architect, Lead Dev), then Docs swap into canonical `roadmap.md` after CEO ratification. Per v15→v16 precedent.
**Supersedes**: v16.0 (May 10, 2026) — to be archived at `docs/internal/planning/historical/roadmap-v16.0-2026-05-10.md` per the v15.0 archive pattern
...
**`[INPUT PENDING: PA — §M5/BYOC review — skunkworks BYOC PoC status + Klatch-pause / Daedalus / DinP-fleet cross-pollination detail]`**
...
- **Mechanism set** — persona-template parameterization + MCP-server packaging alongside FastAPI + RequestContext-based auth abstraction + audit envelope `host_id` field + context-package format negotiated with sibling projects
...
**Companion ADRs queued in Architect's lane**: Q6 (canonical context-package format; Klatch-pause noted) + Q7 (packaging-layer abstraction implementation).

**PA skunkworks BYOC PoC** at `mediajunkie/piper-morgan-skunkworks` exploring plugin/MCP/skills/PM API layering. Strategic-architectural lane stays with PPM + Architect; PoC is operational signal that may inform.
...
|---|---|---|
| **Outcomes** (rubric + grader + retry as API, May 6) | methodology-07/15/17 verification + audit-cascade skill + narrative-verification skill | **Investigation lane** (PA-leads + CIO-co-author, started May 25; findings memo target ~May 30) |
