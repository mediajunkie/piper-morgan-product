...
duty-cycle stall (STALE cio 85h (dyn-threshold 19h wake-window-aware; cron '7 10,16,22')). The cron object likely survives; the session needs a prod/resume to wake it.

- **Detected**: 2026-07-22 22:42:24 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: cio   ·   **all currently stale**: STALE cio 85h (dyn-threshold 19h wake-window-aware; cron '7 10,16,22')
