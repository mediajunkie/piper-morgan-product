...
duty-cycle stall (PARK-NO-EXIT arch — parked with no falsifiable clearing condition, so this row cannot go stale visibly. Reason on file: 'awaiting Amber migration (alerted 3x in 20h while known-dark)';STALE lead 5h (dyn-threshold 5h wake-window-aware; cron '17 6,9,12,15,18,21');PARK-NO-EXIT cxo — parked with no falsifiable clearing condition, so this row cannot go stale visibly. Reason on file: 'awaiting Amber migration'). The cron object likely survives; the session needs a prod/resume to wake it.

- **Detected**: 2026-07-27 12:46:04 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: lead   ·   **all currently stale**: PARK-NO-EXIT arch — parked with no falsifiable clearing condition, so this row cannot go stale visibly. Reason on file: 'awaiting Amber migration (alerted 3x in 20h while known-dark)';STALE lead 5h (dyn-threshold 5h wake-window-aware; cron '17 6,9,12,15,18,21');PARK-NO-EXIT cxo — parked with no falsifiable clearing condition, so this row cannot go stale visibly. Reason on file: 'awaiting Amber migration'
