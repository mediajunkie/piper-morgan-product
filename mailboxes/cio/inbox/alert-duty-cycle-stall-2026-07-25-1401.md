...
duty-cycle stall (STALE arch 145h (dyn-threshold 5h wake-window-aware; cron '27 6,9,12,15,18,21')). The cron object likely survives; the session needs a prod/resume to wake it.

- **Detected**: 2026-07-25 14:01:31 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: arch   ·   **all currently stale**: STALE arch 145h (dyn-threshold 5h wake-window-aware; cron '27 6,9,12,15,18,21')
