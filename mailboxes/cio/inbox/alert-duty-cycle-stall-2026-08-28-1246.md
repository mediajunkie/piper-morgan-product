...
3 duty-cycle roles silent at once (STALE cio 26h (dyn-threshold 13h wake-window-aware, ~2 missed fires; cron '7 10,16,22');STALE arch 26h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '27 6,9,12,15,18,21');STALE host 23h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '37 6,9,12,15,18,21')) — likely machine-asleep/backgrounded (cron-survives-doesn't-fire), not individual failures. One wake of the machine/app likely covers it.

- **Detected**: 2026-08-28 12:46:08 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: cio arch host   ·   **all currently stale**: STALE cio 26h (dyn-threshold 13h wake-window-aware, ~2 missed fires; cron '7 10,16,22');STALE arch 26h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '27 6,9,12,15,18,21');STALE host 23h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '37 6,9,12,15,18,21')
