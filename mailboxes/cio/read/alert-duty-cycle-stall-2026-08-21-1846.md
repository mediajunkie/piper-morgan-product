...
4 duty-cycle roles silent at once (STALE arch 8h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '27 6,9,12,15,18,21');STALE pa 8h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '42 6,9,12,15,18,21');STALE web 8h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '22 6,9,12,15,18,21');STALE docs 8h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '57 6,9,12,15,18,21')) — likely machine-asleep/backgrounded (cron-survives-doesn't-fire), not individual failures. One wake of the machine/app likely covers it.

- **Detected**: 2026-08-21 18:46:07 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: arch pa web docs   ·   **all currently stale**: STALE arch 8h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '27 6,9,12,15,18,21');STALE pa 8h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '42 6,9,12,15,18,21');STALE web 8h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '22 6,9,12,15,18,21');STALE docs 8h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '57 6,9,12,15,18,21')
