...
3 duty-cycle roles silent at once (STALE arch 11h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '27 6,9,12,15,18,21');STALE cxo 32h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '47 6,9,12,15,18,21');STALE pa 11h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '42 6,9,12,15,18,21')) — likely machine-asleep/backgrounded (cron-survives-doesn't-fire), not individual failures. One wake of the machine/app likely covers it.

- **Detected**: 2026-08-26 18:46:09 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: arch pa   ·   **all currently stale**: STALE arch 11h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '27 6,9,12,15,18,21');STALE cxo 32h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '47 6,9,12,15,18,21');STALE pa 11h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '42 6,9,12,15,18,21')
