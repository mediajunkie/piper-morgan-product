...
duty-cycle stall (STALE lead 18h (dyn-threshold 5h wake-window-aware; cron '17 6,9,12,15,18,21')). The cron object likely survives; the session needs a prod/resume to wake it.

- **Detected**: 2026-07-13 12:44:23 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: lead   ·   **all currently stale**: STALE lead 18h (dyn-threshold 5h wake-window-aware; cron '17 6,9,12,15,18,21')
