...
duty-cycle stall (STALE cxo 26h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '47 6,9,12,15,18,21')). The cron object likely survives; the session needs a prod/resume to wake it.

- **Detected**: 2026-08-26 12:46:10 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: cxo   ·   **all currently stale**: STALE cxo 26h (dyn-threshold 7h wake-window-aware, ~2 missed fires; cron '47 6,9,12,15,18,21')
