...
duty-cycle stall (STALE arch 19h (dyn-threshold 19h wake-window-aware; cron '27 8,20')). The cron object likely survives; the session needs a prod/resume to wake it.

- **Detected**: 2026-06-29 08:53:33 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: arch   ·   **all currently stale**: STALE arch 19h (dyn-threshold 19h wake-window-aware; cron '27 8,20')
