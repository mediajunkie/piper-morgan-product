...
duty-cycle stall (STALE exec 13h (threshold 6h; cron '32 6,9,12,15,18,21')). The cron object likely survives; the session needs a prod/resume to wake it.

- **Detected**: 2026-06-25 12:42:37 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: exec   ·   **all currently stale**: STALE exec 13h (threshold 6h; cron '32 6,9,12,15,18,21')
