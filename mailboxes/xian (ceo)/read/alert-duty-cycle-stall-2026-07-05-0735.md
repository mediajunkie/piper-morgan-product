...
duty-cycle stall (STALE arch 11h (dyn-threshold 5h wake-window-aware; cron '27 6,9,12,15,18,21')). The cron object likely survives; the session needs a prod/resume to wake it.

- **Detected**: 2026-07-05 07:35:25 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: arch   ·   **all currently stale**: STALE arch 11h (dyn-threshold 5h wake-window-aware; cron '27 6,9,12,15,18,21')
