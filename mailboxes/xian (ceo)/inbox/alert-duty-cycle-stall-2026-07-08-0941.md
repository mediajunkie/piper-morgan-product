...
duty-cycle stall (STALE exec 24h (dyn-threshold 19h wake-window-aware; cron '32 8,20')). The cron object likely survives; the session needs a prod/resume to wake it.

- **Detected**: 2026-07-08 09:41:48 (freeze-watcher hourly run); thresholds per `dev/active/duty-cycle-registry.tsv`.
- **Newly nudge-worthy**: exec   ·   **all currently stale**: STALE exec 24h (dyn-threshold 19h wake-window-aware; cron '32 8,20')
