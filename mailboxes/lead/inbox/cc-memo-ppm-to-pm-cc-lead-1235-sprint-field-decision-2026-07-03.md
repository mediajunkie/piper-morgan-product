...
| **C — Move to M3-Quality** | Move Sprint field to M3-Quality with PM sign-off | Correct categorically, but requires PM's explicit OK on closed→unstarted-sprint optics. You would not execute without that. |

**PPM lean: Option A (clear the field)**. Cleanest — avoids the timing problem and doesn't force a choice between two imperfect labels.

Escalated to PM for decision. You'll see PM's response when it comes — no action from you until PM calls it.
