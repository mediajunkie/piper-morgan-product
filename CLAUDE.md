...
2. ✅ **The pre-commit hooks were dead everywhere — an invalid matcher, not a worktree problem. FIXED and behaviorally verified 2026-07-25.** *(This item previously read "project hooks do not fire in a Model-A worktree." That diagnosis was wrong, and the way it was wrong is the lesson.)*
   ...
   **What is actually established**: ~~fresh sessions are **deterministic** (Pard, 6/6 headless `verify-hooks` with attribution)~~ — ⚠️ **CONTESTED as of 2026-07-26, see below.** A second long-lived seat is deterministic (HOST, 4/4). CIO's long-lived seat blocked **1 of 5** probes across one four-hour window and **4 of 4** two hours later, with no config change — file shape, command shape, config drift and single-layering all tested and excluded. **The variation is real, unexplained, and on a timescale of hours.**
   ⚠️ **"Fresh sessions are deterministic" has a counterexample (PA, 2026-07-26).** PA's first session on Amber — provisioned with a same-day headless PASS — **bypassed on its first probe**, then blocked on the next three. Four probes, same branch, same file, same config, ~10 minutes: (1) compound w/ `$(date)` → **BYPASS**, commit succeeded; (2) bare `git commit` → BLOCK (user layer); (3) compound plain → BLOCK (project layer); (4) *identical shape to #1* → BLOCK (user layer). Probes 1 and 4 being the same shape with opposite outcomes **independently re-confirms that command shape is excluded**. Both layers were live and alternating on that seat.
   **The consequence that matters: a same-day headless `verify-hooks` PASS does not guarantee the next in-session commit is gated.** Treat the provisioning gate as necessary, not sufficient.
   **Untested hypothesis, recorded so someone can cheaply kill or confirm it** — probe 1 was the *first git-commit-shaped call of the session*; "lazy attach on first matching call" fits all four datapoints, fits HOST's mid-session attach, and would explain a 6/6 headless PASS if the harness makes any tool call before its probe. **n=1.** To test: on a fresh seat, probe immediately on arrival, then probe again. If it reproduces across two or three seats, that's the mechanism. Until then, **assume your session's first commit may be ungated.**
