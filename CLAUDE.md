...
2. ✅ **The pre-commit hooks were dead everywhere — an invalid matcher, not a worktree problem. FIXED and behaviorally verified 2026-07-25.** *(This item previously read "project hooks do not fire in a Model-A worktree." That diagnosis was wrong, and the way it was wrong is the lesson.)*
   ...
   **Two properties worth carrying**: (a) **hook settings reload LIVE — they are not snapshotted at session start**, so a config fix takes effect on your next tool call with no restart; (b) a genuine block may surface as `hook error: [check-branch.sh]: No stderr output`, because the script writes its guidance to stdout — **that is the hook working**. Key on whether the refusal *names the hook*.
   ⚠️ **Property (a) has a known exception — do not assume it covers you** *(CIO, 2026-07-25, two reproducible probes)*. A session that started **before a `hooks` key existed at all** in the settings file does **not** pick it up live. Concretely: CIO's session started 10:48, the user-level `hooks` key was created ~13:55, and after the matcher fix landed CIO's probe **still committed a `mailboxes/` file on a non-main branch with no block — twice**, while HOST's session (started 15:36, after the key existed) blocks correctly. Working hypothesis, consistent with both seats: **live reload applies to *edits* of a hooks config that was present at session start; it does not retroactively attach a hooks key added mid-session.** So: **verify behaviorally on your own seat rather than inferring from someone else's** — and if your session predates the config's introduction, restart. The general rule this file already states applies to property (a) itself.
