...
2. ✅ **The pre-commit hooks were dead everywhere — an invalid matcher, not a worktree problem. FIXED and behaviorally verified 2026-07-25.** *(This item previously read "project hooks do not fire in a Model-A worktree." That diagnosis was wrong, and the way it was wrong is the lesson.)*
   ...
   **One property worth carrying, and one claim that did not survive the day**: (b) ✅ a genuine block may surface as `hook error: [check-branch.sh]: No stderr output`, because the script writes its guidance to stdout — **that is the hook working**; key on whether the refusal *names the hook*. (a) ⚠️ *"hook settings reload live, so a config fix takes effect on the next tool call with no restart"* — **asserted 2026-07-25 and since refuted; see immediately below. Do not act on it.**
   ⚠️ **Property (a) is UNRESOLVED — three models proposed today, all refuted. Do not rely on any of them.** *(CIO + HOST + Pard, 2026-07-25, each refuted by the next round of evidence.)*
   - ❌ *"Live reload is universal"* — refuted: CIO's seat had the corrected matcher on disk at 16:33 and did not block at 16:35 or 16:37.
   - ❌ *"Live reload applies to edits, not to a key added mid-session"* (CIO) — refuted by HOST reproducing a mid-session `settings.local.json` attaching immediately.
   - ❌ *"Project scope re-reads per invocation; user scope is read once at session start"* (HOST, confirmed by CIO on one test, written here) — **refuted 22:39**: on CIO's seat — a session that began at 10:48, before the user-level key existed at ~13:55 — probes were caught **alternately by the project layer (relative script path) and the USER layer (absolute path)**. The user layer attached to a session that predates it.
   - ❌ *"A single-layer seat explains the intermittency"* (HOST) — refuted by the same run: both layers are live on the intermittent seat.
   **What is actually established**: ~~fresh sessions are **deterministic** (Pard, 6/6 headless `verify-hooks` with attribution)~~ — ⚠️ **CONTESTED as of 2026-07-26, see below.** A second long-lived seat is deterministic (HOST, 4/4). CIO's long-lived seat blocked **1 of 5** probes across one four-hour window and **4 of 4** two hours later, with no config change — file shape, command shape, config drift and single-layering all tested and excluded. **The variation is real, unexplained, and on a timescale of hours.**
   ⚠️ **"Fresh sessions are deterministic" is REFUTED, and command shape is BACK IN — it was excluded too early.** Three fresh Amber seats probed independently on 2026-07-26 (PA 13:15, PPM 13:30, CXO 13:05), all provisioned with same-day headless PASSes. **14 datapoints pooled:**

   | Command shape | BLOCK | BYPASS |
   |---|---|---|
   | **standalone** `git commit` (staged separately) | **4** | **0** |
   | **compound** `… && git add … && git commit …` | 3 | **7** |

   **Every bypass ever recorded, on every seat, was a compound command. No standalone commit has ever bypassed (0 of 4).** Compound appears **necessary but not sufficient** for a bypass — which is why single-seat reads disagreed: CXO saw compound bypass 3/3 and called it a discriminator; PA saw compound block 2/2 and called shape excluded. Both were reading one side of a necessary-not-sufficient condition. **Do not repeat PA's error of promoting "not sufficient" to "excluded" — that claim sat in this file for about an hour and was wrong.**
   **The consequence that matters, and it is worse than flakiness:** `git add <paths> && git commit -m …` **is the compound shape**, and it is how essentially every agent in this cohort commits. The shape that bypasses is the routine one; the shape reliably caught is the standalone form you only use when deliberately testing. **So the hook reads as alive whenever probed and is largely absent during ordinary work** (CXO confirmed two real commits in-session were never hook-checked). A net that passes its own test and misses live traffic.
   ✅ **Actionable mitigation available today, no config change**: if you want a commit actually gated, **stage first, then commit as a standalone call** — `git add …` as one call, bare `git commit -m …` as the next. That shape is 4/4 caught across three seats.
   **Status of the two live hypotheses**: *lazy attach on first matching call* (PA/PPM — both fresh seats bypassed their first git-commit-shaped call) is **not sufficient alone**: CXO's probes 2–4 bypassed without being first-call, so first-call is not necessary for a bypass. It may still be a real second factor. *Compound-necessary* is the strongest surviving model. **Open gap**: PPM's probe-1 command shape is unconfirmed; if it was standalone it would break the 0-of-4 and should be reported loudly. **n is small (4 standalone trials) — this is the best current model, not a proven mechanism.**
   **So the operative guidance is behavioral, not modelled**: **verify on your own seat, and re-verify — a result in either direction has a shelf life.** A gate should be **two probes separated by real time**, never one. Read *which script path* a refusal names — **relative = project layer, absolute = user layer** — because that is the only cheap way to see which layer caught it. And **do not consolidate the two hook layers** while the intermittency is unexplained; removing redundancy you don't understand is how you tidy your way into a silent gap.
   **The standing rule this earned**: **verify behaviorally, never by config presence** — an absent hook and a silent hook look identical. And the second-order rule, which is what actually cost us a fix cycle: **a diagnosis of a silent mechanism carries the same evidentiary burden as the mechanism itself.** The worktree diagnosis above was plausible, widely believed, written into this file, and never tested.
...
3. **Mailbox writes always commit to `main`** — never on feature branches. Mail is cross-agent infrastructure; trunk only. Hook-enforced (see below) — and as of 2026-07-25 that is **verified behaviorally**, not just configured; before that date the hook had never once fired.
