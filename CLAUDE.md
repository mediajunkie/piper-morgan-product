...
2. ✅ **The pre-commit hooks were dead everywhere — an invalid matcher, not a worktree problem. FIXED and behaviorally verified 2026-07-25.** *(This item previously read "project hooks do not fire in a Model-A worktree." That diagnosis was wrong, and the way it was wrong is the lesson.)*
   ...
   **Two properties worth carrying**: (a) **hook settings reload LIVE — they are not snapshotted at session start**, so a config fix takes effect on your next tool call with no restart; (b) a genuine block may surface as `hook error: [check-branch.sh]: No stderr output`, because the script writes its guidance to stdout — **that is the hook working**. Key on whether the refusal *names the hook*.
   ⚠️ **Property (a) is SCOPE-CONDITIONED — it is true of project settings and false of user settings.** *(HOST + CIO, 2026-07-25, settled by reproduction on both seats.)*
   - **Project scope** (`.claude/settings.json`, `.claude/settings.local.json`) — **re-read per invocation.** A hooks key created mid-session attaches on the very next tool call. Reproduced independently on two seats, including one where user-scope hooks demonstrably do *not* attach.
   - **User scope** (`~/.claude-pm/settings.json`) — **read once at session start.** A session that began before a user-level hooks key existed never picks it up. CIO's session (10:48) predates the user-level key (~13:55) and still commits `mailboxes/` files on a non-main branch with **no block, twice**, after the matcher fix landed.
   **So: after a user-level hooks change, existing sessions must restart. After a project-level one, they need not.** And **verify behaviorally on your own seat** rather than inferring from someone else's — the two seats above disagreed precisely because each generalized from one observation.
   *How this was settled is the point*: CIO proposed a timing-based narrowing; HOST refuted it with a direct reproduction and proposed scope instead; CIO ran HOST's one-minute discriminating test on the seat that could actually separate them. Two wrong generalizations produced a right answer in under an hour because each was written down as a hypothesis and handed to the other to break.
