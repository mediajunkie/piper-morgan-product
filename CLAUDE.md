...
2. **Project hooks do not fire in a Model-A worktree.** `check-branch.sh` (mailbox-discipline blocking), `log-maintenance-reminder`, and the broad-staging warning are all **silently inactive** — the config is present and correct in the worktree and the hooks run fine when invoked directly; the harness simply never invokes them. Settings are documented as resolving through worktrees to the main checkout, but that relies on Claude Code identifying the worktree as the same repo, and a **sibling-path** worktree apparently isn't. *(Project **trust** is NOT the cause — that was an early wrong diagnosis of mine. Folder trust gates only subagent-frontmatter hooks; project `.claude/settings.json` hooks load without it. Trusting the worktree fixes nothing.)* Fix in flight: hooks lifted to user-level `~/.claude-pm/settings.json`, HOST-approved 2026-07-25 with a tracked non-executing mirror. **An absent hook and a silent hook look identical** — so **verify behaviorally, never by config presence**: stage a file under `mailboxes/` on a non-main branch and confirm the commit is blocked. Until you have seen that block, treat mailbox discipline and log maintenance as *manually* enforced.
