...
| Exploratory Testing Agent (ETA) — *dormant, last session March 2026* | `BRIEFING-ESSENTIAL-ETA.md` | `test-code` |
...
**The mechanical form** (added 2026-08-22, CIO — from cross-project `/insights` reports surfacing the same failure class in two other repos: an invented "49 imports" figure, a file-history diagnosis stated backwards): run the check and quote its actual output in the same turn as the claim. If you have not run it this turn, say **"unverified"** rather than stating it as fact — don't let a check you ran five minutes or five files ago silently stand in for one you'd run now. This is the same discipline as "Name the layer, and state the denominator" below, applied at the point of making an individual factual claim rather than at the point of reporting a check's result.

⚠️ **A worktree you synced earlier this session is not synced now — this bites specifically when a duty-cycle sync happens at a fire, then hours pass with PM engaging you directly (not via a cron fire) in between.** (Added 2026-08-27, Docs — real incident: synced clean at a morning duty-cycle START, then answered a PM question about a draft's frontmatter and ran a full publish audit hours later using that same stale checkout, without re-fetching first. 33 commits — including the very file being audited, PM's full voice pass and the missing art PM had just added — had landed on `origin/main` in the gap. PM caught it by asking "are you synced with origin main?" rather than trusting the stale answer.) **Any time you're about to read file/git state to answer a question or start work — not just at a scheduled fire's START — `git fetch` + fast-forward first if it's been more than a few minutes or any PM/other-agent activity could plausibly have landed.** A sync from earlier in the session is a check you ran "five minutes or five files ago" in exactly the sense the paragraph above warns about — it just doesn't feel that way because "I synced this session" reads as a durable fact instead of a timestamped one.
...
**Dispatch tier — state it, don't inherit it** (PM ruling, 2026-09-14, closing a real incident: one seat's 30 same-day subagent dispatches silently inherited its own top-tier model, exhausting a shared usage ceiling that two unrelated, non-dispatching seats then paid for). A fan-out inherits the dispatcher's model unless the dispatch call's `model` parameter is set explicitly — pass it deliberately rather than let it default by omission. PM's principle: **the dispatcher holds the judgment (planning, prompt design, acceptance criteria, review); the subagent executes against it** — a subagent rarely needs the dispatcher's own top tier to do that. Before dispatching, ask: *does this prompt carry enough specification that a cheaper tier could execute it? If not, is that deliberate, or is the prompt underspecified?* Rough guide, not a mechanical rule — judge the actual unit of work: Haiku for mechanical/fully-specified work (sweeps, inventories, format conversions); Sonnet for bounded implementation against clear criteria (most issue-level work); Opus for genuinely hard reasoning inside the unit (explicitly fine, not a ceiling to apologize for); the dispatcher's own top tier rarely, and reaching for it is a signal to look at the prompt first. Full context and the audit-cascade mechanism: `.claude/skills/audit-cascade/SKILL.md` Step 1b.

**Log the tier you assigned — the rule existed, compliance was unverifiable** (PM ruling, 2026-09-20, closed via Exec's routing: a same-week check found one dispatch stating its tier explicitly against two stating none at all — the rule above was real and unmeasurable, the exact "structural fixes hold, promises don't" shape this cohort keeps re-finding). Two halves, one per side of the dispatch:
- **The dispatcher** notes the model tier it assigned in the same session-log entry that records the dispatch (e.g. "dispatched a Coding Agent subagent, Sonnet, for #1234" — not a separate ceremony, just don't omit the tier when you log the dispatch).
- **A subagent that writes its own session log** (prog role, or any substantive multi-step subagent per the logging rules above) records the model it observes at start — this is already the existing header convention (see Session Discipline's "Model is tracked in the log header" below), so the only actual gap this closes is the dispatcher's half.

Composes with the model-sourcing discipline (a model claim quotes what it's sourced from, or says "unsourced," rather than stating a tier as fact without saying how it's known) — same principle, applied on both sides of a dispatch.
...
**After pushing, sync PM's local checkout**: run `scripts/sync-pm-local.sh` (no args). Fast-forwards PM's local main (`--ff-only`). *(Corrected 2026-08-16, Docs — this line described v1 (2026-07-04) behavior; the script has been v2 since 2026-07-07 and the old description was actively misleading, not just outdated.)* **Not** a binary "abort if anything's dirty" guard — the script runs a 3-tier path classifier (#1368: PA proposed, CIO reviewed, Docs added the content-heuristic refinement): mailbox MANIFESTs/carry-forwards/session logs (tier 1, always safe, cleared surgically via `git checkout origin/main -- <path>`) and decisions.log/editorial-calendar (tier 2, cleared only on a whitespace-only or no-growth diff) get cleared per-path; PM's actual prose WIP (tier 3, everything else) is never touched, full stop, and never blocks the tiers 1/2 clear or the final pull — a tier-3 hit only holds back that one file. Run at natural idle points, not after every commit. Autonomous sessions are allowlisted in `.claude/settings.json` — if it's still being denied or no-oping unexpectedly, flag it rather than routing around it.
...
**GitHub auto-close ignores negation — and there is now a mechanism, not just this warning (#1691, 2026-09-24)**: Pairing a `close/fix/resolve` keyword with `#N` in a commit message closes the issue regardless of surrounding wording — "not yet resolved: #1278" closed a live Beta Blocker in 2026-07, and a MAIL subject "ask(ppm): close #1677/#1488 properly" closed #1677 in 2026-08 with nobody deciding anything. `scripts/check_autoclose_keywords.py` refuses such a message at both doorways: `mail-send.sh` (the commit-tree path git hooks can't see) and the `autoclose-guard.sh` PreToolUse hook on `git commit`. Write the number without `#`, or keep the keyword away from it; a commit that genuinely closes an issue opts in with the trailer line `Auto-Close: intentional`.
